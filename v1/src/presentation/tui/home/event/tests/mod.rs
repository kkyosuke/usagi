...
/// Like [`run_full_external`] but with a custom `dispatch_recover`, so a test can
/// see which quarantine recovery the palette routed to the background worker.
fn run_full_recover(
    keys: Vec<io::Result<Key>>,
    state: HomeState,
    dispatch_recover: &mut dyn FnMut(&Path, &str, crate::usecase::session::QuarantineRecovery),
) -> Result<Outcome> {
    let term = Term::stdout();
    let mut reader = ScriptedReader::new(keys);
    let monitor = MonitorHandle::detached();
