...
3. **基点ブランチへのコミット検証**: 委譲先ワークツリーの基点コミット（プロジェクトの default_branch 設定に応じた、`origin/main` または `main`）の `.usagi/issues/` を列挙し、**issue 番号（`NNN-` 接頭辞）で**対象 issue のファイルを解決する。1 件に解決できたときだけ次へ進み、0 件なら未コミットエラー、2 件以上なら曖昧エラーを返します（[番号による解決](#基点コミット検証は番号で解決する)）。
...
issue の**ファイル名は title から導出できない**。ストアが書き込むときの正規名は `NNN-<title の slug>.md` だが、
markdown を直接書いて起票した issue はその名前のままコミットされ、起票後に title を変えた issue も次のストア書き込みまで
古い名前を保つ。ストア自身の読み出しは番号接頭辞（`NNN-`）で解決するため、on-disk 名が正規名と違っても
`issue_get` / `issue_search` / `issue_to_prompt` は成功する。

したがって基点コミット検証も**番号で解決**する。`git ls-tree` で基点コミットの `.usagi/issues/` を列挙し、
`NNN-` 接頭辞が対象番号に一致する markdown を数える。

| 一致した件数 | 挙動 |
|---|---|
| 1 | コミット済みとして委譲を続行する |
| 0 | 未コミットエラー。基点コミットに無い issue は委譲先ワークツリーに現れない |
| 2 以上 | 曖昧エラー。番号と一致した全ファイルを列挙して拒否し、セッションを作らない |

title から導出した名前を path として探すと、**読めているのにコミット済み判定だけ落ちる** false negative になり、
委譲そのものが blocked になる。2 件以上を拒否するのは、どちらの sibling が本物か決められないまま委譲すると
セッションを誤った作業に向けてしまうためで、ストアの重複番号（`AmbiguousIssueNumber`）と同じ方針である。
