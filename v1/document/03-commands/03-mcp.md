...
| チャネル | キュー先 | 配送タイミング |
|---|---|---|
| 起動時キュー（queue） | [`agent-prompts/`](../data/01-global.md#agent-prompts) | 明示 `mode="queue"` の prompt は、そのセッションのエージェントペインを**次にフレッシュ起動するとき**に、エージェントの**最初のメッセージ**として渡す。`auto` が live pane 無しと判定して queue を選んだ prompt だけは、TUI が既存 Agent pane を持つ場合（既存 Agent pane の復旧後）、その Agent の**次のメッセージ**として渡せる。設定 [`autostart_queued_prompts`](../05-settings.md#設定項目)（既定 ON）が有効なら、TUI 稼働中は人がペインを開くのを待たない（[4. オーケストレーション#キュー済みプロンプトの自動起動](../04-orchestration.md#キュー済みプロンプトの自動起動)） |
