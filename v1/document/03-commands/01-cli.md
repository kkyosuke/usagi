...
`issue` / `memory` / `mcp` / `llm-mcp` は **AI エージェントが MCP 経由で扱うためのコマンド**で、`usagi --help` の一覧には表示しません（実行自体は可能）。人手で叩くものではないため、ヘルプを汚さないよう隠しています。`agent-phase`（エージェントのライフサイクルフックが状態を報告するために呼ぶ内部コマンド）と `guard-workspace`（session 行の Claude の `PreToolUse` フックがツールの対象パスを検査し、worktree の外への読み書きを拒否する内部コマンド）も同様に隠しコマンドで、人手で実行するものではありません。
...
| `usagi config --edit` | （ヘルプ非表示・上級者向け）グローバル設定の private copy を `$EDITOR` で開き、検証後に編集開始時から shared file が変わっていない場合だけ保存する |
...
- `--edit`: 設定の mode 0600 相当の private copy を `$EDITOR`（→ `$VISUAL` → OS 既定の `vi` / `notepad`）で開いて編集します。
  `$EDITOR="code --wait"` のように引数付きの値も POSIX シェル規則で分割して扱います（シェルは起動しません）。
  editor の終了までは shared `settings.json` を変更しません。保存後に再パースして形式（JSON 構文・必須
  `version`・各フィールドの型）を検証し、編集開始時の内容と現在の shared file が一致する場合だけ atomic に
  反映します。編集中に Config 画面など別 process が同じフィールドまたは別フィールドを更新した場合は conflict として
  candidate を反映しません。invalid JSON、private copy の削除、editor の失敗でも shared file の最新状態を維持します。
...
`doctor` / `doctor --fix` は `orphaned` session を自動修復・削除しません。session reconcile が ownership proof の不足を報告した場合は、[state.json との同期](../04-orchestration.md#statejson-との同期孤児セッションの隔離)に従い、`state.json` の recorded provenance と expected repository の `git worktree list` を読み取りで照合してから manual cleanup します。branch 名の一致だけで所有対象と判断してはいけません。
