...
- **Closeup**: 選択中セッションの中を操作する。Closeup モーダル（Menu / Prompt）またはライブな埋め込み端末を扱う。

`:` は **Overview モーダル**（Workspace スコープのコマンド面）を開き、`Ctrl-O a` は **Closeup モーダル**（Session スコープのアクション面、タイトル `Closeup: <name>`）を開きます。ライブ端末は Closeup の内部状態であり、第 3 のモードではありません。
...
| # | ドキュメント | 内容 |
|---|---|---|
| 5.1 | [01-modes.md](01-modes.md) | Switch / Closeup と Overview / Closeup モーダル |
