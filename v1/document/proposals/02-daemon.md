...
- 端末は daemon が spawn 時に採番する **terminal id** で指す（1 worktree に agent＋terminal 複数タブが同居するため、worktree パスだけでは端末を特定できない）。実装済みのメッセージ:
  ...
  | 方向 | メッセージ | 内容 |
  |---|---|---|
  | TUI⇄daemon | `Hello { build }` | terminal 操作前に executable generation を相互確認する。不一致（`cargo run` の再ビルド前 daemon を含む）なら手動の新規ペインはローカル PTY へフォールバックする。保存済み terminal の復旧は fallback しない。queued prompt の自動起動は daemon 不通時には fallback せず、build mismatch かつ永続 registry が対象 worktree の terminal 非在を証明するときだけ fallback する |
  ...
  | TUI→daemon | `Attach { terminal, worktree }` | 端末購読を開始（worktree の照合つき — 古い保存 id が別 worktree の端末に化けない）。成功は `Attached { terminal, pid }`＋viewport `Screen`、拒否は `AttachRejected { terminal, reason: missing | adopted }`。`missing` だけが fresh fallback 可で、`adopted` / transport error は ownership 不明として二重 spawn を止める |
  ...
  | TUI→daemon | `Keys { terminal, data, request_id? }` | キーストロークを PTY へ転送。通常の対話入力は `request_id` を省略して fire-and-forget、durable prompt input は id を付ける |
  | daemon→TUI | `InputResult { request_id, terminal, error? }` | id 付き `Keys` の PTY write 結果。成功時は `error` なし、missing terminal / write failure は理由を返す |
...
- Durable prompt input は `InputResult` を 2 秒待ち、negative ACK / timeout で永続キューへ戻す。timeout は PTY write 済みで ACK だけ失われた場合も区別できないため現状は at-least-once で、request id の永続化や再送 dedupe は行いません。失敗後の `Kill` も `Killed` ACK だけを teardown 証明とし、reader disconnect や既送信だけでは成功扱いしません。
...
#205 では terminal の `Spawn` / `Attach` より先に `Hello { build }` を往復する executable-generation handshake を追加した。build は package version に加えて実行ファイルの device / inode / size / mtime を含むため、version が同じ `cargo run` の再ビルドも区別する。不一致や `Hello` を持たない旧 daemon では terminal 操作を送らず、手動の新規ペインは既存 daemon とその Agent を止めずにローカル PTY へフォールバックする。保存済み terminal id の復旧は local fallback しない。queued prompt の自動起動は daemon 不通時には local fallback せず、build mismatch かつ `terminals.json` が対象 worktree の terminal 非在を証明する場合だけ fallback する。registry 不在・読み取り失敗・対象 terminal ありの場合は、旧 Agent が生存したまま別 Agent を起動するのを防ぐため失敗または再試行として記録する。
