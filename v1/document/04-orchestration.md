...
usagi の自律オーケストレーションは、次の 3 原則の上に成り立ちます。MCP・Git hook・ブランチ保護の
[ガードレール](#ガードレール)が運用を支えます。
...
「root は repo を変更しない」を、MCP・Git hook・ブランチ保護で担保します。
...
を編集したり親リポジトリへ `cd` したりすると、意図したセッションとは別のツリーを触ってしまいます。そのため、
usagi は「作業はこの worktree 配下だけで完結させ、親のメインリポジトリには触れない」旨を Agent に伝えます。
system prompt を持つ CLI は system prompt（Claude の `--append-system-prompt` ／ Codex の
`developer_instructions`）で、持たない CLI は**開始プロンプト**（`-i` ／ headless の `-p`）の先頭にこの指示を置き、
キュー済みプロンプトはその後ろに続けます。

Claude は usagi 独自の OS sandbox や `PreToolUse` workspace guard で包まず、Claude CLI を直接起動します。
したがって、この worktree 指示は強制的な filesystem 境界ではありません。Claude 自身の permission 設定と、
上記の Git/MCP ガードレールが適用されます。起動中の既存 Claude process は旧 wiring のままなので、更新後に
agent pane を閉じて再起動してください。
