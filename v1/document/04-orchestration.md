...
- [state.json との同期（孤児セッションの隔離）](#statejson-との同期孤児セッションの隔離)
- [スキルの配布](#スキルの配布)
...
「root は repo を変更しない」を、MCP・Git hook・ブランチ保護で支えます。root 行の Claude は project root を
書き込み可能な sandbox root として起動するため、直接のファイル操作は規約に従います。
...
| B（backstop） | pre-commit — 非セッションチェックアウトのコミットを拒否（[06-conventions.md#git-hookslefthook](06-conventions.md#git-hookslefthook)） | 人手・Agent のコミット | worktree が `.usagi/sessions/` 配下か |
| C（リモート） | `main` ブランチ保護 — 直 push 禁止＋PR 必須（[06-conventions.md#cigithub-actions](06-conventions.md#cigithub-actions)） | リモートの直更新 | サーバ側 |

読み取り・整形・session 操作（`issue_search` / `issue_get` / `issue_to_prompt` /
...
- **既存のリンク worktree**（`.git` がディレクトリでなくファイル＝他所で管理されている
  ...
  ブランチ作成もしない。
- **git 管理外の通常ファイル・ディレクトリ** → 同じ相対パス
  `.usagi/sessions/<name>/<相対パス>/` へコピーする。走査前に source root を canonicalize し、各 entry は
  source root からの相対パスとして session root 配下にだけ配置する。
- **git 管理外の symlink** → file / directory / dangling のいずれも拒否する。リンク先をたどらず、リンク自体も
  session tree には作らない。
- **FIFO・device・Unix socket などの special file** → open / copy せず拒否する。

> `.git` / `.usagi` も走査対象から除外されます。

non-Git tree はコピーを始める前に全 entry を `symlink_metadata` で分類します。symlink、special file、source root
外への traversal、session root 外への配置を検出した場合、`session create` は typed error で失敗します。構築途中の
Git / filesystem error でも、その試行で作った worktree・branch・session tree を rollback し、`state.json` に session を
記録しません。

> **Security note:** この規則を導入する前に作成した既存 session には、ワークスペース外を指す file symlink のリンク先
> （secret を含む）が通常ファイルとして複製済みの可能性があります。usagi は既存 session を自動削除しません。該当する
> session tree を確認し、必要に応じて `session remove` で削除して secret を rotate してください。
...
## state.json との同期（孤児セッションの隔離）

`session create` / `session remove` の実行時に、`.usagi/sessions/` 配下のディレクトリと `state.json` の記録を照合します。**`state.json` に記録のないディレクトリ**（中断された作成・手で編集された `state.json`・クラッシュなどで取り残されたもの）は、所有権を証明できない「孤児」として `pending_removals` に `orphaned` を記録し、ディレクトリ・worktree 登録・ブランチをそのまま保持します。

- `orphaned` は自動 force delete しません。同名セッションの再作成も既存データを上書きせず失敗するため、ユーザーが内容と所有権を確認して修復します。
- セッション作成時は source repository と worktree の canonical path を `worktree_provenance` に組で記録します。削除時は expected repository・recorded worktree identity・session root への canonical containment・`usagi/<name>` branch の**積集合**だけを managed target とします。branch 単独や raw path fallback は ownership 根拠にしません。
- root / worktree / repository の canonicalize 失敗、symlink、provenance の無い旧 schema、同一 branch の外部 worktree、別 repository の候補、recorded path 上の別 branch は、Git effect より前に全候補の preflight で検出して `orphaned` に隔離します。`--force` を指定してもこの判定は緩みません。
...
- 記録済みセッション本体の削除には引き続き未コミット変更のガード（`--force` 必須）が効きます。
- provenance の無い既存 stray や、worktree 実体だけが消えて branch / Git registration が残る対象は自動回収しません。`state.json` と各 expected repository の `git worktree list` を照合し、必要な変更を退避したうえで、確認済みの repository で `git worktree remove <path>` / `git branch -D usagi/<name>` を実行し、最後に marker を手動修復します。通常 teardown が effect 後にクラッシュした場合は、保存済み provenance と残存 target を再照合し、消去済み component を成功扱いにして冪等に再開します。
...
| 破棄 | `session remove [<name>] [--force]` | durable marker を保存し、全 managed Git teardown の成功後に会話履歴と worktree keyed context（Agent phase・PR リンク・キュー済みプロンプト・ペイン構成）を削除して session record を commit（未コミット変更があれば `--force` 必須） |
...
各 worktree のシェル / Agent は TUI が所有する PTY で動き、ターミナルプールが worktree パスをキーに保持します。セッションを切り替えても同じ TUI の間は端末と Agent CLI が走り続けますが、TUI を閉じると PTY とその子プロセスも終了します。`session remove` は次の二段階で worktree 配下のシェル / Agent、会話履歴、Agent phase・PR リンク・キュー済みプロンプト・ペイン構成を削除します。

1. `git_teardown`: session identity、会話、phase、PR、queue、pane を保持したまま、全リポジトリの managed worktree・ブランチ・コピーを取り外します。
2. `context_cleanup`: Git teardown 完了を永続化してから ancillary context を冪等に消し、最後に session record と marker を同じ `state.json` 更新で落とします。

途中で失敗またはプロセスクラッシュした場合は session record と `pending_removals` が残ります。`session_status` の `removal` は `git_teardown` / `context_cleanup`、ownership proof が曖昧なら `orphaned` を返します。通常の locked/dirty worktree や ancillary storage は修復後に同じコマンドを再実行すると、既に消えた component を成功扱いにして残りから再開します。`orphaned` は再実行しても自動削除せず、上記の manual cleanup を要求します。
...
| 主境界 | 全 Claude command（対話・再開・headless）を `usagi claude-sandbox` 経由で platform OS sandbox 内に起動する | Claude（macOS / Linux） | canonical な allow root 以外への書き込みを OS が拒否する。hook を迂回する Bash・wrapper・absolute executable・redirection・symlink にも適用される |
| 補助防御 | session 起動の `PreToolUse` フックに `usagi guard-workspace` を差し込み、malformed・未知・明白に不正なツール呼び出しを fail closed で拒否する | session 行の Claude | Agent へ早い理由提示を行う。shell の hard boundary ではない |

OS sandbox は起動 cwd と追加の writable root を `canonicalize` してから policy を構築します。存在しない root、解決不能な
path、sandbox executable の欠落、sandbox process の起動失敗はいずれもエラーにし、Claude を無保護では起動しません。
session 起動では session worktree、workspace の `.usagi`、Git common directory、Claude/usagi の専用 state directory を
書き込み可能にします。root coordinator 起動では project root（起動 cwd）も書き込み可能にします。どちらの mode でも
platform の `$TMPDIR` と、存在する `/tmp`・`/var/tmp` を書き込み可能にし、通常の CLI が一時ファイル・socket・lock を
作成できるようにします。それ以外の filesystem は読み取りのみ可能です。
usagi 自身がすでに macOS sandbox 内で動いている場合は子 process が親の policy を継承するため、usagi 側から親の
許可範囲を追加で広げることはできません。

| platform | backend | unavailable 時 |
|---|---|---|
| macOS | OS 標準 `/usr/bin/sandbox-exec` | Claude を起動せず明示エラー |
| Linux | `bwrap`（bubblewrap。PATH 上に必要） | Claude を起動せず明示エラー |
| Windows / その他 | 対応 backend なし | Claude を起動せず unsupported エラー |

session 起動で使う `guard-workspace` は `PreToolUse` payload（stdin の JSON）から `cwd` / `tool_name` / `tool_input` を読み、`cwd` を
canonicalize して session / root モードを選びます。JSON 不正、必須 field 欠落、cwd/path の canonicalization failure、
未知 tool は拒否します。

- **session モード**: file write tool の target は、存在する最深 ancestor を canonicalize して symlink を解決し、session
  worktree 外なら拒否します。Bash と subagent は OS sandbox を継承するため、shape を検証したうえで sandbox に委ねます。
拒否時は Claude の `PreToolUse` 契約どおり `permissionDecision: "deny"` を stdout に返します（理由も添える）。
許可する場合は何も出力せず、Claude の通常の許可フローに委ねます。

- session 起動では `guard-workspace` を状態報告の `agent-phase` と同じ `PreToolUse` 配列に並べて差し込みます。Claude は同一イベントの
  フックをすべて実行し、いずれかが拒否すればツールはブロックされるため、状態報告と保護が両立します。
- root 起動は `guard-workspace` を差し込まず、project root と一時領域への操作を OS sandbox policy に従って許可します。
- 起動中の既存 Claude process は旧 hook / sandbox wiring のままなので、更新後に agent pane を閉じて再起動してください。
...
- **検知の契機**は 2 つです。(1) **起動時**: TUI が起動していない間にキューされたプロンプト（例: 別プロセスの
  ...
  **没入（Attached）中のペインループ**でも同じ間隔で回ります。コーディネータ役のエージェントは自分が没入している
  ...
  起動しませんでした）。
- 既に **live agent ペイン**を持つセッションは新規 spawn しません。live agent ペインが検知される（live marker がある）
  セッションへの明示 `session_prompt(mode=queue)` は、起動時キューが次の **fresh launch** しか消費しない以上そのまま
  滞留する（＝受理されたのに何も起こらない）ため、MCP が actionable error で拒否します（`mode=auto`/`live` で稼働中
  Agent へ送るか、session を作り直すよう案内）。live marker が無いセッション（TUI 不在など）への `mode=queue` は従来
  どおり次の fresh launch 用に起動時キューへ残します。一方、既存 Agent pane は TUI を閉じても生存しますが、
