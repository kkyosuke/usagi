...
| 補助防御 | `PreToolUse` フックに `usagi guard-workspace` を差し込み、malformed・未知・明白に不正なツール呼び出しを fail closed で拒否する | Claude | Agent へ早い理由提示を行う。shell の hard boundary ではない |
...
書き込み可能にします。root coordinator 起動では repository の working tree を read-only に保ち、workspace の `.usagi` と、
MCP による session delegation に必要な Git common directory だけを書き込み可能にします。root hook は Bash からの Git 変更を
別途拒否します。一時ファイルは広い OS temp directory を許可せず `~/.usagi/tmp/claude` に固定します。
filesystem 全体の読み取りは可能です。
...
`guard-workspace` は `PreToolUse` payload（stdin の JSON）から `cwd` / `tool_name` / `tool_input` を読み、`cwd` を
...
- **root モード**（`cwd` がワークスペースルート＝ `.usagi/sessions/` 配下でない。コーディネータの行）: root 行は
  ...
  - `Bash` は単純な read-only command allowlist だけを許可します。読み取り系 git と `rg` / `grep` / `ls` 等は許可し、
    shell interpreter、wrapper、redirection、command substitution、`sed` / `rm`、変更系・未知 git、未知 executable は拒否します。
