...
途中で失敗またはプロセスクラッシュした場合は session record と `pending_removals` が残ります。`session_status` の `removal` は `git_teardown` または `context_cleanup` を返し、エラーは修復対象と `session remove <name>` の再実行を案内します。locked/dirty worktree や ancillary storage を修復して同じコマンドを再実行すると、既に消えた component を成功扱いにして残りから再開します。
