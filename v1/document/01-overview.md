...
- **CLI コマンド**: `usagi init` / `hop` / `run`（うさぎアニメのギャラリー）/ `status` / `clean`（放置セッションを AI が自律整理）/ `config` / `doctor` / `issue`（タスク管理）/ `mcp`（issue・memory・session の MCP サーバ）/ `llm-mcp`（ローカル LLM MCP サーバ）。
...
導入状況は `usagi doctor` で確認できます（git / bash と各 Agent CLI（Claude / Codex / sakana.ai / Gemini / Antigravity）の有無、デスクトップ通知の可否、設定ストレージの健全性、
