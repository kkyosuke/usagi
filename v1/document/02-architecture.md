...
│   ├── pr_link_store.rs        # worktree 別に検出した PR 群（PrLink のリスト）を正規化 URL 単位で重複排除しつつ蓄積/読み出し（~/.usagi/pr-links/）。sync が state.json の pr へ畳み込む
│   ├── pr_title.rs             # PR タイトル/状態を gh CLI で解決する純ロジック（argv 生成・stdout パース・retry/backoff metadata 更新）。実 gh 実行は terminal/pool の bounded worker が注入
...
テストは 3 つの粒度で配置する。すべて `v1/Cargo.toml` を対象にした `cargo test` で実行され、出荷元と同じ source tree を計測する v1 専用 CI で line/function カバレッジ 100% を要求する（[6. 開発規約](06-conventions.md#品質チェックリスク比例の-gate)）。
