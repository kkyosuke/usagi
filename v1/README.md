...
Claude の対話・headless 起動は workspace 隔離のため OS sandbox を必須とします。macOS は OS 標準 backend、Linux は `bwrap` を使用し、利用不能時や Windows では無保護にフォールバックせず起動を拒否します。session worktree と root 行の project root、通常の一時領域は書き込み可能です。allow root と hook の規則は [orchestration の「worktree への閉じ込め」](document/04-orchestration.md#worktree-への閉じ込めメインリポジトリ保護)を参照してください。
...
`usagi mcp` で同じ issue・メモリ操作を MCP（Model Context Protocol）サーバとして公開できます。Claude Code などに登録すると、1 つのサーバでエージェントが `issue_create` / `issue_search` / `issue_update` などの tool でタスクを、`memory_save` / `memory_search` などの tool でメモリを操作できます（一覧は `*_search` の `query` 省略で取得）。git 共有対象を main の checkout で直接変更しないよう、workspace root では issue とメモリの書き込み tool を拒否し、session worktree 内でのみ許可します。
