...
Claude の対話・headless 起動は workspace 隔離のため OS sandbox を必須とします。macOS は OS 標準 backend、Linux は `bwrap` を使用し、利用不能時や Windows では無保護にフォールバックせず起動を拒否します。allow root と hook の fail-closed 規則は [orchestration の「worktree への閉じ込め」](document/04-orchestration.md#worktree-への閉じ込めメインリポジトリ保護)を参照してください。
