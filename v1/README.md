...
Claude の対話・headless 起動は usagi 独自の OS sandbox や workspace guard で包まず、Claude CLI を直接起動します。worktree 内で作業するための指示と適用されるガードレールは [orchestration の「worktree 内での作業」](document/04-orchestration.md#worktree-内での作業)を参照してください。
