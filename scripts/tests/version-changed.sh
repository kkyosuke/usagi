...
script=$(cd "$(dirname "$0")/../.." && pwd)/scripts/ci/version-changed.sh
tmp=$(mktemp -d "${TMPDIR:-/tmp}/usagi-version-changed.XXXXXX")
trap 'rm -rf "$tmp"' EXIT
...
write_manifest 3.0.0
git -C "$tmp" add Cargo.toml
git -C "$tmp" commit -qm base
base=$(git -C "$tmp" rev-parse HEAD)

echo '# unrelated package metadata' >>"$tmp/Cargo.toml"
...
echo "version-changed fixtures: ok"
