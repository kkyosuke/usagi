...
mkdir -p "$tmp/scripts" "$tmp/src/domain" "$tmp/src/usecase" "$tmp/src/presentation/cli" "$tmp/tests" "$tmp/third_party/vt100/src" "$tmp/document"
...
touch "$tmp/tests/old_name.rs" "$tmp/space name.txt" "$tmp/README.md" "$tmp/document/06-conventions.md"
...
echo changed >"$tmp/document/06-conventions.md"
out=$(run)
assert_has "$out" "lychee --config lychee.toml"
assert_not_has "$out" "cargo test --quiet"
git -C "$tmp" checkout -q -- document/06-conventions.md
