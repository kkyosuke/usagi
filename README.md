...
裏で動いている `claude` が処理を終えて入力待ちになる（ターミナルベルを鳴らす）と、アタッチ中でないセッションについては左ペインの該当 worktree に `◆`（黄色）マーカーが付き、デスクトップ通知（`🐰 <ブランチ名> が入力待ちです`）で知らせます。複数セッションを並行で走らせ、入力が必要になったものだけに対応できます（通知は設定 `notifications_enabled` が ON のとき。検知はベルに依存するため `claude` 側でターミナルベル通知を有効にしてください）。
