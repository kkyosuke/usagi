...
| モード | 役割 | 主な操作 |
|---|---|---|
| **切替**（Switch） | 既定。セッションの選択・新規作成 | 左ペインで `↑↓` 選択・`←→`（または `Ctrl-N`/`Ctrl-P`。右ペインのタブチップの左クリックも同じ）でタブ切替・`Enter` 確定・`c` で新規作成・`r` で表示名変更・`n` でメモ編集（選択中の行のメモ＝次回 TODO は右ペインに表示。ルート行はワークスペースルートのメモ） |
