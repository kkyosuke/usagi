...
- **直接対話（`chat`）**: 在席（Focus）の `chat` コマンド／メニュー行でローカル LLM と対話する専用画面を開けます。外部 Agent CLI を起動せずローカルモデルに直接話しかけるので、ちょっとした質問にクラウド Agent のトークンを使いません（`local_llm.enabled` に関係なく `local_llm.model` のモデルを使用）。詳細は [document/03-commands/02-tui.md#chat](document/03-commands/02-tui.md#chat)。
