...
埋め込みシェルにフォーカス中、`Ctrl-O` はリーダーキーとして働きます。`Ctrl-O` 単独で**デタッチ**（シェルや `claude` は裏で動き続けたままサイドバーへ戻る）、`Ctrl-O` に続けて `n`/`p` で**別セッションのターミナルへ切り替え**られます。各 worktree のシェルは画面を開いている間ずっと生き続けるので、セッションを行き来しても作業中の `claude` はそのまま継続します。シェルを完全に終了したいときは、そのシェルで `exit` してください。
