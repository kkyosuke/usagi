...
左ペインの各セッションは 2 行で表示され、Agent が動いているセッションをアイコンでひと目で見分けられます。シェルや `claude` が**稼働中**の worktree には `▶`（緑）、その Agent が処理を終えて**入力待ち**になった worktree には `◆`（黄色）が 1 行目に付きます（入力待ちは稼働中より優先表示。2 行目に `running` / `waiting` を併記）。`◆`（入力待ち）になると、アタッチ中でないセッションについてはデスクトップ通知（`🐰 <ブランチ名> が入力待ちです`）でも知らせます。複数セッションを並行で走らせ、入力が必要になったものだけに対応できます（通知は設定 `notifications_enabled` が ON のとき。入力待ちの検知はベルに依存するため `claude` 側でターミナルベル通知を有効にしてください）。
