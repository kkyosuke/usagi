...
:terminal                # 選択中の worktree でシェルを右ペインに埋め込み起動（サイドバーの Enter でも可。Ctrl-O でセッション切り替え／デタッチ）
:agent                   # terminal を開いて設定中の Agent CLI（既定 claude）を起動。issue MCP サーバを組み込んで起動する
```

作成した worktree は左ペインに表示されます。目的の worktree を選んで `terminal` を実行すると、左ペインの一覧を表示したまま、その worktree を作業ディレクトリとしたシェルが**右ペインにライブで埋め込まれます**。そこで `claude` などの AI エージェントを起動して開発できます。`:agent` を使えば、この「`terminal` を開いて Agent CLI を起動する」操作を 1 コマンドで行えます（起動する Agent CLI は Config 画面・ローカル設定で選べます）。このとき usagi の issue MCP サーバ（後述）を Agent CLI に組み込んで起動するため、エージェントは起動直後から `issue_*` tool でタスクを操作できます（Claude は `--mcp-config` で注入。Gemini は `settings.json` 経由のため現状は組み込みません）。

サイドバーで worktree を選んでいるだけのときも、そのセッションに `claude` やシェルが動いていれば、右ペインはその**ライブ画面を読み取り専用でプレビュー**します（ブラウザのタブのように、カーソルを動かすと右ペインの内容も切り替わる）。操作したくなったら `Enter` でそのセッションのターミナルにアタッチします。

`Ctrl-O` はセッション切り替えのオーバーレイ（ルート行 + 各 worktree の一覧）を**ホーム画面のどこからでも**開けます（サイドバーでもアタッチ中でも）。番号 `1`–`9` または `↑`/`↓` + `Enter` で**別セッションのターミナルへ切り替え**（**切り替え先はそのセッション自身の状態を表示**——`claude` が動いていればその画面、何もしていなければ素のシェル）、`c` で**新規セッションを作成**してそのシェルへ入り、`Esc` で閉じて戻り、アタッチ中にもう一度 `Ctrl-O` で**デタッチ**（シェルや `claude` は裏で動き続けたままサイドバーへ戻る）できます。各 worktree のシェルは画面を開いている間ずっと生き続けるので、セッションを行き来しても作業中の `claude` はそのまま継続します。シェルを完全に終了したいときは、そのシェルで `exit` してください。

左ペインの各セッションは 2 行で表示され、Agent が動いているセッションをアイコンとテキストでひと目で見分けられます。1 行目はカーソル・種別アイコン・名前と右端の git status、2 行目は Agent の状態です。シェルや `claude` が**稼働中**の worktree は 2 行目に `▶ running`（緑）、その Agent が処理を終えて**入力待ち**になった worktree は `◆ waiting`（黄色）を表示します（入力待ちは稼働中より優先）。`◆`（入力待ち）になると、アタッチ中でないセッションについてはデスクトップ通知（`🐰 <ブランチ名> が入力待ちです`）でも知らせます。複数セッションを並行で走らせ、入力が必要になったものだけに対応できます（通知は設定 `notifications_enabled` が ON のとき。入力待ちの検知はベルに依存するため `claude` 側でターミナルベル通知を有効にしてください）。
