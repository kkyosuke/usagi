...
`usagi hop` を実行すると、うさぎのマスコットと `USAGI` タイトルがフェードインするスプラッシュから始まり、ワークスペースを開くとホーム画面へ遷移します。プロジェクト選択画面では、通常は `Enter` でカーソル行を単品で開き、`Space` でカーソル行を選んで **統合 / unite モード**へ切り替えると複数のワークスペースをチェックして同時に開けます（`u` は直近の統合選択を復元。複数のプロジェクトのセッションを 1 画面に積み重ねて操作できます）。
