...
埋め込みシェルにフォーカス中、`Ctrl-O` でセッション切り替えのオーバーレイ（ルート行 + 各 worktree の一覧）が開きます。番号 `1`–`9` または `↑`/`↓` + `Enter` で**別セッションのターミナルへ切り替え**、`Esc` で閉じてシェルに戻り、もう一度 `Ctrl-O` で**デタッチ**（シェルや `claude` は裏で動き続けたままサイドバーへ戻る）できます。各 worktree のシェルは画面を開いている間ずっと生き続けるので、セッションを行き来しても作業中の `claude` はそのまま継続します。シェルを完全に終了したいときは、そのシェルで `exit` してください。
