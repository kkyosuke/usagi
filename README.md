...
- **有効化**: グローバル設定 `settings.json` の `op_mcp.service_account_token` に **1Password サービスアカウントトークン**を登録します（`usagi config --edit`）。登録すると `agent` 起動時に `usagi-op` サーバ（`usagi op-mcp`）が自動で wire されます。空・未登録なら wire しません。
- **トークンの扱い**: トークンは `usagi op-mcp` プロセスが settings から読み取り、`op` へ環境変数 `OP_SERVICE_ACCOUNT_TOKEN` として渡します（エージェントの起動コマンド行やプロセス一覧には出ません）。`settings.json` には平文で保存される点に留意してください。
