...
| モード | 役割 | 主な操作 |
|---|---|---|
| **切替**（Switch） | 既定。セッションの選択・新規作成 | 左ペインで `↑↓` 選択・`←→`（または `Ctrl-N`/`Ctrl-P`。右ペインのタブチップの左クリックも同じ）でタブ切替・右クリックでタブメニュー（左右移動・名前変更・削除）・`Enter` 確定・`c` で新規作成・`r` で表示名変更・`n` でメモ編集・`Tab`（`1`〜`9` / `0`）で[ステータスラベル](document/05-settings.md#ステータスラベルsession_labels)を付与（todo / レビュー中 …をセッションに手で付け、サイドバーに色付き表示。選択中の行のメモ＝次回 TODO は右ペインに表示。ルート行はワークスペースルートのメモ） |
| **在席**（Focus） | 選択中セッションのコマンド | 右ペインで `terminal` / `agent` / `chat`（ローカル LLM と対話）を起動・`Ctrl-N`/`Ctrl-P` でタブ切替・`Ctrl-E` でメモ編集 |
...
| コマンドパレット（**統括** / Overview） | ワークスペース全体のコマンド（常駐モードではない） | `:` で開き、`session` / `unite` / `config` / `env` / `issue` などを実行。`Esc` で閉じて元のモードへ戻る |
...
- **直接対話（`chat`）**: 在席（Focus）の `chat` コマンド／メニュー行でローカル LLM と対話できます。`terminal` / `agent` と同じく**右ペインに表示**され（左のセッション一覧はそのまま）、外部 Agent CLI を起動せずローカルモデルに直接話しかけるので、ちょっとした質問にクラウド Agent のトークンを使いません（`local_llm.enabled` に関係なく `local_llm.model` のモデルを使用）。詳細は [document/03-commands/02-tui.md#chat](document/03-commands/02-tui.md#chat)。
