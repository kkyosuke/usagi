...
`usagi op-mcp` で [1Password CLI（`op`）](https://developer.1password.com/docs/cli/)を MCP サーバとして公開できます。エージェントは、タスクに必要な資格情報（API キー・トークンなど）を**その場で 1Password から読み取れる**ため、秘密情報をプロンプトに貼り付けずに済みます。公開する tool は**読み取り専用**です。
...
認証は `op`（環境）に委ねます（対話的な `op signin`、または `OP_SERVICE_ACCOUNT_TOKEN`）。`op_read` / `op_item_get` / `op_item_list` / `op_vault_list` / `op_whoami` の 5 tool を公開します。詳細は [document/03-commands/05-op-mcp.md](document/03-commands/05-op-mcp.md)。
