...
/// in *either* store (`session_prompt`'s `auto` mode can route to the live queue
/// for a session whose agent has ended but whose lingering phase file still reads
/// as live). Deliberately coarse — it does not read or validate the files (a
