...
pub(super) const ROOT_PROMPT: &str = "rootで統括担当です、issueや指示をもらってどんなタスクをsessionに実行させるか判別してください";
...
/// and those without one (Gemini, Antigravity) lead their opening prompt with it
/// via [`session_opening_prompt`].
pub(super) const SESSION_WORKTREE_PROMPT: &str = "あなたは usagi が管理するセッション専用の worktree 内で起動されています。このディレクトリは既に独立した作業環境のため、新たに git worktree を作成する必要はありません。ここで直接作業を進めてください。なお、この worktree は親のメインリポジトリの内側に置かれていますが、作業はこのディレクトリ配下だけで完結させ、親ディレクトリ（メインリポジトリ本体）のファイルは読み書きせず、そこへ cd もしないでください。受けた指示を実行して、何かしらの結果（設計やPRなど）みれる形で提供してください";

/// The suffix prompt added to Gemini/Antigravity launches to prevent immediate action
/// on this instruction alone.
pub(super) const GEMINI_AGY_ADDENDUM: &str = "この指示だけで実行はさせないで次の指示を待ってから取り掛かるようにしてほしい";
...
pub(super) fn base_agent_prompt(is_root: bool, is_gemini_agy: bool) -> String {
    ...
    if is_gemini_agy {
        format!("{base}{GEMINI_AGY_ADDENDUM}")
    } else {
        base
    }
}
...
pub(super) fn session_system_prompt(is_root: bool, is_gemini_agy: bool, local_llm_model: Option<&str>) -> String {
    let base = base_agent_prompt(is_root, is_gemini_agy);
    match local_llm_model {
        Some(_) => format!("{base}{LOCAL_LLM_PROMPT}"),
        None => base,
    }
}
...
pub(super) fn session_opening_prompt(is_root: bool, is_gemini_agy: bool, initial_prompt: Option<&str>) -> String {
    let base = base_agent_prompt(is_root, is_gemini_agy);
