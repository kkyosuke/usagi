...
pub fn render_frame(raw_height: usize, raw_width: usize, state: &HomeState) -> Vec<String> {
    ...
    if let Some(rabbit) = mascot {
        let mut rabbit: Vec<String> = rabbit
            .into_iter()
            .map(|row| format!("{}{row}", " ".repeat(RABBIT_INDENT)))
            .collect();
        // The workspace CPU / memory total rests beside the full mascot's face;
        // the rail chibi is too small and its strip too narrow, so it carries none.
        if sidebar == Sidebar::Full {
