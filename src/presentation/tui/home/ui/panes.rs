...
pub(super) fn worktree_row(
    ...
    // The manual-status label sits between the branch name and the memo marker in
    // its own fixed-width column (blank when this row has none, dropped entirely
    // when no visible session carries a label — `label_col == 0`), so the note and
    // git-status fields stay aligned down the list.
    ...
    let line1 = format!("{gutter} {kind} {branch}{status_tag}{note}{status}");
...
fn rail_entry(
    ...
    let detail = pad_to_width(
        format!("{bar} {} {}", label.unwrap_or(" "), agent.unwrap_or(" ")),
        RAIL_WIDTH,
    );
