...
/// the selected text to the clipboard (see [`copy_selection`]). `Ctrl-O` is the
/// leader: the *next* key chooses a [`PaneStep`] (detach / tab switch / new /
/// close), or cancels (`Esc` / an unbound key) and stays in the pane (see
/// [`leader_command`]). Other events are ignored so the next redraw picks up any
/// new size.
fn pump_input(
    term: &Term,
