---
number: 511
title: fix(core): v2 issue CRUD と共有採番を fail-closed にする
status: done
priority: high
...
updated_at: 2026-07-23T13:00:37+00:00
---
...
follow-up branch は #1226 を含む `origin/main` 697994a0 を基点とし、既存 issue #511 の未達 acceptance として完了する。新しい issue は作らず、merge 済み PR #1226 へ追加 push しない。
...
- [x] Git common、未作成でもcurrent nested、normalized workspaceと登録済み全Git worktreeそれぞれのdirect-session tree、および全worktreeのpathspecで見つかるmaterialize済みnested legacy/sourceを毎回列挙する。通常のdirect childはstore-local legacyを`next`未作成でも導出してlock/fenceし、registered-worktree childはGit-common authorityを使うためmaterialize済みlocal legacyだけをfoldしてmissing local authorityを作らない。未登録childの独立`.git`は親側列挙とchild側callerの両方から、sessions tree/session entry symlinkはauthoritative write前にfail-closedにする。child側callerはconventional workspaceとのGit common一致とworktree登録もauthority解決前に検証する。source floorも同じ全parentのdirect-session childと全worktreeのmaterialize済みnested sourceを走査する。非Gitはroot/current/存在する全direct sessionを未作成でも列挙し、global migration markerを公開・更新せず、後発sessionを既存markerで隠さない。既存markerはcanonical recovery floorとしてのみfoldする。known Missingも未封鎖として数え、複数ならblocker/sentinel/sequence/reservation/sourceのauthoritative write前（lock materialization除く）に停止する。
...
- [x] full required gates / coverage 100% / link check / 独立 review を完了する。
...
- external linked callerからmain/登録済み全worktreeのdirect sessionを列挙し、empty ordinary childのmissing legacyをsentinel化する一方、registered linked childにmissing local legacyを作らないこと。同じordinary child cwdのold-v2 subprocessはsentinel parse errorで停止し、authority bytesは不変であること。独立Git childは親側とnested child callerの両方向から、session symlinkもzero-effectで拒否すること。
