...
updated_at: 2026-07-04T05:09:58.100705+00:00
---

## 背景

子セッションのエージェントが完了（`ended`）してもデスクトップ通知と TUI バッジで**人間**に知らされるだけで、root 行で動くコーディネータの**エージェント**には届かない。#100 の `session_status` ポーリングで代替できるため優先度は低いが、push 型があればポーリング間隔に依存せず即座に次のタスクへ進める。
