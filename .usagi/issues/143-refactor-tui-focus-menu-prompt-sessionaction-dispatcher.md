...
`focus_prompt_key` は `state.focus_prompt_submit()` の `Effect` を再 match し、`run_focus_command` は command 名文字列を再 match している。`terminal` / `agent` / `chat` / `diff` / `close` の実行が menu と prompt で似た形に重複し、`ai <prompt>` だけ prompt-only という差分も handler 側の知識になっている。
