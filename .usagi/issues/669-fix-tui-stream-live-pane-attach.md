...
updated_at: 2026-08-12T05:31:36.734526+00:00
---
...
- [x] `Stale` / `OrderingMismatch` / `ResyncRequired` / `Orphaned` のいずれで失敗しても、backoff 満了後の
      poll で再 attach し、成功すれば live に戻って入力を受け付ける。
...
- [x] 指示モードで root Agent を起動して drawer を閉じたあと、managed session の Agent tab が live のまま
      入力できることを実 daemon・実 PTY の E2E で固定する。
