...
dependson: [518]
related: [209, 221, 275, 350, 492, 508, 514, 515]
parent: 505
created_at: 2026-07-21T21:20:49.574125+00:00
updated_at: 2026-07-22T11:45:25.434840+00:00
---
...
#514 daemon owner identity ─┐
                            ├─> #516 generation registry / standby / admission
...
- #508 は active/draining inventory merge と `TerminalRef.daemon_generation` による client routing を担当する。

#507 は #518 の完了まで `todo` に保ち、上記 primitive を再実装しない。

## 対象責務

依存 issue の完了後、planned restart を次の shipping lifecycle へ接続する。

1. manual `usagi daemon restart` と build/update replacement を同じ durable rollover operation へ接続する。
2. old active へ rollover を予約し、side-effect-free standby start/readiness 後に #516 の authority handoff を commit する。各 failure では old active を維持し、半端な current や二重 active を残さない。
...
- [ ] start/hydrate/bind/readiness/registry/locator commit の各 failure で old active が利用可能なままになり、二重 active・二重 spawn・state split-brain を起こさない。
...
- readiness / registry / locator failure と rollback
...
[daemon](../../document/05-daemon.md) と [IPC](../../document/04-ipc.md) は実装済みの現在形だけを記載する。#516/#518/#507 が未完了の間は、shipping restart が stop → fresh start で旧 PTY を継続しない事実と、安全な依存順を示す。legacy state は capability と exact identity を検証できる場合だけ移行し、unknown/corrupt registry や固定 child identity から owner を推測しない。
