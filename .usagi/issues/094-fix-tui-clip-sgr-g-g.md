...
## 項目
1. ✅ **`clip_to_width_cow` がスタイルを閉じない**（`widgets/mod.rs`）: スタイル付き文字列を途中で切ると、コピー済みの SGR を閉じずに `…` を付けて終わる。兄弟関数（`overlay_block` / `slice_from_width`）は `RESET` を付けるのに、`clip_to_width` 経由（`boxed` のパディング＋右罫線、`markdown_row`）は開きっぱなしの色が後続へ滲みうる（reverse / 背景色付きスパンで顕在化）。→ ESC を 1 つでもコピーしたら末尾に `RESET` を付与。**対応済み**: `clip_to_width_cow` が cut を跨いで SGR を運んだ場合に末尾へ `RESET` を付与。全角入力で `block_caret` の reverse キャレットがクリップ境界付近にあると `boxed` のパディング＋右罫線が反転表示になり枠が崩れて見える不具合を解消。
