...
updated_at: 2026-07-22T01:25:58.436152+00:00
---
...
|---|---|
| exact `TerminalRef` が trusted active / draining owner で live | 保存順で tab を復元し、保存選択を候補にする |
...
| dismissed lineage の durable record が retention policy で削除された、または利用者が明示 reopen した | 該当 dismissal を回収する。inventory 欠落、transport failure、partial snapshot、generation rollover だけでは回収しない |
...
| saved terminal の generation が current active と異なる | trusted draining owner があれば live のまま扱う。active / draining の双方に owner が無いと daemon が確定した場合だけ stale とする |
| corrupt / unknown schema | quarantine または無視して inventory-only へ安全に縮退し、起動を失敗させない |

復元は UI event loop と別の専用 daemon connection / port で行い、初回 frame、キー入力、animation を待たせない。一時的な transport failure は bounded backoff または daemon reconnect event で再試行し、同じ snapshot の replay は exact ref で 1 tab に収束させる。restore job は dispatch 時の UI interaction / registry revision を持ち、遅延結果が後続の close・reorder・selection を上書きしたり focus を奪ったりしない。
...
order / selection / close の確定 mutation ごとに state を atomic commit する。複数 TUI process が同じ workspace state を更新する場合は file lock と revision / compare-and-swap で read-modify-write を直列化し、CAS conflict は最新 state を再読込して stable key ごとに merge する。dismissal は authoritative GC 条件が成立するまで union し、遅い writer が新しい close intent を失わせない。inventory 失敗時は直前の valid state を空 snapshot で上書きしない。
...
- [ ] stale target、scope / generation mismatch、corrupt / old state、concurrent client の lock / revision conflict で誤 attach・lost update・二重 tab・起動失敗を起こさない。
- [ ] generic Terminal tab は従来どおり unified inventory から復元され、Agent intent / dismissal によって欠落・抑止・重複しない。

## 必須回帰テスト

- shipping TUI process、実 daemon socket / PTY、Claude / Codex fixture を使い、launch → unique output → normal quit → fresh TUI open → retained output → input echo → second reopen を検証する。2 回の reopen を通じて daemon PID / generation が不変であることも記録し、planned restart test と混同しない。
- mixed Claude / Codex、root / session、tab reorder / selection / close、新 runtime append を同じ product E2E に含め、各 provider の spawn count が 1 のままであることを固定する。
- slow / failed / out-of-order inventory、interaction revision fence、reconnect retry、duplicate / stale / active-draining ref、corrupt / future schema、atomic write interruption、concurrent CAS / merge を deterministic fixture で検証する。
