...
/// Serializes tests that read or write process-global state — here the
/// `$USAGI_HOME` override that redirects the data directory — which cargo's
/// parallel test runner would otherwise let race against each other.
///
