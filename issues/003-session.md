...
`session new` / `session list` と再帰的な worktree 構築・永続化を実装済み（`usecase/session.rs`、`domain/session.rs`、`infrastructure/git.rs` の `add_worktree` / `is_repository_root`）。

- `session new <name>`：ルートを再帰走査し、git リポジトリは新ブランチ `<name>` の worktree、それ以外はコピー。git でないルート・複数リポジトリ構成にも対応。
- `session list`：`state.json` の `sessions` を一覧表示。
- セッションを `state.json` に永続化し、`usagi status`（sync）でも保持。作成した worktree はワークスペース画面のサイドバーに反映される。

### 残課題（本 issue を `todo` のまま残す理由）

- `session remove <name>`（worktree・ブランチ・コピーの安全な削除、未コミット変更の警告）は未実装。
- サイドバー反映は新規作成・再読込時の追加のみ。`space` 連携や詳細表示は別 issue。
