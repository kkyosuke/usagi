...
[design/home/01-modes.md](design/home/01-modes.md#状態遷移) が正本です（選択へズームアウト＝集中・没入とも[キー方式](design/home/04-keys.md#ライブ端末closeup-内部状態)に従い既定 `Ctrl-O o`。集中は `Esc` でも戻れる、
`Esc`＝一段戻る、`:`＝[コマンドパレット（統括）](design/home/01-modes.md#2-つの-modal)、終了は `Ctrl+C`）。
...
- 画面・各モードの見た目は [design/home/README.md](design/home/README.md)、画面遷移は
...
- 没入中にナビゲーション用に予約するキーは**[キー方式（`key_scheme`）](05-settings.md#設定項目)**で決まります。既定の **`prefix` 方式**ではリーダー `Ctrl-O` の次キーで操作し（`Ctrl-O o` で**選択**へズームアウト、`Ctrl-O a` で**集中へ**）、`Ctrl-O` 以外のキーはシェルへ流れます。予約キーの全一覧と `alt` 方式は [design/home/04-keys.md](design/home/04-keys.md#ライブ端末closeup-内部状態) が正本です。
...
複数セッションを並行で走らせると、左ペインの各エントリ 2 行目で状態がひと目で分かります。ラベルの先頭には
**AI エージェントを表す Nerd Font のロボットグリフ**（下表では `🤖` で代替表記）が付き、その行が AI エージェントの状態であることを示します。

| 表示 | 意味 |
|---|---|
| `🤖 ▶ running`（緑） | Agent が稼働中 |
| `🤖 ◆ waiting`（黄） | Agent が入力待ち（あなたの応答を待っている） |
| `🤖 ✓ done`（シアン） | Agent が終了してアイドル |
...
[design/home/04-keys.md](design/home/04-keys.md#使用中-agent-の表示入力待ちの検知と通知) を参照。
...
- [design/home/README.md](design/home/README.md) — ホーム画面の 3 モードとキー操作。
