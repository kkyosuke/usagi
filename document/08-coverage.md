...
2026-07-21 時点の v2 には 673 件ある。#484 で固定した 892 件に後続変更で 1 件が加わり、#485 で core の
220 件を返済した。残る全件を `migration_debt` として扱い、コメントだけから正当性を推測して恒久許可にはしない。
registry の全 entry は 2027-01-31 に期限切れとなる。

| owner | 件数 | 返済先 |
|---|---:|---|
| TUI | 487 | #487 |
| daemon | 147 | #486 |
| root・CLI | 39 | 下表で許可候補と削除対象を review |
| **合計** | **673** | `coverage-off-allowlist.json` に全 symbol を列挙 |
