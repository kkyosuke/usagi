...
2026-07-21 の inventory 開始時点では v2 に 892 件あった。#485 で core の 220 件を返済し、#487 で TUI の
487 件の `migration_debt` をすべて削除した。#487 の返済後は 259 件で、TUI は
再審査を通った 77 件だけを source inline metadata で登録している。例外はすべて 2027-01-31 に期限切れとなる。

| owner | 件数 | 返済先 |
|---|---:|---|
| TUI | 77 | #487 で返済済み。下記の理由付き例外のみ |
| daemon | 144 | #486 |
| root・CLI | 38 | 下表で許可候補と削除対象を review |
| **合計** | **259** | registry または source inline metadata に全 symbol を列挙 |
...
3. #487 で TUI controller、Effect routing、presentation 分岐を戻し、real terminal IO を再審査した（完了）。
...
TUI の残存 77 件は `composition` 35 件（production wiring 4 件と unit harness module 31 件）、`real_io` 28 件、
`generic_monomorphization` 14 件である。各属性の inline metadata が理由・owner・期限・証拠 test の正本となる。
