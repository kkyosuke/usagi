...
Overview の workspace editor は、この継承関係を隠さない。編集中のスコープ自身の binding とは別に
**global の binding を read-only で併記**し、workspace 側が同名を持つものは上書き済みとして示す。
Workspace Config と Closeup は scope の誤編集を避けるため workspace binding だけを表示する
...
Config と Closeup は editor を開く直前に対象 scope の最新値を複数行 textarea へ読み、対象 scope の `env` だけを全置換保存する。
Workspace Config と Closeup は global binding を表示・変更しない。
