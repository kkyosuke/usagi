...
Overview の workspace editor も Config / Closeup と同じ textarea を使い、scope の誤編集を避けるため
workspace binding だけを表示する（[3. TUI#env editor](03-tui.md#env-editor)）。
...
Config、Overview、Closeup は editor を開く直前に対象 scope の最新値を複数行 textarea へ読み、対象 scope の `env` だけを全置換保存する。
Workspace Config、Overview の workspace editor、Closeup は global binding を表示・変更しない。
