...
Config は Theme・Modal mode・Agent model・Issue・Memory を scope（Global / Workspace）ごとに編集する。現在の scope は
`Scope: Global` または `Scope: Workspace` の単一値で表示し、`↑↓` で行を、
...
短い確認表示ののち、ユーザー操作なしで呼び出し元へ自動的に戻る。Welcome の Config は Global scope から開いて
Welcome へ戻る。開いている workspace の Overview で `config` を実行した場合は、その workspace を settings port に
束縛したうえで Workspace scope から Home 上の overlay modal として開き、live pane と session を背景に維持したまま
Home へ戻る。保存が失敗した場合は
...
| scope | 保存先 | 読み取り |
|---|---|---|
| Global | build channel ごとの user data directory にある `settings.json` | ファイルが無ければ core `Settings` の既定値。欠損 field と未知 enum token も field ごとの既定値へ縮退する |
| Workspace | 対象 repository の `.usagi/settings.json`（development mode は `.usagi/dev/settings.json`、local mode は `.usagi/local/settings.json`） | optional な local field だけを Global の上へ重ねる。ファイル・field が無い場合と未知 token は Global を継承する |

Issue と Memory はそれぞれ `issue_enabled` / `memory_enabled` として保存し、Global の既定値はどちらも `true` である。
Workspace scope は他の項目と同様に個別上書きを持ち、MCP server は起動時に解決した実効値を tool 公開・実行へ適用する。
無効時の tool 範囲と server lifetime の契約は [MCP サーバ](07-mcp.md#tool-面)を正本とする。

Workspace scope の保存は project store の cross-process lock を取得し、version envelope を付けた既存 local
...
`config` は引数を取らず、現在開いている workspace の Config を Workspace scope の overlay modal で開く。保存後に
Modal mode を再読込するため、次に開く Overview / Closeup から新しい選択方式が反映される。Issue / Memory の
