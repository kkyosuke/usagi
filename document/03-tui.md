...
terminal は起動時点と resize 後の右ペイン実幅・高さで geometry を要求するため、shell の right prompt も pane 内に収まる。geometry が変わると TUI は PTY を先に resize し、その viewport で retained replay を再 attach して、過去行を含む local screen を同じ幅へ組み直す。daemon 不通・stale・orphan は安全な
