...
Home sidebar は `main → session* → + new session` の順序と target identity を保つ。main と作成 action は
1 行、各 session は固定 2 行で描画する。`Sessions` 見出しは表示せず、main と session の間に divider を置く。session の 1 行目は cursor / active marker、表示名、常に幅を
...
request を非同期に開始し、完了まで行の直前に skeleton を表示する。`c` と IME に依存しない `Ctrl-A` も
同じ inline 入力を開く。Esc は入力を取り消し、空の名前は行の下に error を表示する。
