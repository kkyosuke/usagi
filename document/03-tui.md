...
大文字・小文字を区別しない alphabet 順に並べ、`/` で名前を絞り込む。各 workspace は名前と、
session 数・未完了 issue 数・最終更新の相対時刻を 2 行で表示する。Recent は同じ Workspace 画面を
直接開く。New と Config はそれぞれの backend port を通じて作成・保存し、失敗時は入力中の draft を
保持する。
