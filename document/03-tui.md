...
所有する。このとき背景の managed terminal は read-only attachment として通常の Home geometry のまま残り、drawer の
幅・高さや全幅 / 全高への縮退では resize しない。drawer は PR modal と同様に完成済みの Home frame へ合成するだけで、
覆われた managed terminal は同じ geometry のまま出力を取得し続ける。root shell だけを開いた場合はその上、Director も
開いた場合は overlay の左側に見えている範囲でも出力を取得し続ける。
...
Unicode display width による clip と hit-test を維持する。workspace identity は直上の project tab だけに表示する。
mode toggle、pending decision の notice badge、button は 1 つの
...
Console の terminal viewport は drawer の border、breadcrumb、separator、footer を除いて計算し、
...
shortcut と header button を受理せず、modal と drawer は同時に visible にならない。
...
| background worker 停止 | 長寿命 worker が異常終了し daemon が graceful shutdown を開始した | danger |
...
worker 停止は同じ daemon incarnation では回復しないため、endpoint retirement 前の snapshot に process-local failure count が
1 以上なら点灯する。graceful shutdown 後に起動した新しい incarnation では count は 0 へ戻る。
...
shell が attach するのは、現在の active target に属する selected foreground terminal、2 つの workspace drawer を同時に
開いた場合の入力を持たない root surface、workspace drawer の背後に置く selected managed terminal だけである。通常は 1 件、
drawer が 1 つなら root foreground と read-only な managed background の 2 件、両方なら最大 3 件になる。drawer が背景を
全幅 / 全高で覆っても managed background は通常の Home geometry で維持し、drawer の開閉だけでは resize / detach しない。
drawer 以外の遷移で surface / target / tab が表示対象から外れた時点でその subscription を detach する。
...
attach 1 往復 / 2s、しかも attach 済みの表示対象（通常 1 件、workspace drawer 中でも最大 3 件）に限られるため、
...
|---|---|---|---|
| foreground poll pump | attach 済みの表示対象 terminal（通常 1 件、workspace drawer 中は root surface と managed background の最大 3 件）の出力 | `Resume { after_offset }` | 出力がある間は interactive（8ms）。無出力が続くと 64ms 上限まで倍々に後退し、出力・attach・入力・resize で即座に interactive へ戻る |
...
live terminal に focus がある間、leader が無い `Ctrl-C` / `Ctrl-Q` / `Ctrl-D` / `Ctrl-X` / `Ctrl-?`（`Ctrl-/`）以外の
非 prefix キー入力（修飾キー・raw bytes・Tab・矢印など）は management ではなく PTY へ送られる。Director Console も同じ
経路を使い、通常文字・paste・編集キー・`Enter` を横取りする command composer や入力 bar は置かない。
...
この run は起動時の指示に次の 2 つを含め、`PR ready` の判定でも daemon が独立に検証する。

| 規約 | 検証 |
|---|---|
| PR 本文に `Internal-Issue: #<番号>` の行がある | GitHub が返した PR 本文を行単位で照合する |
| 同じ PR の差分で issue が `done` になっている | session worktree の issue を読んで status を確認する |

どちらかが欠けていれば `PR ready` にはならず、不足を待ち理由として表示する。issue の書き込みは
従来どおり session worktree の中だけで行う。
...
- **タイミング**: 初回 frame を paint した後、UI event loop と別の daemon connection で
  ...
  Agent launch / resume が成功した場合、または foreground poll / background inventory が Agent terminal の終了を検知した
  場合は、この dedicated restore lane を即座に wake し、session lifecycle の次回変更を待たずに sidebar・Garden・tab strip
  の membership を再観測する。
...
- **投影**: saved Agent は完全な `TerminalRef` が両 inventory で trusted live と確認できたときだけ保存順で復元する。
  ...
  tab に加え、workspace drawer の背景にある selected managed tab だけを通常の Home geometry で attach / resync し、
  drawer に覆われても維持する。その他の background target と選択外 tab は detached のまま保持する。
