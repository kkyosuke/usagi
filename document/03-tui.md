...
request を非同期に開始し、完了まで行の直前に skeleton を表示する。skeleton の activity glyph と session 名は同じ
左から右へ流れる wave で描き、静的な点滅にはしない。daemon が同一 `OperationId` と revision を持つ `session.created`
