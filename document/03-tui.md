...
Home sidebar は `root → session* → + new session` の順序と target identity を保つ。root と作成 action は
1 行、各 session は固定 2 行で描画する。session の 1 行目は cursor / active marker、表示名、常に幅を
予約する note icon を表示する。note icon は既存の text overlay を開く入力を増やさず、内容の有無だけを示す。

2 行目は daemon snapshot の `last_active`、または旧 record の `created_at` を UTC 時刻として表示し、dismissed
でない PR があれば先頭の PR 番号と残り件数を続ける。PR title の解決はこの行の前提にしない。snapshot に無い
session は selected / active を root に縮退させ、空一覧でも root と作成 action は残る。
...
daemon の snapshot revision を workspace 間で持ち越さない。remove の target は表示名や入力値で再解決せず、
