...
| Workspace | 対象 repository の `.usagi/settings.json`（development mode は `.usagi/dev/settings.json`） | optional な local field だけを Global の上へ重ねる。ファイル・field が無い場合と未知 token は Global を継承する |
