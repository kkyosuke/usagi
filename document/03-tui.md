...
覆らないため、開始 intent を session に残さず中止として畳む。session は次の開始を受け付ける状態に戻り、
担当と依頼を選び直せる。
...
元の依頼と再試行操作を復元する。送り直しても覆らない拒否だけを中止として畳む。起動に成功したあとの失敗は
常に再試行可能として扱うため、Agent が起動済みの run を畳むことはない。
