...
どの mode でも Success（緑）を保ち、cursor が乗る Switch の選択時だけ太字にする。この Success 色は
...
同じ inline 入力を開く。`Ctrl-A` は選択カーソルも `+ new session` 行へ移動する。Esc は入力を取り消す。作成は名前だけを受け取り、profile / model は指定せず daemon の workspace default policy に委ねる（中央 modal ではなく行内の name-only 入力である）。空の名前のまま Enter を押すと行内に error を表示して作成しない。
