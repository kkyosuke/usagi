...
出力は mouse drag により選択できる。macOS の `Cmd-C` は選択した ANSI を含まない表示テキストを OS clipboard にコピーする。
`Ctrl-C` はコピーに使わず、live terminal へそのまま送る。clipboard adapter は macOS の `pbcopy`、Windows の `clip.exe`、
