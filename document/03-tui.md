...
同じ inline 入力を開く。`Ctrl-A` は選択カーソルも `+ new session` 行へ移動する。Esc は入力を取り消す。入力中は英数字・`-`・`_` 以外、64 文字超過、または表示中 session と重複する名前を行の下に error として表示し、空の名前は Enter 時に error を表示する。
