...
Resume Agent work / Cancel run / Mark failed だけを提示し、停止理由と safe evidence を同じ画面に表示する。
`Resume Agent work` は保存済み provenance の exact Agent run へ修正と再報告を指示できた場合だけ escalation を解除し、
Agent が既に停止している場合は結果未確認として run を停止状態のまま保つ。`↑` / `↓` / `←` / `→` で選択して `Enter` で確定する。
