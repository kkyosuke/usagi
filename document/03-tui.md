...
安全なメッセージを notice に出して同画面に留まり、入力は保持する。作成が成功すると、その workspace を
Open / Recent と同じ経路で開いて Home へ遷移する。作成が失敗すると、入力中の draft を保ったまま notice を
...
その session の Closeup を開く（keyboard の Enter と同じ意味）。ダブルクリック判定は clicked cell の座標ではなく
**解決した行の stable identity**（selected 判定と同じ hit-test）で行うため、実際のダブルクリックが 1 セルずれても、
2 行にまたがる session 行のどちらの行を押しても活性化する。逆に 2 回目の click までに scroll や snapshot 更新
（session の削除・同名再作成による新しい identity）で cursor 下の行が入れ替わった場合は、座標が同じでもダブル
クリックにはならず単なる選択に留まる。root・`+ new session`・divider・mascot・footer はこの pointer 活性化の
対象外であり（cursor は移るが click では Closeup も作成フォームも開かない。作成フォームは Enter / `Ctrl-A`）、
