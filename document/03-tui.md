...
Config は Theme・Modal mode・Agent model・Issue・Memory を scope（Global / Workspace）ごとに編集し、`↑↓` で行を、
...
束縛したうえで Workspace scope から開き、live pane と session を維持したまま Home へ戻る。保存が失敗した場合は
...
`config` は引数を取らず、現在開いている workspace の Config を Workspace scope から開く。保存後に
