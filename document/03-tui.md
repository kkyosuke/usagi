...
右ペインは session 名の右に tab を Chrome 風の chip として描き、その直下に active marker を置く。表示中の
live terminal では、その直下に daemon が管理する worktree の絶対 path を `[]` で表示する。chip の表示順・label は表示専用であり、選択は pending / document の `OperationId` または terminal live の完全な `TerminalRef` から投影する。
