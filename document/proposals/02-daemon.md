...
- 端末は daemon が spawn 時に採番する **terminal id** で指す（1 worktree に agent＋terminal 複数タブが同居するため、worktree パスだけでは端末を特定できない）。実装済みのメッセージ:
  ...
  | 方向 | メッセージ | 内容 |
  |---|---|---|
  | TUI⇄daemon | `Hello { build }` | terminal 操作前に executable generation を相互確認する。不一致（`cargo run` の再ビルド前 daemon を含む）なら手動の新規ペインはローカル PTY へフォールバックする。保存済み terminal の復旧は fallback しない。queued prompt の自動起動は、永続 registry が対象 worktree の terminal 非在を証明するときだけ fallback する |
...
#205 では terminal の `Spawn` / `Attach` より先に `Hello { build }` を往復する executable-generation handshake を追加した。build は package version に加えて実行ファイルの device / inode / size / mtime を含むため、version が同じ `cargo run` の再ビルドも区別する。不一致や `Hello` を持たない旧 daemon では terminal 操作を送らず、手動の新規ペインは既存 daemon とその Agent を止めずにローカル PTY へフォールバックする。保存済み terminal id の復旧は local fallback しない。queued prompt の自動起動は `terminals.json` が存在し、対象 worktree の terminal がない場合だけ local fallback する。registry 不在・読み取り失敗・対象 terminal ありの場合は、旧 Agent が生存したまま別 Agent を起動するのを防ぐため失敗または再試行として記録する。
