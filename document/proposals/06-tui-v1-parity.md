...
| Config | global / workspace scope、dirty 表示、明示 Save、各種 chooser / editor / 非同期 probe | global / workspace scope ごとの Theme draft、dirty 表示、Tab / `←→` / `S` による read-edit-save。保存失敗時は draft と error を保つ | Local LLM 導入などの chooser / 非同期 probe を後回しにする | B | `B-CONFIG-1`: scope ごとの変更だけを保存し、失敗時は編集値と error を保つ | settings backend |
...
| overlay: diff / note / PR / text / env | diff、note / todos / decisions、PR、長文、env editor などを Home に合成する | note / todos / decisions と workspace / session env は controller の local editor state と load/save effect、背景を残す modal renderer を持つ。PR は選択 session の実 model で開閉・移動できる。diff / text view は未実装 | 残る view も既存 overlay 合成 / renderer を共用する | B | `B-OVERLAY-1`: 背景 state を失わず開閉し、長文は安全に scroll する。`B-OVERLAY-2`: edit/save failure でも入力と背景を保つ | 機能ごとの backend |
...
| B | rich diff、PR / text modal、bulk remove checklist |
