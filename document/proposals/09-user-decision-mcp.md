...
`user_decision_get` は recovery と解決回答の durable な取得に使う。
...
event recipient は保存済み owner から導く。consumer は event と decision の owner・回答を照合して outbox を
配送済みにし、元 caller は credential 付き `user_decision_get` で同じ durable record を取得する。
cancel/expire は delivery event を作らない。

```text
agent request -> daemon: Pending + waiting -> immediate MCP response
TUI resolve  -> daemon: Resolved + durable delivery event
caller get   -> daemon: resolved answer
```
