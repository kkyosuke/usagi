...
MCP は依頼の受付と実行を分離し、「実行」側を daemon に集約する。TUI は attach クライアントになる。
このとき検討対象となったのは次の 2 点である。

- `usagi <サブコマンド>`（CLI）と `usagi mcp` を**どのクレートに置くか**。
- MCP tool（特に `session_create` / `session_prompt` など実行を伴うもの）の結果を
  **daemon と TUI にどう反映するか**。

`state.json` を複数プロセスが読み書きしてTUIが監視する構成は採らない。daemon が常駐する前提を活かし、
session 状態の書き手を daemon に一本化して、プロセス間の競合処理を daemon 内の直列化に置き換える。
...
JSON 出力の SSoT は usecase 側に置いてCLI・MCPで共有し、共通要素はすべて `usagi-core` に置く。
...
- **live 配送**: daemon が対象端末の PTY に直接書く。live か queue かの `auto` 判定はdaemon自身の
  端末・attachテーブルで行う（daemonが権威なので推測が要らない）。
...
へ畳み込み、本提案はリンクスタブ化する。実装タスクは issue ストアで追跡する。
