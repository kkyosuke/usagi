...
本書は **v2 MVP の後にだけ評価する未実装設計**である。PTY master fd を daemon crash 後にも継続利用するための broker と planned restart 時の Unix FD handoff を比較し、採否と着手条件を定める。本書がこの将来機構の設計判断の正本であり、MVP の crash 契約は [daemon lifecycle](05-daemon-lifecycle.md#daemon-crash-と-orphan) を正本とする。
