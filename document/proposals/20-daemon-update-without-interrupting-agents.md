...
| IPC | inventory の closed vocabulary に追加する。未知 token を受けた client は runtime state を `unavailable`、phase を「報告なし」へ倒し、frame ごと捨てない |
...
| 順序 | issue | 成果物 |
|---|---|---|
| 1 | `feat(core): planned update の runtime state と phase 語彙` | `updating` variant、未知 token を `unavailable` / 報告なしへ倒す tolerant deserialize、pure test |
