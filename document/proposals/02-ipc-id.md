...
`crates/daemon/src/presentation/ipc.rs` は一接続を逐次処理する handler までを実装している。Unix socket bind/connect、
client、ID、handshake negotiation、server push、terminal/session API は未実装である。
