...
| 18 | [18-goal-driven-work-run.md](18-goal-driven-work-run.md) | 目的を一度入力し、PR Ready または明示的な人間判断まで進める Work Run の画面フロー・状態・既存画面との境界 | goal-bound Director、SupervisorRun の progress / 複数 Run 操作面、独立 artifact verifier は [03-tui.md](../03-tui.md) / [04-ipc.md](../04-ipc.md) / [05-daemon.md](../05-daemon.md) へ畳み込み済み。独立 Run Closeup と run-scoped defaults は提案 |
| 19 | [19-director-screen-graph.md](19-director-screen-graph.md) | Organization、Work Runs、Run Overview、Director Console、Start Work Run の責務と stable identity を保つ画面遷移 | [03-tui.md](../03-tui.md) / [11-keybindings.md](../11-keybindings.md) へ畳み込み済み（採用理由の履歴） |
| 20 | [20-daemon-update-without-interrupting-agents.md](20-daemon-update-without-interrupting-agents.md) | daemon artifact の更新で Agent 会話を失わず、planned update を事故の `interrupted` と別語彙で見せる設計（`updating` 投影・hook の静止点待ち・update 導線の一本化・旧 build の印） | 停止 → 引き継ぎ → exact resume は [04-ipc.md](../04-ipc.md) へ畳み込み済み。語彙・導線・待ち方は提案 |
