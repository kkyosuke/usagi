...
没入中の**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`**で、`Esc` を含む他キーはすべてシェルへ流れます。
`Ctrl-O` の**単押し**で[切替](../design/05-home.md#切替switch)へズームアウトし、`Ctrl-N`/`Ctrl-P` で**没入のままタブ（ペイン）を
前後へ切り替え**、`Ctrl-T`/`Ctrl-G` で**没入のまま terminal / agent タブを追加**、`Ctrl-W` で**アクティブタブを閉じ**ます
