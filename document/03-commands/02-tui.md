...
没入中の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-W`**で、`Esc` を含む他キーはすべてシェルへ流れます。
...
前後へ切り替え**、`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-W` で**アクティブタブを閉じ**ます
