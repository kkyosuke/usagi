...
| `diff` | 集中中セッションの差分（既定ブランチとの差分）を**セッションの `diff` タブ**として GitHub のプルリクエスト風に表示（左＝ファイルツリーのエクスプローラ／右＝選択ファイルの差分。行番号・シンタックスハイライト・単語差分・unified/split。`Ctrl-P`/`Ctrl-N` でタブを離脱。Session スコープ） |
...
- **左＝ファイルエクスプローラ**：変更ファイルをディレクトリツリー（GitHub の file tree と同じ順＝ディレクトリ先・各階層アルファベット順）で一覧し、各ファイルに `+A -B` の増減数を添えます。ディレクトリは折り畳めます。
- **右＝選択中ファイルの差分**：カーソル（または `Enter`）で選んだ 1 ファイルの差分だけを描きます。
  ...
  - **unified / split（左右 2 列）レイアウトの切り替え**（`s`）。
...
- キー操作は**フォーカス（左のエクスプローラ／右の差分）で切り替わります**。`Tab` でフォーカスを左右に移します。
