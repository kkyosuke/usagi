...
没入中にナビゲーション用に予約するキーは**[キー方式（`key_scheme`）](../05-settings.md#設定項目)**で決まります。既定の **`prefix` 方式**はリーダー `Ctrl-O` だけを奪い、操作は**その次のキー**で指定します — `Ctrl-O o`（[切替](../design/05-home.md#切替switch既定)へ）・`Ctrl-O a`（在席へズームアウトしてアクションメニュー）・`Ctrl-O n`/`p`（タブ前後。`Ctrl-O →`/`←` も）・`Ctrl-O g`（agent タブへ移動＝無ければ追加・あればその 1 つへ）・`Ctrl-O e`（[セッションメモ](../design/05-home.md#セッションメモの編集)編集）・`Ctrl-O s`（サイドバー開閉）・`Ctrl-O q`（終了確認）。`Ctrl-O Ctrl-O` でも切替へズームアウトできます（IME を ON にしたままでも届く）。`Ctrl-O` 以外の bare Ctrl キー（`Ctrl-E`/`Ctrl-N`/`Ctrl-P`/`Ctrl-T`/`Ctrl-G` など）はシェルへ流れます（`Ctrl-O` 自体をシェルへ送りたいときは `alt` 方式）。
**`alt` 方式**では各操作が `Alt` 単打（`Alt-o`/`a`/`g`/`e`/`s`/`q`・`Alt-→`/`←`）になり bare Ctrl キーを奪いません（macOS は Option=Meta 設定が前提）。どちらの方式でも `Ctrl-^` で**[直前のセッションへ切り替え](../design/05-home.md#直前のセッションへ切り替えctrl-)**（ライブなら再アタッチ）でき、`Esc` や `Ctrl-W`（シェルの「直前の単語を削除」）はシェルへ流れます。`Ctrl-W` を奪わないため、タブを閉じるのは切替（切替へ抜けて `x`）で行います。terminal タブの追加は在席のアクションメニュー（没入では「在席へ」で開く）か切替（`t`）でも行えます。切替では
