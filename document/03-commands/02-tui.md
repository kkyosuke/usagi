...
没入中は **`Ctrl-O` だけが予約キー**で、`Esc` を含む他キーはすべてシェルへ流れます。`Ctrl-O` の**単押し**で
[切替](../design/05-home.md#切替switch)へズームアウトします。**同一セッションのタブ切替（`←`/`→`）と新規ペインの追加（`t`）は
切替で行い**、没入の中ではタブ操作キーを持ちません。シェルは画面を開いている間プールに常駐し、行き来しても終了しません。
