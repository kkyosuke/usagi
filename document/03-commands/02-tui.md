...
レンダリングは Markdown のサブセット（見出し・箇条書き／番号付きリスト・引用・`**強調**`／`*斜体*`／`` `コード` ``／リンク）を色付けして表示します。**フェンスドコードブロック（` ``` ` / `~~~`）は、開きフェンスの言語トークン（例 ` ```rust `）に応じてシンタックスハイライト**します（[syntect](https://github.com/trishume/syntect) によるトークン化を端末の 256 色へマッピング。言語トークンが無い／未知のときはプレーン表示にフォールバック）。
...
没入中の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-^`・`Ctrl-E`**で、`Esc` や `Ctrl-W`（シェルの「直前の単語を削除」）を含む他キーはすべてシェルへ流れます。
`Ctrl-O` の**単押し**で[切替](../design/05-home.md#切替switch既定)へズームアウトし、`Ctrl-T` で**在席へズームアウトしてアクションメニュー**（terminal / agent などを選ぶ画面）を開き、`Ctrl-N`/`Ctrl-P` で**没入のままタブ（ペイン）を
前後へ切り替え**、`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-^` で**[直前のセッションへ切り替え](../design/05-home.md#直前のセッションへ切り替えctrl-)**（ライブなら再アタッチ）、`Ctrl-E` で**[セッションメモ](../design/05-home.md#セッションメモの編集)を編集**（閉じると同じペインへ復帰）します
（その分シェル / Agent 側の同キーは奪われます）。`Ctrl-W` は**奪わず**シェルの「直前の単語を削除」として流すため、タブを閉じるのは切替（`Ctrl-O` で抜けて `x`）で行います。terminal タブの追加は在席のアクションメニュー（`Ctrl-T` で開く）か切替（`t`）でも行えます。切替では
