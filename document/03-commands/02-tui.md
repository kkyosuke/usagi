...
没入中の**予約キーは `Ctrl-O` と `Ctrl-N`/`Ctrl-P`**で、`Esc` を含む他キーはすべてシェルへ流れます。`Ctrl-O` の**単押し**で
[切替](../design/05-home.md#切替switch)へズームアウトし、`Ctrl-N`/`Ctrl-P` で**没入のままタブ（ペイン）を前後へ切り替え**ます
（その分シェル / Agent 側の `Ctrl-N`/`Ctrl-P` は奪われます）。**新規ペインの追加（`t`）は切替で行います**。切替では
