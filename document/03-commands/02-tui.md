...
没入中の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-^`・`Ctrl-E`・`Ctrl-W`**で、`Esc` を含む他キーはすべてシェルへ流れます。
...
前後へ切り替え**、`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-^` で**[直前のセッションへ切り替え](../design/05-home.md#直前のセッションへ切り替えctrl-)**（ライブなら再アタッチ）、`Ctrl-E` で**[セッションメモ](../design/05-home.md#セッションメモの編集)を編集**（閉じると同じペインへ復帰）、`Ctrl-W` で**アクティブタブを閉じ**ます
