...
| `usagi memory <save\|list\|show\|update\|search\|delete>` | カレントリポジトリのエージェントのメモリ（`.usagi/memory/`）を操作する（[data/04-memory.md](../data/04-memory.md)） |
| `usagi mcp` | issue とメモリの操作を MCP（Model Context Protocol）サーバとして stdio で公開し、AI エージェントから使えるようにする |
