...
- [`session_send` の挙動](#session_send-の挙動)
...
presentation/mcp/usagi.rs  … UsagiMcpServer：issue/memory サーバと session サーバを合成し tool をマージ
        ...
        └─ presentation/mcp/session.rs … session tool 実装（prompt/send/remove は AgentBackend 経由）
        │  各 tool が呼ぶ
        ▼
...
（テスト時）FakeBackend / （本番）CliAgentBackend
  ├─ session_prompt → agent-prompts/ へキュー（TUI が起動時に消費）
  └─ session_send   → agent-live-prompts/ へキュー（起動中 TUI が live pane へ注入）
```

| モジュール | 役割 |
|---|---|
| `presentation/cli/mcp.rs` | `usagi mcp` のエントリ。カレントディレクトリ（issue / memory 用）とそこから解決した workspace root（`usecase/session::workspace_root`。session 用）を `UsagiMcpServer` に渡して構築し、stdin を 1 行ずつ読み `handle_line` の戻り値を stdout へ書く。本番 `AgentBackend`（`session_prompt` のプロンプトを `agent-prompts/` へキューし、`session_send` のプロンプトを `agent-live-prompts/` へキューし、`session_remove` で実効設定の agent を解決して `usecase/session::remove` を呼ぶ）もここに置く。ブロッキング I/O のみで、`hop` 同様カバレッジ計測の対象外。 |
...
| `presentation/mcp/session.rs` | session tool を提供する `SessionMcpServer`。実エージェント・実ファイルに触れる操作（`session_prompt` / `session_send` / `session_remove`）を `AgentBackend` トレイトで抽象化し、ユニットテストで網羅。 |
...
`tools/list` で以下の 21 tool（issue 7 + memory 6 + session 8）を公開します。結果はいずれも JSON テキストで
返ります。
...
| `session_send` | `name` / `prompt` | — | プロンプトを対象セッションの live agent へ送るためにキューした旨の確認メッセージ（[挙動](#session_send-の挙動)） |
...
- `session_send` は、対象セッションでエージェントペインがすでに開いているとき、その既存ペインへ
  プロンプトを貼り付けて Enter するための live queue に追加します。セッションやエージェントを起動せず、
  エージェントの応答も返しません（[挙動](#session_send-の挙動)）。
...
- `prompt` にはサイズ上限（128 KiB）があります。超えるとキューに書き込む前にツールエラーとして拒否します
  （`session_send` と同じ上限）。

## `session_send` の挙動

`session_send` は、対象セッションの**すでに起動しているエージェントペイン**へプロンプトを送るための
live queue です。`usagi mcp` は TUI と別プロセスなので live pane の PTY を直接握れません。そのため
プロンプトを worktree 別の一時ファイル（[`agent-live-prompts/`](../data/01-global.md#agent-live-prompts)）へ
追記し、動作中のホーム画面の監視スレッドがこれを取り出して、対象 worktree の agent タブへ
「貼り付け → Enter」として書き込みます。

- `session_send` はセッションやエージェントを起動せず、エージェントの応答も返しません。返るのは live queue
  へ追加した確認メッセージだけです。
- 対象セッションに live agent ペインがある場合、TUI の監視 tick（約 200 ms 間隔）で配送されます。agent
  ペインが無い場合、プロンプトは `agent-live-prompts/` に残り、後から agent ペインが開いたときに配送されます。
- 複数回 `session_send` したプロンプトは追記順に配送されます。各プロンプトは 1 回だけ取り出されます。
- 配送は「取り出し → 書き込み」の順で行い、PTY への書き込みが失敗したプロンプト（および同じ tick で
  それ以降にあった未配送分）は live queue の**先頭へ戻して**次の tick で再試行します。これにより、キュー
  済みと返答したのに書き込み失敗で黙って失われることはありません（送信順は保たれます）。
- 書き込みは端末の paste と同じ扱いです。対象プログラムが bracketed paste mode を有効にしているときは、
  プロンプト全体を bracketed paste で包んでから Enter を送るため、複数行プロンプトが途中で複数回 submit
  されることを避けます。
- `prompt` にはサイズ上限（128 KiB）があります。超えるとバックエンドに渡す前にツールエラーとして拒否し、
  live queue には何も書き込みません。`session_prompt` も同じ上限です。
- `session_prompt` との使い分けは、**まだ agent を起動していないセッションに最初のタスクを渡す**なら
  `session_prompt`、**すでに開いている agent に追加指示を送る**なら `session_send` です。
...
- **1 サーバに合成**: issue/memory（リポジトリの純粋な読み書き）と session（`session_prompt` / `session_send`
  でプロンプトをキューする `AgentBackend` を要する）は依存関係が異なるため、それぞれ独立にユニットテストされた別サーバの
