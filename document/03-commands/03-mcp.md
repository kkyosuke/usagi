...
- `session_list` / `session_status` の各セッション要素は `started_from`（親セッション名、無ければ `null`）を持ち、その
  ...
  `session_delegate_issue` を呼ぶと、その親セッション名が記録されます（ルートから作成した場合は `null`）。これにより
...
| フィールド | 内容 |
|---|---|
| `origin` | セッションを**誰が起動したか**。`human`＝人が TUI で作成、`mcp`＝エージェントが `session_create` / `session_delegate_issue` で作成、`unknown`＝この項目を持たない古い `state.json` のセッション（未記録）。作成時に一度だけ記録し、同期では書き換えない。 |
...
上書きを、プロンプト配送前に保存します。`mode="queue"`（または `auto` が queue を選んだ場合）は、そのキュー済み
プロンプトを次に開く agent にそのまま適用します。`mode="live"`（または `auto` が live を選んだ場合）は、すでに起動中の
ペインを再起動しないため、今回の live 入力は現在の agent に届き、保存した CLI / モデルはそのペインを終了して
次にフレッシュ起動したときから適用します。`agent_cli` は `session_create` / `session_delegate_issue` と同じ
`claude` / `codex` / `codex-fugu` / `gemini` / `agy` を受け取り、インストール済みかつ MCP 対応の CLI だけを許可します。
...
1. `agent_cli` / `model` 引数が指定された場合、それが有効な値であり、インストール済みかつ MCP 対応のエージェントであるかを検証する（エラーがあれば早期リターン）。
