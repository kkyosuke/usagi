...
- **`auto`（既定）**: セッションにライブなエージェントペインが検知できれば **live**、なければ **queue** を選びます。
  ...
  なおこの判定は控えめ（安全側）で、エージェントが終了して phase が `ended` になってもファイルは `✓ done`
  バッジ表示のため残り、TUI が異常終了するとクリアされないため、**実際にはペインが無いのに live と判定して
  ライブキューへ振り分ける**ことがあります。この取りこぼしは、TUI 側の権威的なペイン生存判定に基づく
  自動起動（上記ライブキュー行・[4. オーケストレーション](../04-orchestration.md#キュー済みプロンプトの自動起動)）が
  spawn して救済します。
...
- **ライブキュー**のプロンプトは、対象セッションに live agent ペインがある場合 TUI の監視 tick（約 200 ms 間隔）で
  ...
  **先頭へ戻して**次の tick で再試行するため、キュー済みと返答したのに黙って失われることはありません。書き込みは
  ...
  ライブペインが**無い**セッション（`auto` が終了済みペインを live と誤検知した場合など）にライブキューが
  残っているときは、設定 `autostart_queued_prompts`（既定 ON）が有効なら自動起動が起動時キューと同様にこれを拾い、
