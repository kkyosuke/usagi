...
- エージェントの実行には**タイムアウト**があります。MCP サーバは stdio 上の単一スレッドで動くため、
  応答が返らないエージェントを無制限に待つと後続のリクエストごとサーバが固まり、クライアントには
  原因不明の「内部エラー」として現れます。これを避けるため、既定で 600 秒を超えた子プロセスは強制終了し、
  その旨を明示したエラーを返します。上限は環境変数 [`USAGI_AGENT_TIMEOUT_SECS`](../05-settings.md#環境変数)
  で変更でき、`0` で無制限になります。
