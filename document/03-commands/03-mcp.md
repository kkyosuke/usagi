...
| `session_list` | — | — | セッション配列（各要素に `name` / `display_name` / `origin` / `root` / `created_at` / `worktrees`） |
| `session_status` | — | — | セッション配列（各要素に `name` / `display_name` / `origin` / `root` / `agent_phase` / `worktrees`。各 worktree に `status` / `dirty` / `merged`）（[挙動](#session_status-の挙動)） |
