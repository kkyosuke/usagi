...
request はその進行を必要としない（[workflow lane](05-daemon.md#workflow-lane)が正本）。snapshot /
control request も同じ 1 回の pass を通るため、開いている画面は常に最新の進捗を受け取る。
...
subscription / input ledger の削除と MCP child の active connection lease の解放は daemon-owned cleanup worker へ渡して
直列化する。
...
connection lease を一括して除く（exact process claim は保持する）。この lock 順序により、snapshot 後に admit された connection は
sweep が owner lock を解放するまで state を
