...
Agent request / PTY observation / completion
       ...
       GenerationCoordinator (process-local authority)
        ...
        | exact CompletionFence outcome
                  |
                  v
 shards/<own>.json = generation ownership + runtime records (one CAS)
```
...
daemon restart 後も `inventory` は retained owner shard から復元した generic terminal record を同じ scope と
