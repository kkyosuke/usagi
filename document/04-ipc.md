...
や Claude / Codex の混在を別 item として保持する。`complete: true` は retained durable history 全体の snapshot で
あることを示し、この場合だけ client は欠落した `AgentContinuationRef` を retention 削除と判断できる。旧 producer や
partial projection は `complete: false` として decode し、欠落を dismissal / slot GC の根拠にしない。
...
error として返す。process crash が atomic rename 前に残した fixed-name temporary は、次の publisher が
`current.lock` の排他区間でリンク先を辿らず回収してから作り直す。

current locator の crash-safe condition は、副作用なく `absent` / `live` / `stale` に分類できる。locator の欠損は
`absent`、検証済み generation endpoint へ probe できれば `live`、endpoint の欠損または probe failure は `stale`
である。malformed / unsafe locator、generation directory 外または unsafe permission の endpoint は `stale` へ
弱めず error とする。`stale` は回収許可ではなく、locator と socket は変更しない。generation migration /
replacement は #514 の exact process identity とこの condition を組み合わせる
[#516](../.usagi/issues/516-refactor-daemon-cross-process-generation-registry-standby-handoff-authority.md) が判断する。
