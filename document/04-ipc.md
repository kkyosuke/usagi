...
endpoint、PTY に stop signal を送らない。通常 bootstrap から same-artifact replacement は発行せず、明示
...
その consumer が接続されるまでは trigger を typed outcome として返すだけで、blind cold replacement や二重 daemon
spawn を行わない。通常 envelope は handshake の成功後だけ受理する。
