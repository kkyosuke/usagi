...
Codex を daemon が起動するときは、注入した `usagi` stdio server だけにこの credential を
`env_vars` で forward し、server の tool approval mode を `approve` にする。これは各 MCP call の
対話確認を省略する設定である。認可を省略するものではなく、daemon は credential、live runtime、dispatch
