...
inbox の完了報告は維持して retryable error を返す。同じ report の再送は request の artifact を信用せず、inbox に最初に
保存された `Completed` result だけを読み直して投影するため、別 URL への差し替えを許さず回復できる。late report と
