...
| ペイン復旧 | `restore_panes_enabled` | bool | `true` | 起動時に各セッションの前回開いていたペイン（agent / terminal）をバックグラウンドで復旧し、終了時にいたセッションとエンゲージメント段階（選択 / 集中 / 没入）へ復帰する。agent は会話の続きから再開する。`false` では終了時にこの TUI が開いた daemon 端末も終了する（[4. オーケストレーション#ペインの復旧](04-orchestration.md#ペインの復旧)） |
| キュー済みプロンプトの自動起動 | `autostart_queued_prompts` | bool | `true` | ホーム画面が、Agent ペインを持たないセッションの[起動時キューまたはライブキュー](03-commands/03-mcp.md#session_prompt-の挙動)を検知したら、そのセッションの Agent ペインをバックグラウンドで自動起動し、キュー済みプロンプトを最初のメッセージとして着手させる。人がペインを開かなくても子エージェントが走り出す。`false` では自動 spawn せず、起動時キューは次の fresh launch、ライブキューは次に Agent pane が開いたときの watcher 配送まで保持する（[4. オーケストレーション#キュー済みプロンプトの自動起動](04-orchestration.md#キュー済みプロンプトの自動起動)） |
| キュー自動起動の同時 agent 上限 | `autostart_queued_prompt_limit` | usize | `4` | キュー済みプロンプトの**fresh Agent spawn** が同時に占有できる agent 枠の上限。`running` / `waiting` の agent phase と、自動起動を dispatch してから phase / pane exit を watcher が観測するまでの予約が枠を占有する。`ended` / `exited` / `ready` / `none` は fresh spawn の空きとして扱う。準備中は 120 秒で timeout して late spawn を破棄し、pane 登録後に phase を報告しない CLI の予約は 30 秒で解放する。上限到達時も queue は消費せず、次の空き枠で人手なしに自動起動する。既存 Agent への live/auto handoff は新しい process を spawn しないためこの上限の対象外。自動起動自体の ON/OFF は `autostart_queued_prompts` で行う |
...
| 端末キー方式 | `key_scheme` | enum | `prefix` | 埋め込み端末（[没入](design/home/02-layout.md#ライブ端末closeup-内部状態)）がナビゲーション用に予約するキーの方式。`prefix`（`Ctrl-O` リーダー：`Ctrl-O` の次キーで操作。`Ctrl-O` 以外の Ctrl キーはシェル/エージェントへ流れる）/ `alt`（`Alt` 単打：bare Ctrl キーを一切奪わない。macOS は端末の Option=Meta 設定が前提） |
...
| 端末スクロールバック | `terminal_scrollback_lines` | usize | `2000` | 埋め込み端末ペインが通常画面に保持するスクロールバック行数。PR URL の一時表示も拾うため alt-screen にも同じ上限の検知専用履歴を保持する（ユーザーのスクロール表示には混ぜない）。**ライブなペインごとに最大 2 バッファ**となるため、セッション・ペインを多数開いたときの TUI メモリの主因。深い履歴が欲しければ上げ、メモリを抑えたければ下げる（上限 `50000`） |
...
| キュー自動起動の同時 agent 上限 | `autostart_queued_prompt_limit` | usize? | グローバル設定にフォールバック |
...
- 読み書きロジック・永続化に加え、編集 UI も実装済みです。ホーム画面のコマンドモードで `config` を実行すると
  設定画面が**ワークスペーススコープ**で開き、「Agent CLI」「Notifications」「Restore Panes」「Autostart Queued Prompts」「Autostart Agent Limit」「Default Branch」
  「Branch Source」「Setup Commands」「Env Vars」と、固定項目の下に並ぶ**同梱スキル機能**（`PR Skills` など）を
  編集できます。Agent CLI /
  Notifications / Restore Panes / Autostart Queued Prompts と各スキル機能は **「グローバルに従う / ローカルで上書き（On/Off）」**、Autostart Agent Limit は **「グローバルに従う / ローカルで上書き（数値）」**、Default
...
| `autostart_queued_prompts` / `autostart_queued_prompt_limit` | MCP などで積まれたキュー済みプロンプトをホーム画面が人手なしに agent 起動へつなぐか、またその同時稼働上限をいくつにするか（[4. オーケストレーション#キュー済みプロンプトの自動起動](04-orchestration.md#キュー済みプロンプトの自動起動)） |
...
| `key_scheme` | 埋め込み端末（[没入](design/home/02-layout.md#ライブ端末closeup-内部状態)）がナビゲーション用に予約するキー方式（`prefix` / `alt`）の選択 |
| `terminal_scrollback_lines` | 埋め込み端末ペインが通常画面と検知専用 alt-screen 履歴に保持する各スクロールバック行数。ライブなペインごとに最大 2 バッファ確保されるため、多数のセッションを開いたときのメモリ使用量を左右する |
...
> [没入](design/home/02-layout.md#ライブ端末closeup-内部状態)のキー処理に反映します（ホーム画面を開き直す必要はありません）。
