...
| キュー自動起動の同時 agent 上限 | `autostart_queued_prompt_limit` | usize | `4` | キュー済みプロンプトの自動起動が同時に占有できる agent 枠の上限。`running` / `waiting` の agent phase と、自動起動を dispatch してから phase / pane exit を watcher が観測するまでの予約が枠を占有する。`ended` / `ready` / `none` は空きとして扱う。phase を報告しない CLI の予約は timeout で解放する。上限到達時も queue は消費せず、次の空き枠で人手なしに自動起動する。自動起動自体の ON/OFF は `autostart_queued_prompts` で行う |
