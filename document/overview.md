...
| コマンド | 説明 |
|---|---|
| `usagi init <URL>` | リポジトリをクローンし、作業ディレクトリ（`main/`, `usagi.config`, `.usagi/`）を初期化する |
...
| コマンド | 説明 |
|---|---|
| `session` | セッション（ブランチ + worktree）の作成・管理 |
...
## プロジェクト構造（`usagi init` 実行後）

```text
<project-root>/
├── .usagi/
│   ├── state.json      # 初期化フラグ・worktree 一覧
