...
| **② workspace 毎（リポジトリ単位）** | `<repo>/.usagi/state.json`・`<repo>/.usagi/settings.json` | そのリポジトリの worktree / ブランチの状態と、プロジェクト固有の設定上書き | `infrastructure/workspace_store.rs` (`WorkspaceStore`) |
