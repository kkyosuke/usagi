...
│               │   ├── workspace_agent_drawer # root Agent-only drawer（pure geometry・conversation/VT projection・overlay 合成）
...
`TerminalSession` に残る。Workspace Agent drawer は `AppState` の専用 open/closed state と root target の
Agent-only pane admission を持ち、既存 modal が無いときだけ `Ctrl-O g` / header button で toggle する。
描画は `workspace_agent_drawer` の right-anchored geometry と drawer 専用 terminal viewport を使い、open 中だけ
root の selected Agent を foreground attach する。close 時は managed-session foreground へ戻り、root generic
Terminal / Diff は pane/runtime に admission されない。前面の Pull Request
...
| Agent tab の durable display intent | `crates/tui/src/usecase/application/agent_tab_intent.rs` が `AgentTabIntent` domain、reconcile、typed persistence port を所有する。UI 固有の order / selection / dismissal を `usagi-core` domain へ入れない。合成ルート `src/runtime/agent_tab_intent.rs` が secret-free な `<data-dir>/tui/workspaces/<workspace-id>/agent-tabs.json` を private directory/file として扱い、file lock、revision CAS、atomic publish、corrupt quarantine を束縛する。future schema は元 bytes を変更しない read-only error とする |
