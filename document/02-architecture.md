...
│           └── presentation/    # 画面描画（各画面 view・共通 widget）・キー入力マッピング・起動バナー
...
|---|---|
| `presentation/` | 画面描画（各画面の view・共通 widget）・キー入力マッピング。描画は v1 と同じく自前の差分レンダリングで行い、UI フレームワークに依存しない |
...
| 画面描画・共通 widget | `crates/tui/` の `presentation/` |
