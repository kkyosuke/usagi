...
│               │   ├── workspace          # ホーム画面（header ＋ state-backed な左 session menu／右 closeup の 2 ペイン）の状態と描画
