...
| Agent tab の durable display intent | `crates/tui/src/usecase/application/agent_tab_intent.rs` が `AgentTabIntent` domain、reconcile、typed persistence port を所有する。UI 固有の order / selection / dismissal を `usagi-core` domain へ入れない。合成ルート `src/runtime/agent_tab_intent.rs` が workspace-scoped private file lock、CAS、atomic publish、quarantine を束縛する |
