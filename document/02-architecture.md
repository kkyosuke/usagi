...
shell が transport として担い、daemon-authoritative な session 一覧・metrics・git 差分
だけをキャッシュして毎フレーム投影へ渡す。Home の row state・selection・入力・描画は
controller が単独で所有し、旧 `Workspace` view の二重定義は残さない。
