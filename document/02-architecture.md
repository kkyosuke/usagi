...
│               │   └── pr_modal            # Pull Request ポップアップ（canonical PrEntry 一覧を出す中央モーダル）の状態と描画
...
（`agent` / `close` / `diff` / `env` / `terminal`）を持つ。`interpret` は入力を
...
- **`update`** は実装済み: 通常は最新 GitHub Release を、`usagi update -v` では5行固定の一覧から `↑` / `↓` で選んで Enter で決定した release を、platform 固有 archive と SHA-256・release version artifact を使って
  ...
  installer 自身が lock を解放する前に atomic rename 後の exact installed binary を内部 managed Doctor として unbound 接続で起動し、
  handoff、successor build、serving readiness の検証まで同じ直列化区間に含める。daemon が無ければ update は新規起動しない。
  live Agent の process-local MCP authority を安全に移せない場合、Doctor は replacement を拒否して Agent と旧 daemon を維持する。
  選択した旧 release が managed Doctor capability を解釈できない場合も、その binary の legacy Doctor を実行せず非 0 で終える。
...
- **OS sandbox launcher `claude-sandbox`**: 隠しコマンド `usagi claude-sandbox --mode <session|root>
  ...
  `/var` `/tmp` `/etc` は `/private` 側も許可し、`/dev` 配下は
  [device node への data 書き込みだけ](#dev-の-device-node)許す）、Linux が `bwrap`。backend 不在・未対応 platform では
...
まま古い file 側 credential へ fallback して認証エラー（401）で起動できない。

- cache root は `$TMPDIR` / `$HOME` と同じく **daemon bootstrap が trusted environment から一度だけ確定**し
  （`confstr(_CS_DARWIN_USER_CACHE_DIR)` の canonical path）、launcher へ `--cache-dir <path>` として渡す。
  Agent child の環境変数は policy 解決に使わない。
