...
- **終了 status contract**: 合成ルートは `RunOutcome` と daemon の typed reply を process
  ...
  daemon endpoint の接続・transport failure、protocol rejection、application rejection は 1、引数解析
  エラーは clap の 2 を返す。daemon failure は stdout を空に保ち、stderr に safe user message と
...
- **`update`** は実装済み: GitHub Releases の platform 固有 archive と SHA-256・release version artifact を、
