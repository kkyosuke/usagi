...
- **内部フックコマンド**: Claude の `PreToolUse` フックが呼ぶ `usagi guard-workspace`（worktree の外へ
  出るツール呼び出しを拒否）と、Stop などのライフサイクルフックが呼ぶ `usagi agent-phase <phase>`
  ...
  シム（実 stdin は合成ルートが束ねる）。`agent-phase` は受け取った phase を破棄して正常終了する
  （フック配線を壊さないためのシム）。
- **OS sandbox launcher `claude-sandbox`**: 隠しコマンド `usagi claude-sandbox --mode <session|root>
  ...
  **無保護フォールバックせず起動を拒否する**（fail-closed）。sandbox 計画の純粋な決定部は `usagi-core` の
  ...
  snapshot は素の product program（`claude`）を保つ。Claude 起動を実際にこの launcher で包む配線と、`--settings`
  による前掲フック（`guard-workspace` / `agent-phase`）の hook JSON 材料化
  （`usagi_daemon::usecase::claude::scoped_settings_json`）は、それぞれ実装・ユニットテスト済みだが、現行ビルドの
  live な Claude 起動経路にはまだ接続していない（Claude は従来どおり `--mcp-config` / `--allowedTools` だけで
  起動する）。接続は E2E フィクスチャの sandbox backend 対応と併せて別 issue で行う。
