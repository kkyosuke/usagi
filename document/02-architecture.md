...
│               │   └── welcome            # トップメニュー（Open/New/Config/Quit ＋ recent 2 カラム）の状態と描画
...
| 実装 | 置き場所 |
|---|---|
| `Workspace` / `Settings` / `Issue` などのエンティティ、および画面が並べて見せる読み取り値（recent 一覧の `WorkspaceOverview` = workspace＋セッション/未 done issue/PR の各カウント） | `crates/core/src/domain/` |
