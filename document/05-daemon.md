...
完了後だけ rollover を有効化する。locator はあるが接続不能・draining・不正な場合は replacement を起動せず、安全な
typed lifecycle error を表示する。client が daemon-owned terminal や managed session をローカルに代替実行することはない。
...
| `.current.json.tmp` | private temporary | locator publish の rename 前だけ存在する。crash debris は次の publisher が `current.lock` 下でリンク先を辿らず回収する |
