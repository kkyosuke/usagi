...
置換しない。呼出元へ返る write / rename failure の temporary は回収し、hard crash が残した unique temporary も後続 save を阻害しない。

この順序により停止中の daemon は新規 request を受けず、正常終了後の discovery は stale socket への
`ConnectionRefused` ではなく `NotFound` になる。client bootstrap は `NotFound` の場合だけ replacement を一度起動する。
...
相対 endpoint、`active` または `draining` の state を持つ。current locator と socket endpoint は永続データではなく、
