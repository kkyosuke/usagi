...
runtime mode は `USAGI_RUNTIME_MODE=development`（開発モード）または `USAGI_RUNTIME_MODE=device`（実機検証モード）で明示する。development は `$USAGI_HOME` または `~/.usagi` の `dev/` 子 directory、device はその base directory を使う。未指定時は debug build（`cargo run` を含む）が development、release build が device を既定にする。プロジェクト内の runtime state も同じ定義を使い、development は `<project_root>/.usagi/dev/`、device は `<project_root>/.usagi/` に保存する。旧 `develop/` / `development/` との互換処理は行わない。
