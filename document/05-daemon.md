...
CLI operation、MCP server は共有 bootstrap を通る。binary は同じ build identity の active endpoint を
再利用し、異なる build は lifecycle restart と readiness / handshake 確認を経て切替える。development mode と
local mode は別の endpoint を使うが、どちらの mode でも同 build daemon を再利用する。したがって `cargo run`
を繰り返しても、明示的な restart をしない限り daemon が所有する Agent PTY は失われない。locator はあるが接続不能・draining・不正な
...
| `agents.json` | durable atomic JSON | Agent runtime の launch reservation、semantic operation key、safe outcome、public launch plan snapshot、process identity、runtime state、最小の `ProviderResumeRef`。provider ID は sensitive metadata とし、argv や secret を含む adapter private provision、PTY master、transcript は永続化しない |
...
usagi の `SessionId` / `TerminalRef` と provider-native conversation identity は別の型である。各 Agent runtime record は利用可能な場合だけ `ProviderResumeRef` を持ち、provider、opaque native session ID/name、adapter revision、完全な launch scope、capture provenance、last-known status / safe phase を保存する。native ID の `Debug` は redacted とし、IPC、status projection、error、日次logへ出さない。durable ID はこの専用fieldだけに保存し、public `LaunchPlan.argv`、再現用 `LaunchRequest`、environment、transcript本文、raw CLI outputには複製しない。

Claude の新規 interactive launch は daemon がUUIDを発行してspawn時だけ `claude --session-id <uuid>` を追加し、再開時は検証済み同一IDを `claude --resume <id>` として一時 provision に追加する。Codex は providerが公開する正式な構造化channelから runtime-bound IDを受け取れた場合だけ永続化し、再開時は `codex resume <id>` を一時 provision に追加する。provider state / transcript fileの場所や形式を推測・走査・parseするcapture経路は持たず、構造化IDを取得できないCodex runtimeは fail-closed でresume不可とする。native ID/nameは先頭`-`のoption-like値を拒否し、`--last` / `--continue` の暗黙選択へCLI parseが切り替わる余地を持たない。

利用者が [`ResumeAgent`](04-ipc.md#provider-conversation-resume-request) を明示したときだけ、daemonは provider/profile、native identity、adapter revision、workspace / session / worktree、current adapter capabilityを再検証する。同じscopeの live / reserved / ownership-unknown Agent、metadata欠落・曖昧・stale、legacy recordはspawn前に拒否する。成功時は新しいdaemon-owned PTY、`AgentRuntimeId`、`TerminalRef`を生成するため、crash前PTYへの再attachではない。replacement reservationと同じdurable snapshotでresume元の`identity_unknown` runtimeを`reclaimed`へ遷移し、古いincarnationがcapacityを消費したり`interrupted` projectionを支配し続けたりしない。producer `OperationId` と session incarnationで同一要求をdedupeし、別operationによる同一scopeの二重spawnはlive fenceで拒否する。

daemon restart reconciliation はunfinished recordの provider statusを `interrupted` にするが、自動resumeは行わない。TUI起動、pane inventory復元、daemon / macOS再起動も同様である。schema v1/v2 recordはprovider metadataなしでv3へ読めるが、IDを推測して補完せずresume不可のままにする。fixtureは Claude UUID固定、structured Codex capture、scope/revision mismatch、IDのargv/snapshot/IPC非露出、restart後のinterrupted projection、明示resumeのnew-terminal化、operation replayとlive二重spawn拒否を確認する。
