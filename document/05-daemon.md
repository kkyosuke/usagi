...
device mode は別の endpoint を使うが、どちらの mode でも同 build daemon を再利用する。したがって `cargo run`
...
runtime mode は `USAGI_RUNTIME_MODE=development`（開発モード）または `USAGI_RUNTIME_MODE=device`（実機検証モード）で明示する。development は `$USAGI_HOME` または `~/.usagi` の `dev/` 子 directory、device はその `device/` 子 directory を使う。未指定時は debug build（`cargo run` を含む）が development、release build が device を既定にする。プロジェクト内の runtime state も同じ定義を使い、development は `<project_root>/.usagi/dev/`、device は `<project_root>/.usagi/device/` に保存する。既存 usagi が使う base directory には触れない。旧 `develop/` / `development/` との互換処理は行わない。
したがって開発中に実機検証用の record / locator / lock / daemon-owned state へ触れず、必要なら `USAGI_RUNTIME_MODE=device cargo run` で debug build のまま実機検証モードを選べる。`USAGI_HOME` を明示しても同じ分離を適用する。daemon が起動する Agent の MCP server には選択した mode も転送するため、Agent の完了報告も同じ daemon に届く。
