...
`data_dir` は release では `$USAGI_HOME` または `~/.usagi`、debug（`cargo run` を含む）ではその `develop/` 子 directory である。`develop/` が未作成で旧 `development/` が存在する環境では、既存の debug state を引き継ぐため `development/` を使用する。
...
state が無い場合は、検証済みの `<repository>/.usagi/state.json` session も available record として同じ atomic write で採用する。
