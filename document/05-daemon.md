...
再利用する。bootstrap が build identity mismatch と判定した場合の shipping 経路は、manual restart と同じ
...
live Agent terminal を継続できない。fresh daemon は unfinished runtime を `identity_unknown` へ reconcile し、旧
`TerminalRef` を live として復元しない。この production gap は
[#507](../.usagi/issues/507-fix-daemon-planned-restart-active-draining-generation-rollover.md) で追跡する。前提となる
cross-process authority は
[#516](../.usagi/issues/516-refactor-daemon-cross-process-generation-registry-standby-handoff-authority.md)、owner runtime
の永続化と handoff は
[#518](../.usagi/issues/518-refactor-daemon-owner-generation-runtime-shard-global-resource-allocator.md) に分割する。locator はあるが
接続不能・draining・不正な場合は replacement を起動せず、安全な typed lifecycle error を表示する。client が
...
shutdown signal
  ...
  -> owner generation の current locator / socket を retire
...
`stop` と owner cleanup は最初に読んだ `(pid, started_at)` を最後まで保持する。`daemon.json` の save と
...
`NotFound` の場合だけ replacement を一度起動する。
locator が存在する接続失敗、draining、不正 endpoint では従来どおり replacement を起動せず fail closed にする。
...
`daemon.json` は `pid` と `started_at` を持つ。`current.json` の型は generation、daemon directory からの
...
planned daemon generation の終了時に owner が両方を回収する。`current.lock` は空の同期 node として残ってよい。
`record.lock` も record transaction の安定した同期 node として残る。
...
draining owner として維持する機構ではない。cross-process registry / admission は
[#516](../.usagi/issues/516-refactor-daemon-cross-process-generation-registry-standby-handoff-authority.md)、owner shard /
exit-capacity handoff は
[#518](../.usagi/issues/518-refactor-daemon-owner-generation-runtime-shard-global-resource-allocator.md)、shipping lifecycle
統合は [#507](../.usagi/issues/507-fix-daemon-planned-restart-active-draining-generation-rollover.md) で追跡する。
draining owner endpoint へ client request を送る経路は
