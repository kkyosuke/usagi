...
| `agents.json` | durable atomic JSON | Agent generation/terminal ownership と runtime の launch reservation、semantic operation key、safe outcome、public launch plan snapshot、process identity、runtime state、最小の `ProviderResumeRef`。ownership と runtime record は一つの snapshot で遷移する。provider ID は sensitive metadata とし、argv や secret を含む adapter private provision、PTY master、transcript は永続化しない |
...
usagi の `SessionId` / `TerminalRef` と provider-native conversation identity は別の型である。各 Agent runtime record は利用可能な場合だけ `ProviderResumeRef` を持ち、provider、opaque native session ID/name、adapter revision、完全な launch scope、capture provenance、last-known status / safe phase を保存する。native ID の `Debug` は redacted とし、IPC、status projection、error、日次 log へ出さない。durable ID はこの専用 field だけに保存し、public `LaunchPlan.argv`、再現用 `LaunchRequest`、environment、transcript 本文、raw CLI output には複製しない。redaction が保証するのはこれら durable snapshot・IPC・projection・log の各面であり、provider ID は spawn 時の一時 provision として子 process の argv に載るため、同一 host の process 一覧には露出し得る（provider CLI の入力契約上不可避）。

Claude の新規 interactive launch は daemon が UUID を発行して spawn 時だけ `claude --session-id <uuid>` を追加し、再開時は検証済みの同一 ID を `claude --resume <id>` として一時 provision に追加する。Codex の ID は、provider が公開する正式な構造化 channel を受ける daemon 境界（structured capture）だけが永続化でき、再開時は `codex resume <id>` を一時 provision に追加する。現在のビルドはこの境界へ ID を供給する capture 経路を持たないため、Codex runtime は常に fail-closed で resume 不可である。provider state / transcript file の場所や形式を推測・走査・parse する capture 経路は持たない。native ID/name は先頭 `-` の option-like 値を拒否し、`--last` / `--continue` の暗黙選択へ CLI parse が切り替わる余地を持たない。

利用者が [`ResumeAgent`](04-ipc.md#provider-conversation-resume-request) を明示したときだけ、daemon は provider/profile、native identity、adapter revision、workspace / session / worktree、current adapter capability を再検証する。同じ scope の live / reserved / ownership-unknown Agent、metadata の欠落・曖昧・stale、legacy record は spawn 前に拒否する。成功時は新しい daemon-owned PTY、`AgentRuntimeId`、`TerminalRef` を生成するため、crash 前 PTY への再 attach ではない。replacement reservation と同じ durable snapshot で resume 元の `identity_unknown` runtime を `reclaimed` へ遷移し、古い incarnation が capacity を消費したり `interrupted` projection を支配し続けたりしない。producer `OperationId` と session incarnation で同一要求を dedupe し、別 operation による同一 scope の二重 spawn は live fence で拒否する。

daemon restart reconciliation は unfinished record の provider status を `interrupted` にするが、自動 resume は行わない。TUI 起動、pane inventory 復元、daemon / macOS 再起動も同様である。schema v1/v2 record は provider metadata なしで v3 へ読めるが、ID を推測して補完せず resume 不可のままにする。fixture は Claude UUID 固定、structured Codex capture、scope/revision mismatch、ID の public plan argv / snapshot / IPC 非露出、restart 後の interrupted projection、明示 resume の new-terminal 化、operation replay と live 二重 spawn 拒否を確認する。
...
generation coordinator は active daemon を一つだけ持つ。Agent production owner はこの coordinator を runtime
snapshot と合成し、active generation だけが Agent admission/control mutation
