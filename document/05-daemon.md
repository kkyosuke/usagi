...
session incarnation で fence する。この vocabulary を worktree effect、snapshot 通知、reconcile worker を
伴う実行時 daemon IPC に接続する作業は [#268](../.usagi/issues/268-feat-daemon-session-lifecycle-runtime.md) が
管理する。現在の IPC server は session-shaped request を durable lifecycle operation として実行しない。
