...
CLI operation、MCP server は共有 bootstrap を通る。binary は同じ advertised build tuple の active endpoint を
再利用する。現行 client / server の tuple はともに
`{ version: CARGO_PKG_VERSION, commit: "unknown", target: ARCH }` で、production composition は runtime mode に
かかわらず `force_restart = false` を渡す。そのため同じ version / target の別 artifact を再buildしても old daemon を
同一 build と誤認して再利用し、#275 の development 毎 bootstrap restart も実行しない。この artifact identity と
channel policy の gap は
[#528](../.usagi/issues/528-fix-daemon-build-artifact-identity-safe-rollover-trigger.md) で追跡する。

bootstrap が build identity mismatch と判定できた場合の shipping 経路は、manual restart と同じ
**旧 daemon の stop → fresh daemon の start → readiness / handshake 確認**であり、active / draining process の
handoff ではない。development mode と local mode は別の endpoint を使うが、どちらの mode でも同 build daemon を
再利用する。

したがって TUI の終了や同 build client の再接続だけでは daemon-owned Agent PTY は失われない。一方、明示的な
`daemon restart` または build replacement は旧 owner process を終了するため、その process が持つ PTY master と
...
manual restart / detected build mismatch
...
| `usagi daemon stop` | 稼働中 daemon に終了を要求し、endpoint cleanup の完了後に lifecycle record を消去する。stale record は process に signal を送らず、singleton lock 下で stale endpoint を回収してから消去する |
...
IPC endpoint は `serve` が lock を取得して lifecycle record を登録した後に、明示的な ready hook からだけ公開する。
lock を取得できない replacement は ready hook に到達しないため session request を受理できず、endpoint 公開に
失敗した daemon は endpoint cleanup の完了を証明できた場合だけ lifecycle record を消去して終了する。接続済み client は publish 済み generation の lifecycle
...
`daemon.json` は endpoint retirement の completion fence である。running `stop` は SIGTERM を送っても先行消去せず、
...
現行 `(pid, started_at)` は durable incarnation fence であって exact OS process identity ではない。比較と conditional clear は
一部 field でなく `DaemonRecord` 全体を使うため、exact process identity を record に追加する lifecycle identity 変更と
rebase するときも、その field を含む whole-record fence を維持する。

`stop` と owner cleanup は最初に読んだ lifecycle record 全体を最後まで保持する。`daemon.json` の save と
conditional clear は private な `record.lock` の同じ cross-process transaction で直列化し、比較と unlink の間に
replacement save が割り込む隙間を作らない。同じ pid が再利用されても `started_at` が異なる replacement record は
...
`daemon.json` は `pid` と `started_at` を持つ。この tuple は durable incarnation fence であり、stale cleanup と
conditional clear は lifecycle record 全体を比較する。`current.json` の型は generation、daemon directory からの
...
#514 / #515 を前提に、cross-process registry / admission の
