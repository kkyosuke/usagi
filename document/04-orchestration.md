...
- **同時稼働上限**は agent phase で数えます。`running` / `waiting` は枠を占有し、`ended` / `ready` / `none` は空きとして扱います。
  ペインやプロセスが生きていても agent のターンが `ended` になれば次の queued prompt を起動できます。上限に達しているときは
