...
各 worktree のシェル / Agent のプロセスは（Unix では）**常駐 daemon が所有する端末**として動き、TUI の
「ターミナルプール」は worktree パスをキーに各端末への attach クライアントを保持します。セッションを
切り替えても、TUI を閉じても、端末（とその中の Agent CLI）は daemon 側で走り続けます（daemon を起動
できないときはローカル PTY にフォールバックし、その端末は従来どおり TUI と命運を共にします。接続時の
executable-generation handshake で TUI と daemon の build が一致しないときも、ユーザーが明示的に開いた新規ペインは
同じ fallback を使います。これは `cargo run` の再ビルド前に起動した daemon が残る場合を含みます。旧 daemon と、
その daemon がすでに所有する Agent は停止しません。保存済み terminal id の復旧では local fallback しません。
queued prompt の自動起動は daemon 不通時に local fallback しません。build mismatch のときだけ、daemon の永続 terminal
registry が存在し対象 worktree の terminal がないと確認できれば local fallback できます。registry がない旧 daemon、
読み取り失敗、対象 worktree の terminal がある場合は、旧 Agent が生存している可能性があるため復旧失敗または再試行として
記録します。仕組みの
全体像は [proposals/02-daemon.md](proposals/02-daemon.md)）。`session remove` でセッションを破棄する
ときは、その worktree パス配下のシェル / Agent も（daemon 側の端末ごと）終了させ、さらに**その worktree の会話履歴も削除します**
...
[design/home/04-keys.md](design/home/04-keys.md#ライブ端末closeup-内部状態) を参照してください。
...
端末プロセスは daemon が所有するため usagi の終了では死にませんが、TUI 側の**ペイン構成**（どのセッションに
どのタブが開いていたか）はプロセスと共に消えます。次回起動時にそれらを呼び戻すのが**ペインの復旧**で、設定
[`restore_panes_enabled`](05-settings.md#設定項目)（既定 ON）で切り替えます。

- **保存**: 各セッションのペイン構成（タブ順のペイン種別＝agent / terminal と、agent ペインはどの CLI か、
  どのタブがアクティブだったか、そして daemon 側の terminal id）を worktree 別のスナップショットに記録します
  （保存フォーマットは
...
- **復旧**: 起動直後、まだどのペインも attach していない段階で、各セッションのスナップショットを読み、
  記録されたペインをバックグラウンドで呼び戻します。スナップショットに terminal id が残っていれば、まず
  **daemon で走り続けている当の端末へ再 attach** します——走行中の Agent を会話も画面もそのまま引き継ぐので、
  再 spawn も resume も要りません。daemon が `AttachRejected(Missing)` でその id の不在を明示したときだけ
  従来どおり spawn し直します。daemon 再起動後に生存 process を adopt 済みで画面 stream だけ復元できない場合や、
  transport / handshake error は ownership 不明として fresh spawn を止め、二重 Agent を作りません。安全に再 spawn
  できる場合、terminal ペインは素のシェルを開き直し、agent ペインは記録された CLI を
  起動して、対象 worktree に前回の会話が残っていれば**前回の続きから再開**します（`agent` 起動時の再開
  ロジックと同じ。上記「アクティブなセッションと AI 連携」参照）。復旧されたペインは監視スレッドが拾うため、
  attach しなくても左ペインのバッジ（`▶ running` / `◆ waiting` / `✓ done`）が動きます。
...
- 復旧は**ベストエフォート**です。スナップショットが無いセッションや、呼び戻しに失敗したペインは黙って飛ばし、
  画面の起動を妨げません。設定を OFF にすると保存も復旧も行わず、常にまっさらな状態で起動します。OFF
  のまま usagi を終了すると、この TUI が開いた daemon 所有の端末も明示的に終了します。terminal id を保存しない
  状態で端末だけを daemon に残すと次回起動から再 attach できないためです。
...
（`~/.usagi/agent-prompts/`）に積まれます。同時に daemon/TUI 共通の durable start request を
`~/.usagi/agent-start-requests/` へ publish します。**キュー済みプロンプトの自動起動**は daemon がこの request を検知したら、
...
までエージェントが走り出さない、という自律オーケストレーションのギャップを埋めます。TUI が閉じていても daemon の
control tick が lease 付き claim を取得し、daemon 所有 Agent を開始します。
...
- **daemon が consumer です**。グローバルな同時実行上限に空きがある tick だけ claim し、保存済み CLI / model、workspace
  env、MCP/phase wiring を解決して daemon terminal を spawn します。terminal registry を保存してから初期 prompt を PTY へ
...
- 既に **live agent ペイン**を持つセッションは新規 spawn しません。明示 `session_prompt(mode=queue)` の prompt は既存
  Agent へ渡さず、次の fresh launch まで起動時キューに残します。一方、daemon 所有 Agent は TUI を閉じても生存しますが、
  TUI 不在中は live marker が無いため `session_prompt(auto)` が起動時キューを選びます。この `auto` のフォールバック記録
  だけは、次回 TUI が既存 Agent pane を持つと（典型的にはその daemon Agent への再 attach 後）、監視スレッドが
  ...
  daemon terminal では durable prompt input に request id を付け、daemon が PTY へ書き込んだ後の `InputResult` ACK を
...
- TUI の daemon autospawn は daemon record と socket が ready になるまで（最大 3 秒）待ちます。unattended な復旧・queue
  launch は daemon 不通時に TUI-local PTY へ fallback せず、queue の retry/backoff へ戻します。自動 worker が一見起動しても
  TUI 終了と同時に止まる縮退を避け、daemon ownership を確認できたときだけ永続 pane として開始します。人が明示的に開く
  pane は従来どおり local fallback を許します。unattended fresh Agent の前には、pool が把握する対象 worktree の live daemon
  terminal ID 集合と `terminals.json` の生存 ID 集合を完全一致で照合します。既知の remote terminal-only pane だけなら Agent を
  追加できますが、registry に未知 ID がある、既知 ID が registry から欠落する、registry を読めない場合は retry とし、snapshot に
  無い Agent を複製しません。local terminal は daemon ID を持たないため、この照合を迂回できません。
