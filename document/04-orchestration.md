...
| バックグラウンド常駐と入力待ち通知 | `terminal` / `agent`（監視） | [issue 027](../issues/027-agent-wait-notify.md) |
...
- ✅ **`terminal`**：アクティブ／選択中の worktree（未選択時はワークスペースルート）で対話シェルを右ペインに埋め込み起動（疑似ターミナル portable-pty + vt100）。`Ctrl-O` でデタッチ（[issue 006](../issues/006-terminal.md)）。
- ✅ **`agent`**：`terminal` と同じ埋め込みシェルを起動し、実効設定の Agent CLI（既定 `claude`）を自動入力（実質 `terminal` → `claude`）。([issue 026](../issues/026-agent.md))。
- ✅ **バックグラウンド常駐と入力待ち通知**：`terminal` / `agent` で開いたセッションは worktree ごとに常駐し、
  `Ctrl-O` デタッチ後も動作継続（再実行で再アタッチ）。監視スレッドが各セッションのターミナルベルを監視し、
  アタッチ中でないセッションが入力待ちになると左ペインに `◆` マーカー＋デスクトップ通知で知らせる
  （`infrastructure/terminal_manager.rs` の `TerminalManager`、判定ロジックは純粋な
  `infrastructure/session_monitor.rs`）。([issue 027](../issues/027-agent-wait-notify.md))。
