...
- **起動時キューとライブキューの両方**を対象にします。ライブペインを持たないセッションでは、
  ...
  の `auto` は agent phase ファイル（`ended` でも残る）からライブ判定するため、**エージェントが終了済み・TUI が異常終了**
  などで実際にはペインが無いのにライブキューへ振り分けられることがあります。ペインが無ければ流し込み先が無く滞留し
  続けるので、TUI 側の権威的なペイン生存判定（PTY の生死）で「ペイン無し」と分かるこのセッションに対しては、
