...
セッションは**作成時に「誰が起動したか」を記録**します。人がホーム画面の[切替（Switch）](design/home/02-layout.md#切替switch既定)で
作成すれば `human`、エージェントが MCP の `session_create` / `session_delegate_issue` で作成すれば `mcp` が
`state.json` の [`SessionRecord.origin`](data/02-workspace.md#セッションごとsessionrecord) に一度だけ書き込まれ、
以後（切替・メモ編集・同期など）変わりません。コーディネータ役のエージェントは `session_list` / `session_status` の
...
**あるセッションの中から** MCP の `session_create` / `session_delegate_issue` を呼ぶと、その親セッション名が
