...
- `agent` は設定の Agent CLI（`claude` / `gemini` など。[5. 設定](05-settings.md)）を埋め込みシェルで起動し、
  usagi の issue MCP サーバ（`usagi mcp`）を組み込みます。ローカル LLM が有効なら `usagi llm-mcp` も組み込み、
...
セッションを切り替えても裏で動き続けます。入力待ちになったセッションは左ペインの `◆ waiting` マーカーと
デスクトップ通知で知らされます。この埋め込みターミナルの永続化・切り替え・入力待ち通知の詳細は
[design/05-home.md](design/05-home.md#没入のキー操作attached--terminal--agent-実行中) を参照してください。

### Agent フックによる状態報告

`claude` で起動した Agent が「稼働中（running）」か「入力待ち（waiting）」かを正確に判定するため、usagi は
...
| `SessionEnd` | `ended` | Agent 終了（素のシェルに戻る） |
...
- フックは payload を stdin で受け取り、usagi はそこから `cwd`（Agent を起動した worktree）を読んで対象を特定
  ...
  読み取って左ペインの `▶ running` / `◆ waiting` を駆動します（描画と検知の詳細は
