...
- `agent`（Claude）は対象 worktree に前回の会話が残っていれば `claude --continue` で**前回セッションの続きから**起動し、
  無ければ通常起動します（挙動の正本は [3.2 TUI 内コマンド#agent](03-commands/02-tui.md#agent)）。
...
（Claude の transcript ディレクトリ `~/.claude/projects/<encoded>` と、usagi が記録する Agent phase
