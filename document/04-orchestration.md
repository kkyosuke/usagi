...
- ✅ **`terminal`**：アクティブ／選択中の worktree で対話シェルを右ペインに埋め込み起動（疑似ターミナル portable-pty + vt100）。`Ctrl-O` でデタッチ（[issue 006](../issues/006-terminal.md)）。
- 🚧 **同期・統合・AI 連携**（`ai` / `sync` / `finish` / `list` / `clean` /
