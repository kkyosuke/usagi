...
- `agent` は設定の Agent CLI（`claude` / `codex` / `gemini` など。[5. 設定](05-settings.md)）を埋め込みシェルで起動し、
  ...
  `session_pr` / `session_remove` で並行セッションを作成・委譲・PR 参照・整理できます（別セッションのエージェントへのタスク委譲・不要セッションの削除）。ローカル LLM が有効なら
...
`~/.gemini/tmp/<project>/chats` のうち当該 worktree のもの）。あわせて usagi が**その worktree パスをキーに保存している
一時ファイルもすべて消します**——Agent phase（`~/.usagi/agent-state/`）・PR リンク（`~/.usagi/pr-links/`）・キュー済み
プロンプト（`~/.usagi/agent-prompts/`）・ペイン構成（`~/.usagi/open-panes/`）。これらの削除はユースケース層の
...
| 段 | 仕組み | 対象 | 効果 |
|---|---|---|---|
| ソフト | system prompt（`--append-system-prompt` ／ Codex の `developer_instructions`）に「作業はこの worktree 配下だけで完結させ、親のメインリポジトリには触れない」旨を含める | Claude / Codex（system prompt を持つ全 CLI） | Agent に意図を伝える指示。強制力はない |
| ハード | `PreToolUse` フックに `usagi guard-workspace` を差し込み、ツールの対象パスが worktree の外なら**そのツール呼び出しを拒否**する | Claude | 指示を破った読み書きを実際にブロックする |

- ハード側は `PreToolUse` の payload（stdin の JSON）から `cwd`（worktree）と `tool_input.file_path`（ツールが
  触れようとするパス）を読み、対象パスを worktree 基準で正規化（`.` / `..` を解決）して worktree の外に出るかを
  判定します。外に出る場合だけ、Claude の `PreToolUse` 契約どおり `permissionDecision: "deny"` を stdout に返して
  拒否します。worktree 内・パスを持たないツール（`Bash` / `Grep` など）・解釈できない payload は素通しし、Claude
  の通常の許可フローに委ねます。
