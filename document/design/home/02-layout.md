...
Agent の状態（アイコン＋テキスト）と、その行の右端に**更新時刻**（[後述](03-sidebar.md#更新時刻)の `Nmin ago`）・**ahead/behind マーカー**（[後述](03-sidebar.md#aheadbehind-マーカー)の `↑N ↓M`）・**差分バッジ**（[後述](03-sidebar.md#差分バッジ)の `+N -M`）・**PR バッジ**（[後述](03-sidebar.md#pr-バッジ)の `#N`）です。
