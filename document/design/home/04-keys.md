...
| マウス左クリック（ドラッグなし） | ポインタ上の `http(s)` リンク、またはサイドバーの `#N` PR バッジ（[PR バッジ](03-sidebar.md#pr-バッジ)）があれば既定のブラウザで開く（[没入のリンクを開く](#没入のリンクを開くマウス)）。サイドバーのセッション行を**ダブルクリック**するとそのセッションへ切り替える（ライブなら再アタッチ。切替のダブルクリックと同じ確定）。タブチップ・リンク・バッジ・行以外をクリックしても何も起きない |
...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  管理画面ではホイール以外のレポートと同様に読み捨てます）。唯一の例外がホーム画面の**左クリック**で、
  当たり判定が[切替の左ペインのセッション行](#モードとキー操作)に当たればその行を選択（ダブルクリックで確定）、
  [左下のマスコット](02-layout.md#レイアウト)に当たれば反応を再生します（外れたクリック・ホイール・その他のレポートは
  従来どおり読み捨て）。
...
> 左ボタン押下は `ClickEvent`、それ以外は読み捨てて、キー入力とあわせて `Input` として各画面へ渡します。ホーム画面の
> イベントループ（`home/event`）だけが `Input::Click` を受け取り、まず切替の左ペインのセッション行（座標→行は `home/ui`
> の `left_pane_session_at`、ダブルクリック判定は `home/event` の `switch_click`）を判定し、外れたらマスコットの当たり矩形
> （`home/ui` の `mascot_hit_rect`＝描画と同じレイアウト計算）に当たれば反応を起動します（どちらにも当たらないクリック・
> その他の画面・ホイールは反応しません）。代替スクリーンとマウスレポートの有効化は
