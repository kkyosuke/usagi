...
| （各ワークスペースの `+ new session` 行） | セッション一覧の末尾に常駐する[新規セッション行](03-sidebar.md#新規セッション行-new-session)。カーソルを乗せて印字可能な文字を打つとその文字を 1 文字目にインライン名入力が開き、`Enter` は空の入力欄を開く。左クリックでも入力モードに入る（ダブルクリック不要）。統合（unite）モードではワークスペースごとに 1 行ある |
...
| `a` / `Ctrl-A` | カーソル中セッションで Agent ペインを新規起動して没入（`Ctrl-A` は IME ON のままでも届く別キー。物理 `Home` と同じキーコード） |
| `c` | 左ペイン内のインライン名入力を開いてセッションを新規作成（`Enter` で作成 → 在席。空文字・重複はバリデーション。`Esc` で入力をキャンセル）。`+ new session` 行にカーソルがあるとき・クリックしたときも同じ入力欄が開く。統合（unite）で複数 workspace を表示している場合は、カーソルがある workspace のブロック内に入力欄を出す（折りたたんだ workspace で作成すると自動で開く）。名前は共通の入力ウィジェットで編集され、`←`/`→`/`Home`/`End` でキャレット移動、`Backspace`/`Del` でキャレット前後の削除に対応 |
...
> **`Ctrl-A`／`Ctrl-E` と物理 `Home`／`End` は同一キー**: 端末は `Ctrl-A` を `Home`、`Ctrl-E` を `End` と同じキーコードで送るため、切替では物理 `Home` を押すと Agent 起動（`a` と同じ）、物理 `End` を押すとメモ編集（`n` と同じ）が開きます（IME を ON にしたままでも届く別名を兼ねる）。この 2 キーは Agent／メモに割り当て済みのため、一覧の先頭・末尾へ一気に飛ぶ操作は用意していません（上下移動は `↑`/`↓`・`j`/`k`）。
