...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  **ホバー（ポインタ移動）**で、左クリックは当たり判定が[切替の左ペインのセッション行](#モードとキー操作)に当たればその行を選択
  （ダブルクリックで確定）、[左下のマスコット](02-layout.md#レイアウト)に当たれば反応を再生し、ホバーは PR を持つセッションの行で
...
> イベントループ（`home/event`）だけが `Input::Click` / `Input::Hover` を受け取り、クリックはまず切替の左ペインのセッション行（座標→行は `home/ui`
> の `left_pane_session_at`、ダブルクリック判定は `home/event` の `switch_click`）を判定し、外れたらマスコットの当たり矩形
