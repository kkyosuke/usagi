...
> タブ（ペイン）の切替は切替・在席では `Ctrl-N`/`Ctrl-P`（切替では素の `←`/`→` も同じ）、没入では[キー方式](#没入のキー操作attached--terminal--agent-実行中)に従い `Ctrl-O n`/`p`（`alt` 方式は `Alt-→`/`←`）で行えます。切替・在席・没入では**タブストリップのチップを左クリック**しても切り替えられます。没入では `Ctrl`+`Shift`+`N`/`P` でアクティブタブを右／左へ移動でき、タブチップをドラッグして任意の位置へ並び替えられます。ペインの追加は
...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  当たり判定が[切替・在席の左ペインのセッション行](#モードとキー操作)に当たればその行を選択（切替は選択／在席はフォーカス切替、いずれもダブルクリックで確定）、切替・在席の右ペインのタブチップに当たればそのタブを選択、
