...
- **`prefix` 方式（既定）**: リーダー `Ctrl-O` **だけ**を奪い、操作はその**次のキー**で指定します — `Ctrl-O o`（[切替](02-layout.md#切替switch既定)へ）・`Ctrl-O a`（在席へズームアウトしてアクションメニュー。terminal タブの追加はここから）・`Ctrl-O n`/`Ctrl-O p`（タブを前後へ。`Ctrl-O →`/`←` も）・`Ctrl-O g`（agent タブを追加してアクティブに）・`Ctrl-O e`（[メモ編集](05-overlays.md#セッションメモの編集)。閉じると同じペインへ復帰）・`Ctrl-O s`（[サイドバー開閉](03-sidebar.md#サイドバーの開閉ctrl-b)）・`Ctrl-O q`（没入を抜けてアプリ終了の[確認モーダル](05-overlays.md#終了確認モーダル)）。`Ctrl-O Ctrl-O` でも切替へズームアウトできます（制御文字だけの 2 打なので、日本語 IME を ON にしたままでも届く）。`Ctrl-E`（行末）・`Ctrl-N`/`Ctrl-P`（履歴）・`Ctrl-T`・`Ctrl-G` などの **bare Ctrl キーはシェル / Agent へそのまま流れます**（`Ctrl-O` 自体をシェルへ送りたいときは `alt` 方式を使う）。
...
どちらの方式でも `Ctrl-^`（[直前のセッション](03-sidebar.md#直前のセッションへ切り替えctrl-)）は直キー、`Ctrl-Q`（アプリ終了の確認）は…と言いたいところですが終了は方式に従い（`prefix`: `Ctrl-O q` ／ `alt`: `Alt-q`）、`Ctrl-C` は選択中のみコピー（無ければ `SIGINT`）、`Esc` と `Ctrl-W` はシェルへ流れます。`Ctrl-W` を奪わないため、タブを閉じるのは[切替](02-layout.md#切替switch既定)（切替へ抜けて `x`）で行います。
...
> 閉じるは切替の `x`（没入からは切替へ抜けてから）で行えます（[切替](02-layout.md#切替switch既定)で `←`/`→`・`Ctrl-N`/`Ctrl-P` がタブ、`↑`/`↓` がセッション）。
