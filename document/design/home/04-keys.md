...
> タブ（ペイン）の切替は切替・在席では `Ctrl-N`/`Ctrl-P`（切替では素の `←`/`→` も同じ）、没入では[キー方式](#没入のキー操作attached--terminal--agent-実行中)に従い `Ctrl-O n`/`p`（`alt` 方式は `Alt-→`/`←`）で行えます。在席・没入では**タブストリップのチップを左クリック**しても切り替えられます。ペインの追加は
