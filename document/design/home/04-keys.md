...
| マウス左クリック（ドラッグなし） | ポインタ上の `http(s)` リンク、またはサイドバーの PR バッジ（アイコン＋件数。[PR バッジ](03-sidebar.md#pr-バッジ)）があれば既定のブラウザで開く（バッジは記録した全 PR を開く。[没入のリンクを開く](#没入のリンクを開くマウス)）。サイドバーのセッション行を**ダブルクリック**するとそのセッションへ切り替える（ライブなら再アタッチ。切替のダブルクリックと同じ確定）。タブチップ・リンク・バッジ・行以外をクリックしても何も起きない |
...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  ボタンなしの移動も報告する DECSET 1003 はサイドバーの **[PR バッジ](03-sidebar.md#pr-バッジ)のホバーポップアップ**に使い、
  それ以外のモーション／レポートは管理画面ではホイール以外と同様に読み捨てます）。例外はホーム画面の**左クリック**と
  **ホバー（ポインタ移動）**で、左クリックは当たり判定が[切替・在席の左ペインのセッション行](#モードとキー操作)に当たればその行を選択
  （切替は選択／在席はフォーカス切替、いずれもダブルクリックで確定）、[左下のマスコット](02-layout.md#レイアウト)に当たれば反応を再生し、ホバーは PR を持つセッションの行で
  PR ポップアップを出します（外れたクリック・ホイール・その他のレポートは従来どおり読み捨て）。
...
> イベントループ（`home/event`）だけが `Input::Click` / `Input::Hover` を受け取り、クリックはまず切替・在席の左ペインのセッション行（座標→行は `home/ui`
> の `left_pane_session_at`、受理可否は `click_selects_session`、ダブルクリック判定は `home/event` の `switch_click`（切替）／`focus_click`（在席）が共有の `is_double_click` で行う）を判定し、外れたらマスコットの当たり矩形
> （`home/ui` の `mascot_hit_rect`＝描画と同じレイアウト計算）に当たれば反応を起動します。ホバーは `home/ui` の `sidebar_pr_hover_at` で
> PR を持つセッションの行かを判定し、その結果を `HomeState::pr_hover` に記録（変化したときだけ再描画）して、描画時に `pr_hover_popup` の枠を
> `widgets::overlay_at` でその行の脇に重ねます（どれにも当たらないクリック／ホバー・その他の画面・ホイールは反応しません）。代替スクリーンとマウスレポートの有効化は
