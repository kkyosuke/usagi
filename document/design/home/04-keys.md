...
| 在席へズームアウト（`prefix`: `Ctrl-O a` ／ `alt`: `Alt-a`） | 在席へズームアウトしてアクションメニュー（terminal / agent などを選ぶ画面）を開く（元のペインのタブに留まり、そのプレビューの上に[メニューが浮かぶ](05-overlays.md#在席のアクションメニューオーバーレイ)。全ペインは生かしたまま） |
