...
アクション UI（[Menu / Prompt](02-layout.md#在席のアクション-uimenu--prompt)）で選択中セッションのコマンドを操作します（**Menu も Prompt も右ペイン中央に浮かぶ[オーバーレイモーダル](05-overlays.md#在席のアクションのオーバーレイ)**。キー操作は置き場所によらず同じ）。
...
| （Menu）`/` | フィルタを開始。ラベルが `Filter:` に変わり、以降の文字入力でコマンド名（前方一致・大文字小文字を区別しない）に一覧を絞り込む。`Backspace` で末尾 1 文字を削除、`↑↓`（`j`/`k` は文字入力になる）+ `Enter` で残った候補を選んで実行、`→`/`Tab` はカーソル行のピッカーを展開。一致が無ければ `No commands match the filter.`（淡色）を表示。フィルタ中は `t`/`a`/`C` などの 1 キーショートカットは文字入力として扱う（コマンド入力そのものは [Prompt](02-layout.md#在席のアクション-uimenu--prompt) が担う） |
...
| `Esc` | 1 段ずつ戻る — Menu で `/` フィルタが有効なら最初の `Esc` はフィルタを解除して全一覧へ戻し、ピッカー展開中なら畳む。そのうえで没入からズームアウトした直後は元のペインへ再アタッチ、ペインのタブ上に浮いたメニューは閉じる、ライブなペインを持つ「+ new」タブは破棄、それ以外は切替へ（[在席の説明](01-modes.md#モードと状態遷移切替在席没入)を参照） |
...
| 在席へズームアウト（`prefix`: `Ctrl-O a` ／ `alt`: `Alt-a`） | 在席へズームアウトしてアクション UI（terminal / agent などを選ぶ画面）を開く（元のペインのタブに留まり、そのプレビューの上に[アクション surface が浮かぶ](05-overlays.md#在席のアクションのオーバーレイ)。全ペインは生かしたまま） |
...
running > ready。語つきのラベルは[在席・没入の見出し](02-layout.md#在席focus)に出ます）。ラベルの先頭には **AI エージェントを表す Nerd Font のロボットグリフ（`nf-fa-android`）** を付け、
