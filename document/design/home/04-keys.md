...
| マウス左クリック（ドラッグなし） | ポインタ上の `http(s)` リンクがあれば既定のブラウザで開く（[没入のリンクを開く](#没入のリンクを開くマウス)）。サイドバーの PR バッジ（アイコン＋件数。[PR バッジ](03-sidebar.md#pr-バッジ)）をクリックすると番号一覧のポップアップをピン留めし、その中の `#<番号>` をクリックするとその 1 件をブラウザで開く（枠の外をクリックすると閉じる）。サイドバーのセッション行を**ダブルクリック**するとそのセッションへ切り替える（ライブなら再アタッチ。切替のダブルクリックと同じ確定）。タブチップ・リンク・バッジ・行以外をクリックしても何も起きない |
