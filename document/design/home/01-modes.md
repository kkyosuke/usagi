...
- **在席（Focus）** — 1 つのセッションを選んだ状態で、そのセッションで実行できるコマンドを**右ペイン**で操作します
  ...
  **在席中も左ペインのセッション行をマウス左クリックで切り替えられます** — シングルクリックでその行へ**フォーカスを移し**（在席のまま右ペインのアクション UI をその行のものへ作り直す）、同じ行を続けてダブルクリックすると**確定**（ライブなら没入へアタッチ、そうでなければ在席に留まる）します（切替のクリックと同じダブルクリック文法。ルート行 `⌂ root` も同じ。区切り線・マスコット・空行・右ペインのクリックは無視。モーダルやコマンドパレット・メモエディタが開いているあいだは受け付けません。クリックの検出にはマウスレポートが要るため、それを報告しない端末ではキーボードで操作します）。
