...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは[キー方式（`key_scheme`）](../../05-settings.md#設定項目)で決まり**、既定の `prefix` 方式ではリーダー `Ctrl-O` の次キー（`Ctrl-O o` で切替へ・`Ctrl-O a` で在席へ など）、`alt` 方式では `Alt` 単打で操作します。予約キーの全一覧・タブの追加/切替/クローズは[没入のキー操作](04-keys.md#没入のキー操作attached--terminal--agent-実行中)が正本です。
