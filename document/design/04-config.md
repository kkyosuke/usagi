...
> `Enter` で確定すると、公式インストーラ（`curl -fsSL https://ollama.com/install.sh | sh`）をバックグラウンドで実行し、
> sudo を事前認証して非対話で導入したうえで `ollama pull <model>` を行います。実行中は**スピナー**を表示し続け、
> 完了すると `Local LLM` 行が `On`（有効）の **on/off トグル**に切り替わり、カーソルが `Local LLM Model` 行へ移動して
> **モデルを選べる**状態になります（`Esc` で確定前にキャンセル可能）。`Local LLM Model` 行でモデルを変えると未導入扱いに
> 戻ります。結果（成功・失敗）は通知行に表示します。
...
| `Enter` | 入力した sudo パスワードで導入を確定（バックグラウンド実行＋スピナー） |
