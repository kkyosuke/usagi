...
起動画面で `Config`（`c` / Enter）を選ぶ、またはホーム画面のコマンドモードで `config` を実行すると
遷移する、グローバル設定の閲覧・編集を行う画面。
...
| Local · Default Branch | `session create` で worktree を切る基点 | `Local` ⇄ `Remote` をトグル（未設定時は `Default (Remote)` を表示） |
...
- ホーム画面の `config` から開いた場合は、起動中のワークスペースのパスをプロジェクトコンテキストとして
  渡すため、そのワークスペースのローカル設定（`<workspace>/.usagi/settings.json`）が常に編集対象になります。
