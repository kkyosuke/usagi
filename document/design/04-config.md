...
> **インストールアクション**で、これは選択肢を循環するのではなく動作を起動するため、山かっこなしの緑のラベル `Install` で表示します。

> **ローカル LLM の導入（#032）**: `Local LLM` 行は資材（`ollama` 本体＋選択モデル）の有無で振る舞いが変わります。
> 未導入のうちは値が `Install` のアクションで、`Space` または `Enter` で**インストールモーダル**を開きます。モーダルは
> 導入内容（`ollama` ランタイム＋選択モデル）を説明し、ランタイム導入に必要な **sudo パスワード**を伏せ字で入力させます。
> `Enter` で確定すると導入を**バックグラウンドで開始**してすぐ操作に戻ります。導入処理（公式インストーラ
> `curl -fsSL https://ollama.com/install.sh | sh` の実行＋sudo 事前認証による非対話導入＋`ollama pull <model>`）は
> 別スレッドで進み、その**生出力は画面に出しません**（`pulling manifest …` などで TUI が崩れないように抑制します）。
> 導入中も Config を含む usagi の他機能を**そのまま操作でき**、`Esc` で Config を抜けて他画面へ移動しても導入は継続します。
> 進行状況は全画面共通の[**ローディングうさぎ**オーバーレイ](#インストール中のローディングうさぎ)で表示します。
> 導入が完了すると `Local LLM` 行が `On`（有効）の **on/off トグル**へ切り替わり（次のキー操作時に反映）、完了メッセージを
> 通知行に表示します。失敗時はオーバーレイに結果を表示します。`Local LLM Model` 行でモデルを変えると未導入扱いに戻ります。
...
- **アクション行**: `Local LLM`（未導入時）の `Install` のように、選択肢を循環するのではなく動作を起動する
  行は chooser ではなく**山かっこなしの緑のラベル**で描画し、選択行では太字にします。
...
| 設定一覧 | 各設定を「カーソル列 + 変更マーク列 + ラベル(幅揃え) + 値」で 1 行ずつ表示 | 選択行：左端 `>` 赤太字／ラベル シアン太字。値は chooser（常に `< 値 >`）、ただしアクション行（`Install`）は山かっこなしの緑ラベル。変更済みはラベル左に `●`（黄色）＋値も黄色 |
...
| `→` / `l` | 選択中の項目の値を次の選択肢へ切り替え（メモリ上のみ。保存はしない）。Save ボタン上・`Local LLM` の `Install` 上では無効 |
| `←` / `h` | 選択中の項目の値を前の選択肢へ切り替え（メモリ上のみ。保存はしない）。Save ボタン上・`Local LLM` の `Install` 上では無効 |
| `Space` | `Local LLM` の `Install` 行でインストールモーダルを開く（他の行・スコープでは無効） |
| `Enter` | Save ボタン上では編集内容を `settings.json` へ保存。`Local LLM`（未導入時）上ではインストールモーダルを開く。その他の設定項目上では `→` と同じく値を次へ切り替え |
...
パスワードは共通の入力ウィジェットで編集され、キャレット位置での挿入・削除と左右移動に対応します（伏せ字でもキャレットは正しい文字間に表示されます）。
...
| `Enter` | 入力した sudo パスワードで導入を**バックグラウンドで開始**し、モーダルを閉じて操作に戻る |
