...
> CLI の `usagi config` も**グローバル設定**を対象にしますが、`usagi --help` には表示しない上級者向けコマンドです。
> 通常の設定変更は本画面（TUI のセレクタ）で行い、CLI は `$EDITOR` での編集／一覧表示に使います。詳細は [3. コマンドリファレンス（CLI）](../03-commands/01-cli.md) を参照。
...
位置がガタつきません（レイアウトシフトなし。設定が収まらないほど縦に短い端末でのみ引き上げられます）。
