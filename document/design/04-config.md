...
> **ローカル LLM の導入**: ローカル LLM の有効化は **2 段階**です。
>
> 1. **ランタイム導入（`Local LLM` 行）**: 資材は `ollama` ランタイムの有無で振る舞いが変わります。未導入のうちは値が
>    `Install` のアクションで、`Space` または `Enter` で**インストールモーダル**を開きます。モーダルはランタイム導入に必要な
>    **sudo パスワード**を伏せ字で入力させます。`Enter` で確定すると、公式インストーラ
>    （`curl -fsSL https://ollama.com/install.sh | sh`）をバックグラウンドで実行し、sudo を事前認証して非対話で導入します
>    （モデルはここでは取得しません）。実行中は**スピナー**を表示し続け、完了すると `Local LLM` 行が `On`（有効）の
>    **on/off トグル**に切り替わり、カーソルが `Local LLM Model` 行へ移動して**モデルを選べる**状態になります
>    （`Esc` で確定前にキャンセル可能）。
> 2. **モデル選択（`Local LLM Model` 行）**: ランタイム導入後、`Space` / `Enter` で**モデル選択モーダル**を開きます。
>    モーダルは提供モデル（`qwen2.5-coder:7b` / `:3b` / `:1.5b` / `qwen2.5:7b`）を導入状態のマーカー付き（`✓ 導入済` /
>    `⬇ 未導入`）で一覧し、↑↓ で選び `Enter` で決定します。**導入済み**のモデルはそのまま使用モデルに採用し、**未導入**の
>    モデルは `ollama pull <model>`（sudo 不要）を**その場でスピナー表示しながら取得**してから採用します。`Esc` でキャンセル。
>
> 結果（成功・失敗）は通知行に表示します。既定は無効で、usagi が勝手に有効化することはありません
...
| `Enter` | 入力した sudo パスワードでランタイム導入を確定（バックグラウンド実行＋スピナー） |
