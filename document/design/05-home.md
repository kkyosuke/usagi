...
最下部まで伸びてうさぎの行（と区切りの空行）に届くときや、サイドバーをレールに畳んで幅が足りないときは、
一覧を隠さないよう**うさぎのほうが引っ込みます**。新しいリリースが見つかったときは、このうさぎが頭上の吹き出しで
[更新通知](#更新通知最新リリースの検知)を喋ります。
...
│ [switch] ↑↓ session / K/J move / ←→ tab / Enter focus / : palette │  ← フッター（モード別・淡色）
└───────────────────────────────────────────────────────────┘
```
...
│ [switch]  ↑↓ session / K/J move / ←→ tab / Enter focus / t new / x close tab / c new / r rename / : palette │  ← フッター（切替・操作キーはここだけ）
└──────────────────────────────────────────────────────────────────────────┘
```
...
| マスコット（左ペイン最下部） | セッション一覧の下に余白があるとき、**うさぎ**を 3 行で表示。一覧との間には空行を 1 行挟む。表情と仕草は**現在のモード**に追従し、切替 `(o.o)?`・在席 `(^.^)/`・没入 `(>.<)9`。一覧が最下部まで届くとき、またはサイドバーをレールに畳んで幅が足りないときは表示しない（一覧を隠さない）。新しいリリースがあるときは頭上の吹き出しで[更新通知](#更新通知最新リリースの検知)を喋る | 切替：マゼンタ・太字／在席：シアン・太字／没入：緑・太字（吹き出しは黄色・太字） |
...
| テキストモーダル | `man` / `history` / `session list` の出力を画面中央の**スクロール可能なモーダル**で表示（長い場合は上下に隠れ行数を表示）。`↑↓`/`j`/`k`・PageUp/PageDown でスクロール、`Esc` / `Enter` / `q` で閉じる。`man`（ヘルプ）はコマンド一覧をひと目で読めるよう**端末いっぱいに広がる大きいモーダル**で開き、幅・表示行数を端末サイズに合わせて拡縮する（端の余白は確保）。`history` / `session list` などその他は従来の**コンパクトなモーダル**（固定サイズ） | 枠付き・中央寄せ／行種別で色分け／ヒント淡色 |
...
| マウスホイール | ペイン上でスクロール。**通常画面（シェル）**では埋め込みターミナルの履歴（スクロールバック）をさかのぼる／戻す。**代替スクリーン（全画面の Agent／ページャ／TUI）**ではスクロールバックが無いため、ホイールを矢印キーに変換してそのプログラムへ転送し、プログラム自身のビューを動かす |
...
- **起動後の右ペイン（埋め込みターミナル）だけスクロールできる**: `terminal` / `agent` 実行中の右ペインでは、
  ...
  - **代替スクリーンのプログラムはそのプログラム自身がスクロールする**: 全画面 UI を持つ Agent（`claude` など）・
    ページャ（`less`）・TUI（`vim`）は**代替スクリーン**（DECSET 1049）に描画し、`vt100` はそこにスクロールバックを
    保持しません。そのためホイールを usagi の履歴スクロールに使っても無反応になります。代わりに、代替スクリーン中は
    **ホイールを矢印キー（`Shift` なしの `↑`/`↓`）に変換してプログラムへ転送**し、プログラム自身のビュー（メッセージ
    履歴・ページャの本文など）を動かします。素の端末で代替スクリーンのプログラムにホイールが効くのと同じ挙動です
    （矢印の符号化は DECCKM に従い、アプリケーションカーソルモードなら `ESC O A`/`ESC O B`、通常は `ESC [ A`/`ESC [ B`）。
...
> ただしホイールについては、`vt100` が代替スクリーンか（`alternate_screen()`）を見て分岐し、代替スクリーン中は
> スクロールバックの代わりに矢印キーへ変換して PTY へ書き込みます（端末側の代替スクロールモードをソフトウェアで
> 模倣。グローバルに DECSET 1007 を有効化しないので管理画面の一覧が誤って動く副作用はありません）。
