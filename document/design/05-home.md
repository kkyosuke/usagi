...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは[キー方式（`key_scheme`）](../05-settings.md#設定項目)で決まります**。既定の **`prefix` 方式**ではリーダー `Ctrl-O` の**次のキー**で操作します — `Ctrl-O o`（**切替**へズームアウト）・`Ctrl-O a`（**在席へズームアウトしてアクションメニュー**で terminal / agent などを選ぶ）・`Ctrl-O n`/`Ctrl-O p`（**没入のままアクティブタブを前後へ**切替。`Ctrl-O →`/`←` も）・`Ctrl-O g`（**agent タブへ移動**＝無ければ追加し、あればその 1 つへ。agent は 1 セッション 1 つ）・`Ctrl-O e`（**[メモ編集](#セッションメモの編集)**。閉じると同じペインへ復帰）・`Ctrl-O s`（**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**）・`Ctrl-O q`（**アプリ終了の[確認モーダル](#終了確認モーダル)**）。`Ctrl-O Ctrl-O` でも切替へズームアウトできます（制御文字だけの 2 打なので IME を ON にしたままでも届く）。`Ctrl-O` 以外の bare Ctrl キー（`Ctrl-E` 行末・`Ctrl-N`/`Ctrl-P` 履歴など）はすべてシェル / Agent へ流れます（`Ctrl-O` 自体をシェルへ送りたいときは `alt` 方式を使う）。
...
そのエントリの**全行**（セッションは 1〜3 行目、ルートは 1〜2 行目）に縦に走らせます（位置が分かりやすいよう左端固定。`*` 印は廃止しました）。**切替（Switch）**で
