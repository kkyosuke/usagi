...
切替 --↑↓/jk--> セッション移動 ;  --←→/hl--> タブ切替（選択中セッションのアクティブタブを実切替） ;  --Enter--> ライブ? 没入 : 在席 ;  --t--> 在席（新規ペイン作成の起点） ;  --c→インライン名入力→Enter--> 在席(新規) ;  --r→インライン表示名入力→Enter--> 表示名を変更 ;  --Esc--> 開いた元のモード ;  --Ctrl-O--> 統括
在席 --terminal/agent--> 没入（新規ペインを追加して入る） ;  --Esc--> 統括 ;  --Ctrl-O--> 切替
没入 --Ctrl-O--> 切替（全ペインは生かしたまま。タブ切替・ペイン追加・閉じるは切替で行う） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入で予約キーは `Ctrl-O` だけになり、単押しで切替へズームアウトします。タブの切替（`←`/`→`）と新規ペインの
> 追加（`t`）は切替で行います。詳細は [没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)。
...
- **切替（Switch）** — セッションを選ぶ／作る／タブを切り替えるためのピッカー。**キーボードは左ペイン**に移り（中央モーダルや
  オーバーレイは使いません）、`↑↓`/`k`/`j` で**セッション間**を移動、`←→`/`h`/`l` で**選択中セッションのタブ間**を移動します
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**`Ctrl-O` だけが予約キー**で（`Esc` を含む他のキーはすべてシェルへ流れます）、`Ctrl-O` の
  単押しで**切替**へズームアウトします。**タブの切替・ペインの追加は切替で行い**、没入の中ではタブ操作キーを持ちません。
...
`←→`/`h`/`l` で**選択中セッションのタブ間**を移動します（アクティブタブを実際に切り替えるので右ペインのプレビューも追従）。
...
│ ↑↓ session  ←→ tab  Enter select  t new  c new  r rename  Esc back  Ctrl-O home │  ← フッター（切替）
└───────────────────────────────────────────────────────────┘
```
...
含む他のキーはすべてシェルへ流れる）で、`Ctrl-O` の**単押し**で**切替**へズームアウトします。**タブの切替・ペインの
追加・閉じるは切替で行い**、没入の中ではタブ操作キーを持ちません。シェルが終了したとき、他のペインが残れば没入の
...
切り替え専用の予約キーが必要）。`Ctrl-O` は**単押し**で[切替](#切替switch)へズームアウトします（リーダーキー方式は廃止）。
タブ（ペイン）の切替・追加・閉じるは没入の中ではなく**切替で行います**——切替で `←`/`→` がアクティブタブを「ペイン軸」で
動かし、`↑`/`↓` がセッションを「セッション軸」で動かす二軸操作にまとまります。切替で別セッションを選べばライブはそのまま
再アタッチ、ライブでなければ在席に着きます。
...
> タブ（ペイン）の切替・追加・閉じるは切替で行います（[切替](#切替switch)で `←`/`→` がタブ、`↑`/`↓` がセッション、
> `t` が新規ペインの起点）。
