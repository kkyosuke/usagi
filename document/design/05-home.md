...
没入 --Ctrl-N/Ctrl-P--> タブ切替（没入のままアクティブペインを前後へ） ;  --Ctrl-T--> 在席へズームアウト（アクションメニューで terminal / agent などを選ぶ。全ペインは生かしたまま） ;  --Ctrl-G--> 新しい agent タブを追加（没入のまま） ;  --Ctrl-W--> アクティブタブを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席） ;  --Ctrl-O--> 切替（全ペインは生かしたまま） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入の予約キーは `Ctrl-O`（切替へズームアウト）・`Ctrl-T`（在席へズームアウトしアクションメニューを開く）・`Ctrl-N`/`Ctrl-P`（タブを前後へ切替）・`Ctrl-G`（agent
> タブを追加）・`Ctrl-W`（アクティブタブを閉じる）・`Ctrl-B`（[サイドバー開閉](#サイドバーの開閉ctrl-b)）です。タブの切替はどのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-W`・`Ctrl-B`**で（`Esc` を含む他のキーは
  ...
  切り替え**、`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-W` で**アクティブタブを閉じ**、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**します
...
`[attached] Ctrl-O: switch / Ctrl-T: focus / Ctrl-N/P: tab / Ctrl-G: agent / Ctrl-W: close` を表示します。
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-W`・`Ctrl-B`**（`Esc` を含む他のキーはすべてシェルへ流れる）で、
`Ctrl-O` の**単押し**で**切替**へズームアウト、`Ctrl-T` で**在席へズームアウトしてアクションメニュー**（terminal / agent などを選ぶ画面）を開き、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ切り替え**、
`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-W` で**アクティブタブを閉じ**、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**します（その分シェル / Agent 側の
...
| フッター | モード別・状態別の操作ヘルプ。統括は先頭に `[workspace]`、在席は `[session: <名前>]` を表示。切替は `↑↓ session / ←→ tab / Enter focus / t new / x close tab / c new / r rename / Esc back`、没入は `Ctrl-O: switch / Ctrl-T: focus / Ctrl-N/P: tab / Ctrl-G: agent / Ctrl-W: close` を案内 | 淡色（dim） |
...
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-W`**で、`Esc` を含む他のキーはすべてシェルへ流れます
...
**没入のまま新しい agent タブを追加**して、そのタブをアクティブにします。`Ctrl-W` は**アクティブタブを閉じ**ます
...
| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-W`・選択中の `Ctrl-C` 以外。`Esc` 含む） | そのままシェルへ転送（通常のターミナルとして操作） |
