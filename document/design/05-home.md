...
| フッター | モード別・状態別の操作ヘルプ。コマンドパレットは先頭に `[palette]`、在席は `[session: <名前>]` を表示。切替は `↑↓ session / ←→ tab / Enter focus / c new / r rename / n/Ctrl-E note / x close tab / : commands / ? keys`（メモの読み取り表示中は末尾が `Esc close note` に変わる）、在席は末尾に `? keys` を加え、没入は `Ctrl-O: switch / Ctrl-T: focus / Ctrl-^: last / Ctrl-N/P: tab / Ctrl-G: agent / Ctrl-E: note` を案内（没入はキーがシェルへ流れるため `?` ではチートシートを開けない。まず `Ctrl-O` で[切替](#切替switch既定)へ抜ける） | 淡色（dim） |
...
キーとその動作を並べます。`↑`/`↓`（`j`/`k`）でスクロール、`Esc` / `Enter` / `q` で閉じます
...
（コマンド一覧の `:man` と同じ事情）。まず `Ctrl-O` で[切替](#切替switch既定)へ抜けてから `?` を押すか、
