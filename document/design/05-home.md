...
Agent の状態（アイコン＋テキスト）と、その行の右端に**更新時刻**（[後述](#更新時刻)の `Nmin ago`）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)の `↑N ↓M`）・**差分バッジ**（[後述](#差分バッジ)の `+N -M`）です。
...
│▶ running 3min ago +124 -18│  hi                            │  │   2 行目：Agent 状態（左）＋ 更新時刻・↑N↓M・差分バッジ（右端）
...
**更新時刻**（[後述](#更新時刻)）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)）・**差分バッジ**（[後述](#差分バッジ)）を
`更新時刻 ahead/behind 差分バッジ` の順でまとめ、右端に寄せて添えます。**この 3 フィールドはそれぞれ固定幅の桁に収め、
桁幅は描画ごとに「いま表示しているセッション」の中の最大値で決めます**。そのため変更行数の桁数や "ago" の長さが行ごとに違っても、
...
- **狭いとき**: 右端のまとまりが収まらない幅では、**バッジを最優先**で右端に残します。次いで Agent 状態、ahead/behind、最後に更新時刻の順で省きます（省くかどうかは表示中で最も長い Agent ラベルを基準に行を揃えて判定するので、どの行も同じ列構成になります）。
