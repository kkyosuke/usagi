...
没入 --Ctrl-N/Ctrl-P--> タブ切替（没入のままアクティブペインを前後へ） ;  --Ctrl-T--> 在席の「+ new」タブへズームアウト（アクションメニューで terminal / agent などを選ぶ。全ペインは生かしたまま） ;  --Ctrl-G--> 新しい agent タブを追加（没入のまま） ;  --Ctrl-^--> 直前のセッション（ライブなら再アタッチ。全ペインは生かしたまま） ;  --Ctrl-E--> メモ編集（閉じると同じペインへ復帰） ;  --Ctrl-O--> 切替（全ペインは生かしたまま） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入の予約キーは `Ctrl-O`（切替へズームアウト）・`Ctrl-T`（在席へズームアウトしアクションメニューを開く）・`Ctrl-N`/`Ctrl-P`（タブを前後へ切替）・`Ctrl-G`（agent
> タブを追加）・`Ctrl-^`（[直前のセッションへ切替](#直前のセッションへ切り替えctrl-)）・`Ctrl-E`（[メモ編集](#セッションメモの編集)）・`Ctrl-B`（[サイドバー開閉](#サイドバーの開閉ctrl-b)）・`Ctrl-Q`（アプリ終了の[確認モーダル](#終了確認モーダル)）です。`Ctrl-W` は予約せず、シェルの「直前の単語を削除」としてそのまま流します（タブを閉じるのは切替の `x`）。没入ではキーがシェルへ流れるため、`:` のコマンドパレットは開けません（まず `Ctrl-O` で切替へ抜けます）。タブの切替はどのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で
...
> `t` から、agent タブの追加は没入の `Ctrl-G`・在席のメニュー・切替の `t` から、タブを閉じるのは切替の
> `x`（没入からは `Ctrl-O` で抜けてから）で行えます。詳細は [没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)。
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-E`・`Ctrl-B`・`Ctrl-Q`**で（`Esc` や `Ctrl-W` を含む他のキーは
  ...
  切り替え**、`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-E` で**[メモ編集](#セッションメモの編集)**（閉じると同じペインへ復帰）、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**し、`Ctrl-Q` で**アプリ終了の[確認モーダル](#終了確認モーダル)**を出します
  （これらの予約キーはシェル / Agent 側のキー操作からは奪われる点に注意）。`Ctrl-W` は**予約せず**シェルの「直前の単語を削除」として流すため、タブを閉じるのは切替（`Ctrl-O` で抜けて `x`）で行います。terminal タブの追加は在席のアクションメニュー（`Ctrl-T` で開く）か切替（`t`）でも行えます。
...
`[attached]  Ctrl-O: switch / Ctrl-T: focus / Ctrl-^: last / Ctrl-N/P: tab / Ctrl-G: agent / Ctrl-E: note` を表示します。
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-E`・`Ctrl-B`**（`Esc` や `Ctrl-W` を含む他のキーはすべてシェルへ流れる）で、
`Ctrl-O` の**単押し**で**切替**へズームアウト、`Ctrl-T` で**在席へズームアウトしてアクションメニュー**（terminal / agent などを選ぶ画面）を開き、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ切り替え**、
`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-E` で**[メモ編集](#セッションメモの編集)**（閉じると同じペインへ復帰）、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**します（その分シェル / Agent 側の
...
| フッター | モード別・状態別の操作ヘルプ。コマンドパレットは先頭に `[palette]`、在席は `[session: <名前>]` を表示。切替は `↑↓ session / ←→ tab / Enter focus / c new / r rename / n/Ctrl-E note / x close tab / : palette`（メモの読み取り表示中は末尾が `Esc close note` に変わる）、没入は `Ctrl-O: switch / Ctrl-T: focus / Ctrl-^: last / Ctrl-N/P: tab / Ctrl-G: agent / Ctrl-E: note` を案内 | 淡色（dim） |
...
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-^`・`Ctrl-G`・`Ctrl-E`・`Ctrl-Q`**で、`Esc` や `Ctrl-W`（シェルの「直前の単語を削除」）を含む他のキーはすべてシェルへ流れます
...
**没入のまま新しい agent タブを追加**して、そのタブをアクティブにします。`Ctrl-E` は**没入のまま[メモ編集](#セッションメモの編集)**を開き、閉じると同じペインへ復帰します。`Ctrl-W` は**奪わず**シェルの「直前の単語を削除」としてそのまま流すため、タブを閉じるのは[切替](#切替switch既定)（`Ctrl-O` で抜けて `x`）で行います。`Ctrl-B` は**没入のまま[サイドバーを開閉](#サイドバーの開閉ctrl-b)**し、レールに畳むとそのぶん埋め込みターミナルが広がります。`Ctrl-Q` は**没入を抜けてアプリ終了の[確認モーダル](#終了確認モーダル)を出す**グローバル終了キーです（全ペインは生かしたまま。確定するまで終了しないので動作中の Agent を巻き込みません）。これらの予約キーの分だけシェル / Agent 側の同キーは奪われます。
...
| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-G`・`Ctrl-E`・`Ctrl-Q`・選択中の `Ctrl-C` 以外。`Esc` や `Ctrl-W` を含む） | そのままシェルへ転送（通常のターミナルとして操作。`Ctrl-W` は「直前の単語を削除」） |
...
> 閉じるは切替の `x`（没入からは `Ctrl-O` で抜けてから）で行えます（[切替](#切替switch既定)で `←`/`→`・`Ctrl-N`/`Ctrl-P` がタブ、`↑`/`↓` がセッション）。
