...
切替 --Enter/l--> ライブ? 没入 : 在席 ;  --c→インライン名入力→Enter--> 在席(新規) ;  --r→インライン表示名入力→Enter--> 表示名を変更 ;  --Esc/h--> 開いた元のモード ;  --Ctrl-O--> 統括
在席 --terminal/agent--> 没入 ;  --Esc--> 統括 ;  --Ctrl-O--> 切替
没入 --Ctrl-O Ctrl-O--> 切替 ;  --Ctrl-O ]/[/数字--> タブ切替（没入のまま） ;  --Ctrl-O t/a--> ペイン追加（没入のまま）
没入 --Ctrl-O w / シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入では `Ctrl-O` は**リーダーキー**で、続く 1 キーで動作が決まります（タブ操作 or ズームアウト）。詳細は
> [没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)。
...
- **切替（Switch）** — セッションを選ぶ／作るためのピッカー。**キーボードは左ペイン**に移り（中央モーダルや
  ...
  **入力中に逐次バリデーション**され、使えない名前(空文字・パス区切り・既存ブランチとの重複や名前空間の衝突)は
  ...
  `Esc`/`h` で**開いた元のモード**へ戻り、`Ctrl-O` で**統括**へズームアウトします。
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  （`[agent] [terminal] …`）が出て、1 セッションの中に複数ペイン（agent と並行して terminal など）を持てます。**`Ctrl-O`
  だけが予約キー**で（`Esc` を含む他のキーはすべてシェルへ流れます）、`Ctrl-O` はリーダーとして働き、続く 1 キーで動作が
  決まります：`Ctrl-O Ctrl-O` で**切替**へズームアウト、`Ctrl-O ]`/`[`/数字でタブ切替、`Ctrl-O t`/`a` でペイン追加、
  `Ctrl-O w` でペインを閉じる。シェル終了やペインを閉じたとき、他のペインが残っていれば没入のまま、最後の 1 枚なら
  **在席**へ戻ります。切替へズームアウトしてもセッションの全ペインは生かしたままで、再アタッチすると最後に居たペインに戻ります。
...
│ ↑↓ move  Enter/l select  c new  r rename  Esc/h back  Ctrl-O home │  ← フッター（切替）
└───────────────────────────────────────────────────────────┘
```
...
上端の**タブストリップ**（`[agent] [terminal] …`、アクティブなタブを反転表示）と、その下にシェルのライブ出力
（疑似ターミナルの画面グリッド）を描画します。入力欄は `● live terminal`、フッターは
`Embedded terminal — Ctrl-O: switch session` を表示します。没入では **`Ctrl-O` だけが予約キー**（`Esc` を
含む他のキーはすべてシェルへ流れる）で、`Ctrl-O` は**リーダー**として働き、続く 1 キーで動作が決まります
（`Ctrl-O Ctrl-O` で**切替**へズームアウト、`Ctrl-O ]`/`[`/数字でタブ切替、`Ctrl-O t`/`a` でペイン追加、
`Ctrl-O w` でペインを閉じる）。シェル終了やペインを閉じたとき、他のペインが残れば没入のまま、最後の 1 枚なら
**在席**へ戻ります。
...
│    ⌂   root             │ [agent] [terminal]             │  ┐ 右ペイン上端：タブストリップ
│        workspace root   │ $ cargo test                   │  │   （アクティブタブは反転）
│  ▎ ○   feat/login local │ running 42 tests…              │  │ その下：アクティブペインのライブ出力
│  ▎   ▶ running          │ $ ▏                            │  ┘
...
切り替え専用の予約キーが必要）。`Ctrl-O` は**リーダーキー**として働き、続けて押した 1 キーで動作が決まります（タイムアウト
なしで次の 1 キーを必ず読み取る）。`Ctrl-O Ctrl-O` が従来の「切替へズームアウト」に相当し、残りはタブ（ペイン）操作に
割り当てられます。タブは「ペイン軸」、`Ctrl-O Ctrl-O` で開く[切替](#切替switch)は「セッション軸」で、2 つは直交します。
切替で別セッションを選べばライブはそのまま再アタッチ、ライブでなければ在席に着きます。

| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・選択中の `Ctrl-C` 以外。`Esc` 含む） | そのままシェルへ転送（通常のターミナルとして操作） |
...
| `Ctrl-O Ctrl-O`（または `Ctrl-O o`） | 切替へズームアウト（全ペインは生かしたまま。再アタッチで最後に居たペインに戻る） |
| `Ctrl-O ]` / `Tab` | 次のタブ（ペイン）へ |
| `Ctrl-O [` / `BackTab` | 前のタブ（ペイン）へ |
| `Ctrl-O 1`〜`9` | その番号のタブへジャンプ |
| `Ctrl-O t` / `Ctrl-O a` | terminal / agent ペインを追加してそこへ移る |
| `Ctrl-O w` | アクティブペインを閉じる（他が残れば没入継続、最後の 1 枚なら在席へ） |
| `Ctrl-O Esc`（または未割り当てのキー） | キャンセル（そのまま没入を続ける） |
| （シェル / Agent の `exit` 等） | 終了でそのペインを閉じる（他が残れば次のペイン、最後の 1 枚なら在席へ） |

> 別セッションへ切り替えても元のシェルはプールに残って動き続けるため、戻れば続きから操作できます。同じ
> セッション内でも、agent を動かしながら `Ctrl-O t` で terminal を開くなど、複数ペインをタブで行き来できます。
...
- **見た目**: 検出した URL のセルは**下線つき・水色（RGB `102,178,255`）**で描画され、クリックして開けることが
  ひと目でわかります。ドラッグで範囲選択すると、その上から反転表示が重なるため選択範囲も区別できます。
  シェル自身が付けた色や属性は URL 部分でこの表示に置き換わります。
...
> 実装は、URL 検出（クリック地点の 1 本・画面全体のリンクセル一覧）と OS ごとの起動コマンド表
> （`home/terminal_link.rs`）を純粋ロジックに分け、リンクセルの下線・水色描画を `home/terminal_view.rs`、
> マウスイベントの配線とブラウザコマンドの起動を `home/terminal_pane.rs`（`crossterm` / `std::process`）が担います。
