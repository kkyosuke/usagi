...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  管理画面ではホイール以外のレポートと同様に読み捨てます）。唯一の例外として、ホーム画面では**左クリック**だけは
  読み捨てず、当たり判定が[左下のマスコット](#レイアウト)に当たったときに反応を再生します（外れたクリック・
  ホイール・その他のレポートは従来どおり読み捨て）。
...
> イベントループ（`home/event`）だけが `Input::Click` を受け取り、マスコットの当たり矩形（`home/ui` の
> `mascot_hit_rect`＝描画と同じレイアウト計算）に当たれば反応を起動します（それ以外の画面・ホイール・外れたクリックは
> 反応しません）。代替スクリーンとマウスレポートの有効化は
