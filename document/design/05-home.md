...
切替で `n`、没入で `Ctrl-E` を押すと、対象セッション名をタイトルに置いた**中央のエディタモーダル**が開きます
（既存のメモがあれば初期値として読み込まれます）。`state.json` の[`SessionRecord.note`](../data/02-workspace.md#セッションごとsessionrecord)に保存され、
ブランチ名・識別子には影響しません。
...
- 状態は `home/state.rs`（`NoteEditor`）、描画は `home/ui/`、キー処理は `home/event/` に実装します。
