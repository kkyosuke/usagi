...
（カーソル行の status・Agent 状態をヘッダーに添え、ライブでないセッションは在席のアクションメニュー、ライブのセッションは**タブストリップ＋実際のターミナル画面**を表示。**右ペイン最下部には操作キーのヒント行を常に表示**）。
...
│    ⌂   root             │  1 agent   2 terminal           │  ┐ 左：キーボードがここにある
│        workspace root   │  ▔▔▔▔▔▔▔                         │  │   （カーソル以外は淡色）
│    ●   main      pushed │ SH$ echo hi                     │  │ 右：タブストリップ＋選択後のプレビュー
...
上端の**タブストリップ**（ペインごとの番号付きチップ＋アクティブタブの下線、2 行）と、その下にシェルのライブ出力
...
│    ⌂   root             │  1 agent   2 terminal          │  ┐ 右ペイン上端：タブストリップ（2 行）
│        workspace root   │            ▔▔▔▔▔▔▔▔▔▔            │  │   （アクティブタブを下線で強調）
