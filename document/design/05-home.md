...
切替・在席で**うさぎ本体を左クリックすると、約 0.66 秒だけ「可愛い反応」を 1 回再生**します。反応は 3 種類
（ぴょんと跳ねる `(>w<)`・きらきら喜ぶ `(^v^)` ＋ `*`・びっくりから照れる `(O.O)`→`(^/^)`）からクリックごとに
**ランダムに選ばれ**、連打すると変わります。反応中はうさぎの**足元の位置・占有幅は変わらない**ので、サイドバーの
レイアウトはぶれません（クリック判定の当たり矩形も動きません）。当たり判定は**うさぎ本体の 3 行**（更新通知の
吹き出しが出ているときは、その吹き出しではなく本体の行）だけで、頭上の吹き出しや本体以外をクリックしても何も
起きません。モーダル・コマンドパレット・プレビュー・メモ編集が開いているとき、および没入中は反応しません。
レールのちびうさぎはクリックしても反応しません。
反応の再生にもマスコットアニメーション設定が必要で、OFF のときはクリックしても動きません。
