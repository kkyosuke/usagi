...
左ペインの各エントリ（ルート行 / 各セッション）は基本 **2 行**で表示します。1 行目は左端のガター・鮮度ドット・
...
**生きたプロセスを持つセッションだけ**、さらに**3 行目**に**リソース使用量**（[後述](#リソース使用量)の `CPU 8%  MEM 120MB`）を足します（プロセスのないセッションは 2 行のまま）。
...
`CPU 23%  MEM 512MB` の形で淡色表示します（[リソース使用量](#リソース使用量)の合計）。生きたセッションが
...
│  CPU 8%  MEM 120MB       │                                │  │   3 行目：リソース使用量（live なセッションだけ）
...
**生きたプロセスを持つセッションだけ**は、2 行目の下にさらに **3 行目**として[リソース使用量](#リソース使用量)（`CPU 8%  MEM 120MB`）を
名前の真下にインデントして添えます（2 行目の固定桁クラスタは変えず、増えてきた情報はこの行が引き受けます）。
...
- **狭いとき**: 右端のまとまりの優先順は **差分バッジ > リソース使用量 > ahead/behind マーカー > 更新時刻**。Agent 状態の表示が削られるほど狭い幅では、更新時刻を**先に省き**、次いで ahead/behind、最後にリソース使用量を省きます（バッジと Agent 状態を優先）。
...
- **狭いとき**: 差分バッジ・リソース使用量・Agent 状態が優先で、収まらなければ（更新時刻の次に）省かれます。
- **レール（畳んだサイドバー）には出しません**。

#### リソース使用量

セッションの **3 行目**（2 行目の Agent 行の下、名前の真下にインデント）に、そのセッションで動いているプロセス群が
使っている **CPU とメモリ**を `CPU 8%  MEM 120MB` の形で淡色表示します。どのセッションが実際に CPU・メモリを食っているかを
...
- **表記**: CPU は CPU 1 コアぶんを 100% とした整数値（マルチコアでは合算で 100% を超えることがあり、`top` と同じ読み方）、メモリは 1 GiB 未満は整数 MB（`120MB`）、以上は小数 1 桁の GB（`1.2GB`）。
- **出る条件**: そのセッションに**生きたプロセスがあるときだけ**。プロセスが無い行（過去に終了した・まだ起動していない）には出しません。
...
- **合計**: 全セッションぶんを合算した**ワークスペース合計**を、左ペイン最下部の[マスコット](#レイアウト)の顔の右隣に `CPU 23%  MEM 512MB` で表示します。
- **レール（畳んだサイドバー）には出しません**（幅 5 桁に収まらないため）。
...
| セッション一覧（左ペイン） | 先頭にどのセッションにも属さない**ルート行**（`⌂ root`）を常設し、続けて各セッションを **2 行**で表示（複数 git を含むワークスペースでも 1 セッション = 1 行にまとめる）。1 行目は「左端ガター + 鮮度ドット + セッション名(幅揃え) + status(右端)」、2 行目は名前の下にインデントして「左端ガター + Agent アイコン + Agent 状態テキスト」。左端ガターは**アクティブ**（操作対象）を**緑 `▎` バー**で 2 行にわたり示し、**切替（Switch）中のみ**選択行に**赤 `>` カーソル**を 1 行目に出す。名前は左ペイン幅に収まるよう末尾を省略。**切替中はカーソル行以外を淡色**に落として選択中を際立たせる | カーソル（切替のみ）：`>` 赤太字／アクティブ：`▎` 緑太字（2 行）／鮮度：`●`（15 分以内, 緑）/`◐`（4 時間以内）/`○`（4 時間超, 淡色）、ルート行は `⌂`（マゼンタ）／Agent：ready `☾` 淡色・running `▶` 緑太字・waiting `◆` 黄色太字・done `✓` シアン太字／名前 シアン（選択・アクティブ行は太字）／切替の非カーソル行は淡色 |
