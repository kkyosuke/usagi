...
なお**切替（Switch）はセッションピッカー**で左ペインそのものが選択 UI のため、`sidebar` 設定・トグル状態にかかわらず**常にフル幅**で描画します（インラインの新規作成 `c` / 表示名編集 `r` にも幅が要るため）。没入の埋め込みターミナルはレールに合わせて即リサイズされるので、畳むとそのぶん広く使えます。
