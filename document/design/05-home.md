...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  管理画面ではホイール以外のレポートは読み捨てます。ただし**切替の左ペインでの左ボタン押下だけは例外**で、
  セッション行の選択／確定に使います（[モードとキー操作](#モードとキー操作)））。
...
> 実装は、管理画面の入力を読む `term_reader.rs` がマウスレポート（SGR/X10）を解析し、ホイールはスクロールイベントへ、
> 左ボタン押下はクリックイベントへ振り分け、ドラッグ・離上・他ボタンは読み捨てます。各画面へはキー・スクロール・クリックを
> `Input` として渡し、ホームのイベントループだけが切替の左ペインのクリックをセッション行の選択／確定（座標→行の対応は
> `home/ui` の `left_pane_session_at`、ダブルクリック判定は `home/event` の `switch_click`）に使います。代替スクリーンとマウスレポートの有効化は
