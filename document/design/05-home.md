...
ホーム画面を閉じるのは `Ctrl+C` または `Ctrl+Q` で、いずれも[終了確認モーダル](#終了確認モーダル)を挟みます（`Ctrl+Q` は没入中でも効き必ず確認します）。worktree の情報はワークスペースの
...
`Ctrl-O` =「切替へズームアウト」、`Esc` =「一段戻る」（基底の切替では戻り先がないため `Esc` は無効。終了は `Ctrl+C` / `Ctrl+Q`）。
...
> タブを追加）・`Ctrl-^`（[直前のセッションへ切替](#直前のセッションへ切り替えctrl-)）・`Ctrl-E`（[メモ編集](#セッションメモの編集)）・`Ctrl-W`（アクティブタブを閉じる）・`Ctrl-B`（[サイドバー開閉](#サイドバーの開閉ctrl-b)）・`Ctrl-Q`（アプリ終了の[確認モーダル](#終了確認モーダル)）です。没入ではキーがシェルへ流れるため、`:` のコマンドパレットは開けません（まず `Ctrl-O` で切替へ抜けます）。タブの切替はどのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`・`Ctrl-G`・`Ctrl-E`・`Ctrl-W`・`Ctrl-B`・`Ctrl-Q`**で（`Esc` を含む他のキーは
  ...
  切り替え**、`Ctrl-G` で**agent タブへ移動**（無ければ追加し、すでにあればその 1 つへ切替＝agent は 1 セッション 1 つ）、`Ctrl-E` で**[メモ編集](#セッションメモの編集)**（閉じると同じペインへ復帰）、`Ctrl-W` で**アクティブタブを閉じ**、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**し、`Ctrl-Q` で**アプリ終了の[確認モーダル](#終了確認モーダル)**を出します
...
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-^`・`Ctrl-G`・`Ctrl-E`・`Ctrl-W`・`Ctrl-Q`**で、`Esc` を含む他のキーはすべてシェルへ流れます
...
（他ペインが残れば次のタブへ、最後の 1 枚なら在席へ）。`Ctrl-B` は**没入のまま[サイドバーを開閉](#サイドバーの開閉ctrl-b)**し、レールに畳むとそのぶん埋め込みターミナルが広がります。`Ctrl-Q` は**没入を抜けてアプリ終了の[確認モーダル](#終了確認モーダル)を出す**グローバル終了キーです（全ペインは生かしたまま。確定するまで終了しないので動作中の Agent を巻き込みません）。これらの予約キーの分だけシェル / Agent 側の同キーは奪われます。
...
| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-G`・`Ctrl-E`・`Ctrl-W`・`Ctrl-Q`・選択中の `Ctrl-C` 以外。`Esc` 含む） | そのままシェルへ転送（通常のターミナルとして操作） |
...
| `Ctrl-Q` | 没入を抜けてアプリ終了の[確認モーダル](#終了確認モーダル)を出す（全ペインは生かしたまま。確定するまで終了しない。同上） |
