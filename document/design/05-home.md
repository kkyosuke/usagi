...
切替で `n`、没入で `Ctrl-E` を押すと、メモの**エディタが右ペインに開きます**（既存のメモがあれば初期値として
読み込まれます）。**全画面モーダルではなく**、サイドバーやタイトル・モードラダー・フッターはそのまま残る
**その場編集**なので、画面が切り替わりません（[切替の読み取り表示](#切替での読み取り表示次回-todo-の確認)と同じ見え方のまま編集に入れます）。
...
> ● login   local │ ・API は未着手
  ☾ ready         │ ・PR #321 でレビュー待ち█
▎ ○ feat    new   │
 [note]  Ctrl-S: save / Esc: cancel / Enter: newline / ←→↑↓: move
```

- 右ペイン上部に `note: <セッション名>` ヘッダー、その下に複数行のメモ本文。編集中の行にはブロックキャレットが乗り、メモがペインより縦に長いときはキャレット行が見えるようスクロールします。操作キーはフッターに出ます。
...
切替の**右ペイン上部にメモのブロック**（`note` 枠）が表示され、選択中セッションの「次回 TODO」がすぐ目に入ります。
プレビュー（ライブ端末の再アタッチ像 / 在席アクションメニュー）の上に乗り、行数は[最大 6 行に丸めて](../06-conventions.md#可読性)
`… (N more)` で省略します（全文は `n` でエディタを開いて確認・編集）。メモが無いセッションでは何も出ません。
