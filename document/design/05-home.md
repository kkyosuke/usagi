...
タブ切替・別セッションへの没入など）は一切止まりません**。実行中・完了直後のタスクは**タイトルバー（最上行）の右側**に
**1 行のタスクステータス行**として重ねて表示します。

```text
┌────────────────────────────────────────────────────────────────┐
│        usagi · 4 sessions      ⠹ 作成中… wip [==>     ] 1/3     │  ← タイトルバー右にタスクステータス行
│        Overview › Switch › Focus › Attached                    │
...
- 表示は**タイトルバー行の右側**に重ねる 1 行です。タイトルは中央寄せで右側が空くため、右ペイン（切替のプレビュー・
  在席のメニュー／プロンプト・没入のライブ端末）の**領域外**に置けて衝突しません（従来は本文行に枠付きパネルで重ねており、
  右ペインに内容がある行は描画を見送られて**一部しか表示されない**問題がありました）。
- 1 行の構成は **マーク ＋ 代表タスクのラベル ＋ 進捗バー ＋ `done/total`**。代表タスクは「最初に**実行中**のもの、
  無ければ最後に**完了**したもの」を選び、バッチが終わると最後の結果に落ち着きます。
...
- 各フィールドは**固定幅**で、長いセッション名は省略（`…`）します。文言やスピナーが変わっても行の幅・右寄せ位置はガタつきません。
  タイトルが長い／端末が狭くて収まらないときは、（更新通知と同様）はみ出さないよう描画を見送ります。
...
- 右上の優先順位は **ローディングうさぎ（後述の同期処理）＞ タスクステータス行 ＞ 更新通知**。タスクステータス行は
  タイトルバー行に出るため、**右ペインにライブの埋め込み端末がある間（没入のアタッチ中・切替のライブプレビュー中）でも表示します**
...
タスクステータス行と同じ右上を共有しますが、**この同期処理中はローディングうさぎを最優先**で見せ、終わると
タスクステータス行（あれば）→更新通知（あれば）に戻ります。
...
- `Enter` で選択したセッションを**一括削除**。各セッションは[バックグラウンドタスク](#バックグラウンドタスクセッション作成削除)として
  ワーカースレッドへ投入され（モーダルは即座に閉じ、画面は止まらない）、右上のタスクステータス行に集約されて順に処理される。
