...
3 行目には**リソース使用量**（[後述](#リソース使用量)の `CPU 8%  MEM 120MB`）を**常に**出します。計測値の無い（生きたプロセスを持たない）セッションも行を畳まず `CPU 0%  MEM 0MB` と表示するので、セッションがライブになったり止まったりしても**一覧の縦位置はずれません**。
...
`CPU 23%  MEM 512MB` の形で淡色表示します（[リソース使用量](#リソース使用量)の合計）。CPU 値は**固定幅で揃える**ので、
桁数が変わっても（`8%` → `100%`）MEM の位置や合計表示全体がずれません。生きたセッションが
...
│  CPU 8%   MEM 120MB      │                                │  │   3 行目：リソース使用量（全セッション。計測値が無ければ CPU 0%  MEM 0MB）
│    ○   feat/login  local │  （カーソル行のプレビュー）    │  │   左端ガター：アクティブ ▎（緑、行数ぶん）／切替のカーソル >
│         +8 -2            │                                │  │   右：選択後に開く画面のプレビュー（Agent 無しでもバッジは出る）
│  CPU 0%   MEM 0MB        │                                │  ┘   計測値が無いセッションも行を畳まず 0 表示（縦位置が固定）
...
各セッションは 2 行目の下に**常に 3 行目**として[リソース使用量](#リソース使用量)（`CPU 8%  MEM 120MB`）を
名前の真下にインデントして添えます（2 行目の固定桁クラスタは変えず、増えてきた情報はこの行が引き受けます）。計測値が無いセッションも行を畳まず `CPU 0%  MEM 0MB` と表示します。
...
- **表記**: CPU は CPU 1 コアぶんを 100% とした整数値（マルチコアでは合算で 100% を超えることがあり、`top` と同じ読み方）、メモリは 1 GiB 未満は整数 MB（`120MB`）、以上は小数 1 桁の GB（`1.2GB`）。CPU 値は**固定幅で右に詰めて揃える**ので、桁数が変わっても（`0%` → `100%`）後続の MEM が同じ桁に並び、表示がガタつきません。
- **出る条件**: **全セッションで常に**出します。計測値が無い（生きたプロセスを持たない・まだ計測前の）セッションも、行を畳まず `CPU 0%  MEM 0MB` と表示します。これによりセッションがライブになったり止まったりしても一覧の縦位置がずれません。
