...
没入 --Ctrl-N/Ctrl-P--> タブ切替（没入のままアクティブペインを前後へ） ;  --Ctrl-O--> 切替（全ペインは生かしたまま。ペイン追加・閉じるは切替で行う） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入の予約キーは `Ctrl-O`（切替へズームアウト）と `Ctrl-N`/`Ctrl-P`（タブを前後へ切替）です。タブの切替は
> どのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で行えます（切替・在席では `←`/`→`・`h`/`l` も同じ）。
> 新規ペインの追加（`t`）は切替で行います。詳細は [没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)。
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは `Ctrl-O` と `Ctrl-N`/`Ctrl-P`**で（`Esc` を含む他のキーはすべてシェルへ流れます）、
  `Ctrl-O` の単押しで**切替**へズームアウト、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ切り替え**ます
  （`Ctrl-N`/`Ctrl-P` はシェル / Agent 側のキー操作からは奪われる点に注意）。**ペインの追加は切替で行います**。
...
`[attached] Ctrl-O: switch session / Ctrl-N/P: tab / …` を表示します。没入の**予約キーは `Ctrl-O` と
`Ctrl-N`/`Ctrl-P`**（`Esc` を含む他のキーはすべてシェルへ流れる）で、`Ctrl-O` の**単押し**で**切替**へズームアウト、
`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ切り替え**ます（その分シェル / Agent 側の `Ctrl-N`/`Ctrl-P` は
奪われます）。**ペインの追加・閉じるは切替で行います**。シェルが終了したとき、他のペインが残れば没入の
...
│ [attached] Ctrl-O: switch session  ^N^P tab  …           │  ← フッター
└──────────────────────────────────────────────────────────┘
```
...
没入の**予約キーは `Ctrl-O` と `Ctrl-N`/`Ctrl-P`**で、`Esc` を含む他のキーはすべてシェルへ流れます（フォーカス中の衝突を
避けるため切り替え専用の予約キーが必要）。`Ctrl-O` は**単押し**で[切替](#切替switch)へズームアウトします（リーダーキー方式は廃止）。
`Ctrl-N`/`Ctrl-P` は**没入のままアクティブタブ（ペイン）を前後へ切り替え**ます（その分シェル / Agent 側の `Ctrl-N`/`Ctrl-P` は
奪われます）。ペインの追加・閉じるは没入の中ではなく**切替で行います**——切替で `←`/`→`（または `Ctrl-N`/`Ctrl-P`）が
アクティブタブを「ペイン軸」で動かし、`↑`/`↓` がセッションを「セッション軸」で動かす二軸操作にまとまります。切替で
別セッションを選べばライブはそのまま再アタッチ、ライブでなければ在席に着きます。

| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・`Ctrl-N`/`Ctrl-P`・選択中の `Ctrl-C` 以外。`Esc` 含む） | そのままシェルへ転送（通常のターミナルとして操作） |
...
> 切替で行います（[切替](#切替switch)で `←`/`→`・`Ctrl-N`/`Ctrl-P` がタブ、`↑`/`↓` がセッション、`t` が新規ペインの起点）。
> 別セッションへ切り替えても元のシェルはプールに残って動き続けるため、戻れば続きから操作できます。同じ
> セッション内でも、agent を動かしながら切替の `t` で terminal を追加するなど、複数ペインをタブで行き来できます。
