...
最下部まで伸びてうさぎの行に届くときや、サイドバーをレールに畳んで幅が足りないときは、一覧を隠さないよう
**うさぎのほうが引っ込みます**。
...
| マスコット（左ペイン最下部） | セッション一覧の下に余白があるとき、**うさぎ**を 3 行で表示。表情と仕草は**現在のモード**に追従し、切替 `(o.o)?`・在席 `(^.^)/`・没入 `(>.<)9`。一覧が最下部まで届くとき、またはサイドバーをレールに畳んで幅が足りないときは表示しない（一覧を隠さない） | 切替：マゼンタ・太字／在席：シアン・太字／没入：緑・太字 |
