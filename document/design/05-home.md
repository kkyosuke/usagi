...
ターミナルに切り替わります（左ペインの一覧は表示したまま）。埋め込みターミナル（`terminal` / `agent`）は
`Ctrl-O` でデタッチしても**バックグラウンドで動き続け**、`claude` などの Agent が入力待ちになる（ベルを鳴らす）と
左ペインの該当 worktree に `◆`（黄色）マーカーが付き、デスクトップ通知が出ます。worktree の情報はワークスペースの
`<workspace>/.usagi/state.json`（画面外で `usagi` が同期した内容）から読み込みます。起動画面と同じ代替スクリーン上に描画されます。
...
| 入力待ちマーカー | バックグラウンドの埋め込みターミナルで Agent がベルを鳴らし入力待ちになった worktree に付く `◆` | 黄色・太字 |
...
| フッター | モード別・状態別の操作ヘルプ（ターミナル実行中は `Ctrl-O: detach (keeps running in background)` を案内） | 淡色（dim） |
...
| `Ctrl+O` | デタッチしてコマンドモードへ戻る（**シェル / Agent はバックグラウンドで動作継続**） |
| （シェル / Agent の `exit` 等） | 終了で自動的に右ペインがコマンド履歴/出力へ戻り、そのセッションは破棄される |
...
埋め込みターミナルは worktree ごとに 1 つ、ワークスペース画面が開いている間ずっと生き続けます
（`infrastructure/terminal_manager.rs` の `TerminalManager` が PTY を保持）。`Ctrl-O` でデタッチしても
シェルや `claude` は終了せず、同じ worktree で再度 `terminal` / `agent` を実行すると**動作中のセッションに
再アタッチ**します（再起動しません）。

`claude` などの Agent は処理を終えてユーザーの入力を待つときにターミナルベル（`^G`）を鳴らします。
バックグラウンドの監視スレッドが各セッションのベル回数を定期的に（約 200ms 間隔で）監視し、**アタッチ中でない**
セッションが新たに入力待ちになったら、
...
該当 worktree に再アタッチするとマーカーは消えます。検知はベルに依存するため、`claude` 側でターミナルベル通知を
有効にしておく必要があります（鳴らさない CLI では入力待ちマーカー・通知は出ません）。

- 入力待ちの判定ロジック（ベル基準値・入力待ち集合・アタッチ中セッションの扱い）は純粋な
  `infrastructure/session_monitor.rs`（`SessionMonitor`）に切り出してテスト済み。PTY 保持・監視スレッド・
  通知の I/O は `infrastructure/terminal_manager.rs`（端末 I/O・スレッドのためカバレッジ計測対象外）。
...
| `agent` | `terminal` と同じ埋め込みシェルを開き、設定中の Agent CLI（既定 `claude`）を自動入力して起動。実質 `terminal` → `claude` | ✅ 実装済み |
...
スナップショットは純粋な `home/terminal_view.rs`、疑似ターミナル本体は `infrastructure/pty.rs`）。`agent` は
`Effect::OpenAgent` を返し、`terminal` と同じ経路で埋め込みシェルを開いたうえで、`home/mod.rs` が実効設定
（`AgentCli::command()`）から解決した Agent CLI（既定 `claude`）をシェル起動直後に自動入力します。
