...
2 行目の**右端（差分バッジの右隣）**に、そのセッションに紐づく **Pull Request の番号**を `#N`（青・下線、リンクであることを示す）で表示します。セッションが**複数の PR を持つときは `#412 #98` のように全部**並べます。`#N` を**クリックするとその番号の PR だけ**を既定のブラウザで開きます（`#412` と `#98` が並んでいれば、クリックした側だけが開く）。
...
- **クリックで開く**: 没入（Attached）中にフル幅サイドバーで**`#N` バッジそのものをクリック**すると、その 1 件の PR の URL を既定ブラウザで開きます（エージェント返信内の URL クリックと同じブラウザ起動経路を共有）。バッジ以外（行の余白・Agent ラベル側・バッジ間の空白）をクリックしても PR は開かず、行の**ダブルクリック**はそのセッションへの切り替え（[没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)）に割り当てられています。サイドバーがレールのとき・没入以外のモードでは、表示はしてもクリックでは開きません（クリックを処理するのは没入のターミナル面だけのため）。
...
| マウス左クリック（ドラッグなし） | ポインタ上の `http(s)` リンク、またはサイドバーの `#N` PR バッジ（[PR バッジ](#pr-バッジ)）があれば既定のブラウザで開く（[没入のリンクを開く](#没入のリンクを開くマウス)）。サイドバーのセッション行を**ダブルクリック**するとそのセッションへ切り替える（ライブなら再アタッチ。切替のダブルクリックと同じ確定）。タブチップ・リンク・バッジ・行以外をクリックしても何も起きない |
