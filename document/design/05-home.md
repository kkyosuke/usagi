...
| マウス移動（ホバー） | ポインタの下にあるものに合わせてマウスポインタの形を変える — 選択できるグリッド上は I ビーム、リンク／タブチップ／PR バッジ行はハンド、それ以外は既定（[没入のマウスポインタ形状](#没入のマウスポインタ形状マウス)。OSC 22 対応端末のみ） |
...
没入中の右ペインでは、ポインタの下にあるものに合わせて**ホスト端末のマウスポインタの形**を切り替え、
「いまここで何ができるか」を見た目で示します。ポインタを動かすたびに次の判定を行い、形が変わったときだけ
端末へエスケープを書きます（同じ形のあいだは送り直しません）。

- **テキストの I ビーム**: 選択（[ドラッグでコピー](#没入のテキスト選択とコピーマウス)）できる埋め込み端末のグリッド上。
- **指差し（ハンド）**: クリックで反応する対象の上 — 端末出力の [`http(s)` リンク](#没入のリンクを開くマウス)・
  上端の**タブチップ**（[切り替えられるタブ](#没入attached)。アクティブタブは切り替え先が無いので対象外）・
  サイドバーの **PR バッジ（`#N`）を持つセッション行**（[PR バッジ](#pr-バッジ)）。
- **既定のポインタ**: 上記以外（タブストリップの余白や、PR を持たないサイドバー行など）。

ポインタ形状は **OSC 22**（`ESC ] 22 ; <名前> ST`。`text` / `pointer`、リセットは空名）で指定します。
没入を抜けるとき（切替・在席へのズームアウトやアプリ終了）に**既定へ戻す**ので、ホーム画面や終了後の
ユーザー自身のシェルへ形が残りません。OSC 22 に対応する端末（iTerm2・Ghostty・WezTerm・kitty・foot・xterm など）で
効き、**非対応の端末はエスケープを黙って捨てます**（ホイール／ドラッグ／OSC 52 と同じく、対応端末でのみ働く上乗せ機能）。

> 実装は、ポインタ形状の判定（グリッド内か・クリック対象か → I ビーム／ハンド／既定）と OSC 22 文字列を
> 純粋ロジック（`home/pane_input.rs` の `pointer_shape` / `PointerShape`）に分け、ヒットテスト（リンク・タブ・
> PR バッジ）と形が変わったときの書き出しを `home/terminal/pane.rs` が担います。
