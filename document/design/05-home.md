...
内側へ深く入り込む「**関与のはしご（engagement ladder）**」になっており、`Ctrl-O` は**切替へズームアウト**、
`Esc` は**一段戻る**で統一されています（ただし基底の**切替**では戻り先がないため `Esc` は無効）。
...
深く入り込みます。`Ctrl-O` は**切替へズームアウト**（没入・在席 → 切替）、`Esc` は**一段戻る**で統一されています
（基底の**切替**では戻り先がないため `Esc` は無効。終了は `Ctrl+C` のみ）。ワークスペース全体のコマンドは
...
`Ctrl-O` =「切替へズームアウト」、`Esc` =「一段戻る」（基底の切替では戻り先がないため `Esc` は無効。終了は `Ctrl+C`）。
`:` はどのモードからも（没入を除く）[コマンドパレット](#コマンドパレット統括overview)を開き、`Esc` で閉じて元のモードへ戻ります。

```text
起動 → 切替（Switch）         （既定。Esc 無効。Ctrl+C で終了＝ライブ時は確認モーダル）
切替/在席 --`:`--> コマンドパレット（Esc で閉じて元のモードへ戻る。モード遷移コマンドは閉じてから遷移）
切替 --↑↓/jk--> セッション移動 ;  --←→/hl または Ctrl-N/Ctrl-P--> タブ切替（選択中セッションのアクティブタブを実切替） ;  --Enter--> ライブ? 没入 : 在席 ;  --t--> 在席（新規ペイン作成の起点） ;  --x--> 選択中セッションのアクティブタブ（ペイン）を閉じる ;  --c→インライン名入力→Enter--> 在席(新規) ;  --r→インライン表示名入力→Enter--> 表示名を変更 ;  --n または Ctrl-E→メモ編集→Ctrl-S--> メモを保存 ;  --Ctrl-^--> 直前のセッション ;  --Esc--> 無効（基底のため戻り先なし）
...
> タブを追加）・`Ctrl-^`（[直前のセッションへ切替](#直前のセッションへ切り替えctrl-)）・`Ctrl-E`（[メモ編集](#セッションメモの編集)）・`Ctrl-W`（アクティブタブを閉じる）・`Ctrl-B`（[サイドバー開閉](#サイドバーの開閉ctrl-b)）です。没入ではキーがシェルへ流れるため、`:` のコマンドパレットは開けません（まず `Ctrl-O` で切替へ抜けます）。タブの切替はどのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で
...
- **切替（Switch）** — ワークスペースを開いた直後の既定モード。セッションを選ぶ／作る／タブを切り替えるためのピッカー。**キーボードは左ペイン**に移り（中央モーダルや
  ...
  **入力中に逐次バリデーション**され、使えない名前(空文字・パス区切り・既存ブランチとの重複や名前空間の衝突)は
  ...
  切替は関与のはしごの**基底**なので `Esc` は無効です（戻り先がない。終了は `Ctrl+C`）。`:` で[コマンドパレット](#コマンドパレット統括overview)を開いてワークスペース全体のコマンドを実行できます。
- **在席（Focus）** — 1 つのセッションを選んだ状態で、そのセッションで実行できるコマンドを**右ペイン**で操作します
  ...
  `terminal` / `agent` を実行すると**新しいペインを追加して没入**へ。ただし**1 セッションが持てる agent は 1 つだけ**で、すでに agent ペインがあるときに `agent` を実行すると新規追加せず**その agent タブへ移動**します（terminal は何枚でも追加できる）。`close` で在席中のセッションを強制削除し、次のセッションを選べるよう**切替**へ移ります。`close` はセッションに属さない**ルート行（`⌂ root`）では出しません**（ワークスペース自体は閉じられないため）。アクション UI／切替プレビューに並ぶコマンドは**その行に対して実行できるものだけ**で、切替プレビューはカーソル行（選んだら開く行）を基準に並べます（アクティブ行ではなく）。
...
画面中央に**オーバーレイ（モーダル）**として重なり、ワークスペース全体のコマンドを 1 か所から実行します。
...
│  > ●   main       pushed │  $ echo hi                     │  │   1 行目：種別アイコン + 名前 + status（左端にガター）
│      ▶ running           │  hi                            │  │   2 行目：Agent アイコン + 状態
│    ○   feat/login  local │  （カーソル行のプレビュー）    │  │   左端ガター：アクティブ ▎（緑、2 行）／切替のカーソル >
│      ◆ waiting           │                                │  ┘ 右：選択後に開く画面のプレビュー
│ Pick a session                                            │  ← 下部（切替：ピッカーの見出し）
│ [switch] ↑↓ session / ←→ tab / Enter focus / : palette     │  ← フッター（モード別・淡色）
└───────────────────────────────────────────────────────────┘
```
...
`Esc` は[読み取り表示中のメモ](#切替での読み取り表示次回-todo-の確認)があればまずそれを閉じます（無ければ無効。切替は基底のため戻り先がありません。終了は `Ctrl+C`）。`:` で[コマンドパレット](#コマンドパレット統括overview)を開いてワークスペース全体のコマンドを実行できます。フッターが切替の操作ヘルプに変わります。
...
│ [switch]  ↑↓ session / ←→ tab / Enter focus / t new / x close tab / c new / r rename / : palette │  ← フッター（切替・操作キーはここだけ）
└──────────────────────────────────────────────────────────────────────────┘
```
...
**実行中ビルドより新しいバージョンが公開されていれば**ヘッダー行（タイトルバー／モードはしごの行）に
`アップデートがあるぴょん` ＋ 新バージョン（`v<X.Y.Z>`）を黄色・太字で重ねて表示します。差分が無い（最新と同じ・
取得失敗・オフライン）ときは何も表示しません。右ペインは既定で空でなくなった（切替のプレビューなどが常に入る）ため、
通知は本文行ではなくヘッダー行に出します。

```text
┌────────────────────────────────────────────────────────────────┐
│                  usagi · 4 sessions     アップデートがあるぴょん v0.2.0 │  ← ヘッダー行右側に更新通知
...
- 取得は git CLI（`git ls-remote --tags`）で行い、HTTP 依存を増やさず、ユーザーの git 認証/プロキシ設定を
  ...
  再描画で通知が現れる）。
- 通知はヘッダー行（タイトル／はしご）の右側に重ねます。タイトル・はしごは中央寄せで右側が空くため、右ペインの内容
  （切替のプレビュー・没入のライブ端末など）と衝突しません。右側に[バックグラウンドタスク](#バックグラウンドタスクセッション作成削除)の
  ステータス行が出ているときや端末幅が足りないときは、はみ出さないよう描画を見送ります。
...
| コマンドパレット（統括 / Overview） | `:` で開く中央オーバーレイ。入力欄 `❯ …`・コマンド候補ヒント・**直前に実行したコマンドのレスポンス**を表示（エコー `❯`・出力・エラー・通知）。`man` / `history` / `session list` などテキストを読むコマンドはテキストモーダルをパレットの上に重ねて開く。`Esc` で閉じて元のモードへ戻る | コマンド：シアン太字／出力：素／エラー：赤／通知：黄色／枠付き・中央寄せ |
...
| フッター | モード別・状態別の操作ヘルプ。コマンドパレットは先頭に `[palette]`、在席は `[session: <名前>]` を表示。切替は `↑↓ session / ←→ tab / Enter focus / c new / r rename / n/Ctrl-E note / x close tab / : palette`（メモの読み取り表示中は末尾が `Esc close note` に変わる）、没入は `Ctrl-O: switch / Ctrl-T: focus / Ctrl-N/P: tab / Ctrl-G: agent / Ctrl-E: note / Ctrl-W: close` を案内 | 淡色（dim） |
...
- 補完（`Tab`）・入力候補・`man` の一覧は「いまの入力面のスコープから見えるコマンド」を対象にします。コマンドパレットに
  セッション固有の `terminal` / `agent` も出ますが（パレットはワークスペース全体の入口）、在席の右ペイン（Prompt）に `session` / `config` は出ません。
...
[3 つのモード](#モードと状態遷移切替在席没入)ごとのキー操作です。`Ctrl-O` は**切替へズームアウト**、
`Esc` は**一段戻る**で統一されています。ワークスペース全体のコマンドは `:` で開く
[コマンドパレット](#コマンドパレット統括overview)から実行します。
...
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-^`・`Ctrl-G`・`Ctrl-E`・`Ctrl-W`**で、`Esc` を含む他のキーはすべてシェルへ流れます
（フォーカス中の衝突を避けるため切り替え・管理専用の予約キーが必要）。`Ctrl-O` は**単押し**で[切替](#切替switch既定)へズームアウトします
...
> 閉じるは没入の `Ctrl-W`・切替の `x` で行えます（[切替](#切替switch既定)で `←`/`→`・`Ctrl-N`/`Ctrl-P` がタブ、`↑`/`↓` がセッション）。
...
ホーム画面を閉じる手段は `Ctrl+C` のみです（`Esc` は基底の[切替](#切替switch既定)では無効）。実行中の
