...
| `session` | `session <name>`（または `session new <name>`）でセッション作成（name 省略時は名前入力モーダル）。`session list` で一覧、`session switch <name>` でアクティブセッション切り替え（引数なしで一覧、worktree 一覧の Enter でも切り替え）、`session remove <name> [--force]` で削除（未コミット変更があれば警告し `--force` で破棄） | ✅ 実装済み |
| `terminal` | 選択中の worktree（未選択時はワークスペースルート）で対話型シェルを起動。TUI を一時退避し、シェル終了後に復帰 | ✅ 実装済み |
...
`Effect::OpenTerminal` を返して event loop が選択中 worktree でシェルを起動します（端末 I/O は
`home/mod.rs` に注入した `open_terminal` コールバックが担当）。
...
- `terminal` は選択中の worktree（未選択時はワークスペースルート）で対話シェルを起動します。
  シェルは `$SHELL`（未設定なら `bash`、Windows は `cmd.exe`）で、起動時に alternate screen を抜け、
  シェル終了後に TUI へ復帰します。実体は `infrastructure/terminal.rs` と `home/mod.rs` の `open_terminal`。
