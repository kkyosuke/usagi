...
Agent の状態（アイコン＋テキスト）と、その行の右端に**更新時刻**（[後述](#更新時刻)の `Nmin ago`）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)の `↑N ↓M`）・**差分バッジ**（[後述](#差分バッジ)の `+N -M`）・**PR バッジ**（[後述](#pr-バッジ)の PR アイコン＋件数）です。
...
│▶ running +124 -18  1    │  hi                            │  │   2 行目：Agent 状態（左）＋ 更新時刻・↑N↓M・差分バッジ・PR(アイコン+件数)（右端。番号はホバーで一覧）
...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  ボタンなしの移動も報告する DECSET 1003 はサイドバーの **[PR バッジ](#pr-バッジ)のホバーポップアップ**に使い、
  それ以外のモーション／レポートは管理画面ではホイール以外と同様に読み捨てます）。例外はホーム画面の**左クリック**と
  **ホバー（ポインタ移動）**で、左クリックは当たり判定が[切替の左ペインのセッション行](#モードとキー操作)に当たればその行を選択
  （ダブルクリックで確定）、[左下のマスコット](#レイアウト)に当たれば反応を再生し、ホバーは PR を持つセッションの行で
