...
内側へ深く入り込む「**関与のはしご（engagement ladder）**」になっており、**切替へズームアウト**（在席・没入とも[キー方式](#没入のキー操作attached--terminal--agent-実行中)に従い、既定の `prefix` 方式ではリーダー `Ctrl-O` の次に `o`。在席は `Esc` でも一段で戻れる）と
...
深く入り込みます。**切替へズームアウト**（在席・没入とも[キー方式](#没入のキー操作attached--terminal--agent-実行中)に従い既定 `Ctrl-O o`。在席は `Esc` でも戻れる）と `Esc` の**一段戻る**で統一されています
...
「切替へズームアウト」（在席・没入とも[キー方式](#没入のキー操作attached--terminal--agent-実行中)に従い既定 `Ctrl-O o`。在席は `Esc` でも戻れる。`alt` 方式では在席は `Ctrl-O` 単打）と `Esc` =「一段戻る」（基底の切替では戻り先がないため `Esc` は無効。終了は切替・在席で `Ctrl+C` / `Ctrl+Q`、在席・没入は既定 `Ctrl-O q` も可）。
...
切替 --↑↓/jk--> セッション移動 ;  --左クリック--> その行を選択（ダブルクリックで確定＝Enter 相当） ;  --K/J--> 選択中セッションを上下へ移動し手動順を永続化 ;  --s--> 入力待ち（◆）を先頭へ寄せる並び替えのオン/オフ（表示順のみ） ;  --←→/hl または Ctrl-N/Ctrl-P--> タブ切替（選択中セッションのアクティブタブを実切替） ;  --Enter--> ライブ? 没入 : 在席 ;  --t--> 在席（新規ペイン作成の起点） ;  --x--> 選択中セッションのアクティブタブ（ペイン）を閉じる ;  --c→インライン名入力→Enter--> 在席(新規) ;  --r→インライン表示名入力→Enter--> 表示名を変更 ;  --n または Ctrl-E→メモ編集→Ctrl-S--> メモを保存 ;  --Ctrl-^--> 直前のセッション ;  --Esc--> 無効（基底のため戻り先なし）
在席（既定の `prefix` 方式は没入と同じリーダー `Ctrl-O` の次キーでも操作できる。直キーも併用可） --terminal/agent（「+ new」タブ）--> 没入（新規ペインを追加して入る） ;  --Ctrl-N/Ctrl-P（または Ctrl-O n/p ｜ Ctrl-O →/←）--> タブ巡回（ライブなペインのタブ＋末尾の「+ new」タブ。ペインのタブはアクティブタブを実切替しプレビューが追従） ;  --Enter（ペインのタブ）--> 没入（そのペインへ再アタッチ） ;  --agent 起動（Ctrl-O g）--> 没入（agent ペインを追加して入る） ;  --メモ編集（Ctrl-E ｜ Ctrl-O e）／サイドバー（Ctrl-O s）／終了確認（Ctrl-O q）--> 在席に留まる ;  --Ctrl-^（または Ctrl-O Ctrl-^）--> 直前のセッション ;  --Esc（没入から Ctrl-O a／Ctrl-T でズームアウトした直後）--> ズームアウト元のペインへ再アタッチ（没入。直前のタブへ戻る） ;  --Esc（「+ new」タブ・ライブなペインあり・上記以外）--> アクティブペインのタブへ戻る（破棄して 在席 に留まる） ;  --Esc（それ以外）--> 切替 ;  --Ctrl-O o（`alt` 方式は Ctrl-O 単打）／Ctrl-O Ctrl-O--> 切替
...
> 没入が予約するキーは[キー方式（`key_scheme`）](../05-settings.md#設定項目)で決まります。既定の **`prefix` 方式**はリーダー `Ctrl-O` の次キーで操作し（`Ctrl-O o`＝切替へ・`Ctrl-O a`＝在席へ など）、**`alt` 方式**は `Alt` 単打です。`Ctrl-O` 以外の bare Ctrl キー・`Esc`・`Ctrl-W` はシェルへ流れ、没入中は `:` のコマンドパレットを開けません（まず切替へズームアウトします）。**予約キーの全一覧・`alt` 方式・タブの追加/切替/クローズは [没入のキー操作](#没入のキー操作attached--terminal--agent-実行中) が正本です。**
...
- **切替（Switch）** — ワークスペースを開いた直後の既定モード。セッションを選ぶ／作る／タブを切り替えるためのピッカー。**キーボードは左ペイン**に移り（中央モーダルや
  ...
  **左ペインのセッション行をマウス左クリック**しても選べます — シングルクリックでその行を選択（カーソルを移動）し、
  ...
  （ルート行 `⌂ root` も同じ。区切り線・マスコット・空行・右ペインのクリックは無視。インライン入力やコマンドパレット・モーダルが開いているあいだはクリックを受け付けません。
...
- **在席（Focus）** — 1 つのセッションを選んだ状態で、そのセッションで実行できるコマンドを**右ペイン**で操作します
  ...
  **在席は没入と同じ[キー方式（`key_scheme`）](../05-settings.md#設定項目)のリーダー文法を共有します。** 既定の **`prefix` 方式**では `Ctrl-O` がリーダーになり、没入とまったく同じ次キーで操作できます — `Ctrl-O o`（**切替**へズームアウト。`Ctrl-O Ctrl-O` も同じ）・`Ctrl-O n`/`Ctrl-O p`（タブ前後。`Ctrl-O →`/`←` も）・`Ctrl-O g`（**agent を起動**）・`Ctrl-O e`（[メモ編集](#セッションメモの編集)）・`Ctrl-O s`（[サイドバー開閉](#サイドバーの開閉ctrl-b)）・`Ctrl-O q`（[終了確認](#終了確認モーダル)）。在席はシェルを持たないため、`Ctrl-N`/`Ctrl-P`（タブ）・`Ctrl-E`（メモ）・`Ctrl-^`（直前のセッション）・`Esc`（一段戻る）は**リーダー無しの直キーとしても併用できます**（`Esc` が**切替への 1 キー離脱**）。**`alt` 方式**では没入が `Ctrl-O` をリーダーにしないのに合わせ、在席でも `Ctrl-O` は**単打で切替へ**戻る直キーのままです。`:` で[コマンドパレット](#コマンドパレット統括overview)を開けます。
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  （ペインごとの番号付きチップ＋アクティブタブの下線、2 行）が出て、1 セッションの中に複数ペイン（agent と並行して
  terminal など）を持てます。**予約キーは[キー方式（`key_scheme`）](../05-settings.md#設定項目)で決まり**、既定の `prefix` 方式ではリーダー `Ctrl-O` の次キー（`Ctrl-O o` で切替へ・`Ctrl-O a` で在席へ など）、`alt` 方式では `Alt` 単打で操作します。予約キーの全一覧・タブの追加/切替/クローズは[没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)が正本です。
...
Agent の状態（アイコン＋テキスト）と、その行の右端に**更新時刻**（[後述](#更新時刻)の `Nmin ago`）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)の `↑N ↓M`）・**差分バッジ**（[後述](#差分バッジ)の `+N -M`）・**PR バッジ**（[後述](#pr-バッジ)の `#N`）です。
3 行目には**リソース使用量**（[後述](#リソース使用量)。CPU・メモリの**アイコン + 数値**で ` 8%   120MB` のように）を**常に**出します。計測値の無い（生きたプロセスを持たない）セッションも行を畳まず `0%` / `0MB` と表示するので、セッションがライブになったり止まったりしても**一覧の縦位置はずれません**。
...
全幅のうさぎの**顔の行の右隣**には、いま生きているセッションが使っている **CPU・メモリの合計**を
セッション行と同じ**アイコン + 数値**（` 23%   512MB` のように）で淡色表示します（[リソース使用量](#リソース使用量)の合計）。CPU 値は**固定幅で揃える**ので、
桁数が変わっても（`8%` → `100%`）メモリ値の位置や合計表示全体がずれません。生きたセッションが
...
切替・在席で**うさぎ本体を左クリック**したときの反応は、うさぎが[更新通知](#更新通知最新リリースの検知)を
喋っているかどうかで変わります。

- **更新通知を喋っているとき** — [アップデート確認モーダル](#アップデート確認モーダル)を開き、最新版を入れるか尋ねます
...
> （[ペインのスクロールとマウスホイール](#ペインのスクロールとマウスホイール)）。
...
- **在席** — 右ペインに**アクション UI**（[Menu / Prompt](#在席のアクション-uimenu--prompt)）が出て、そのセッションの `agent` / `close` / `terminal` を操作します。
...
│▶ running +124 -18 #412   │  hi                            │  │   2 行目：Agent 状態（左）＋ 更新時刻・↑N↓M・差分バッジ・PR(#N)（右端）
...
**更新時刻**（[後述](#更新時刻)）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)）・**差分バッジ**（[後述](#差分バッジ)）・**PR バッジ**（[後述](#pr-バッジ)）を
`更新時刻 ahead/behind 差分バッジ #N` の順でまとめ、右端に寄せて添えます。**この各フィールドはそれぞれ固定幅の桁に収め、
桁幅は描画ごとに「いま表示しているセッション」の中の最大値で決めます**。そのため変更行数の桁数や "ago" の長さ・PR 番号の桁が行ごとに違っても、
各フィールド（および差分の `+` / `-`、PR の `#`）は**どのセッションでも同じ桁に揃って**並び、表示がガタつきません。ルート行の 2 行目は
常に `workspace root`（淡色）です。アクティブを左端のガターバーで示すことで、2 行目の Agent アイコンと隣り合わず読み取りやすくしています。
各セッションは 2 行目の下に**常に 3 行目**として[リソース使用量](#リソース使用量)（CPU・メモリのアイコン + 数値、` 8%   120MB` のように）を
...
- **表記**: 先頭に **CPU（マイクロチップ `nf-fa-microchip`）・メモリ（メモリ `nf-fa-memory`）の Nerd Font アイコン**を置き、`CPU` / `MEM` の語は出しません（git status と同じ流儀。要 Nerd Font、未対応フォントでも数値で判読可）。CPU は CPU 1 コアぶんを 100% とした整数値（マルチコアでは合算で 100% を超えることがあり、`top` と同じ読み方）、メモリは 1 GiB 未満は整数 MB（`120MB`）、以上は小数 1 桁の GB（`1.2GB`）。CPU 値は**固定幅で右に詰めて揃える**ので、桁数が変わっても（`0%` → `100%`）後続のメモリ値が同じ桁に並び、表示がガタつきません。
...
- **合計**: 全セッションぶんを合算した**ワークスペース合計**を、左ペイン最下部の[マスコット](#レイアウト)の顔の右隣に、同じアイコン + 数値（` 23%   512MB` のように）で表示します。
- **レール（畳んだサイドバー）には出しません**（幅 5 桁に収まらないため）。
...
2 行目の**右端（差分バッジの右隣）**に、そのセッションに紐づく **Pull Request の番号**を `#N`（青・下線、リンクであることを示す）で表示します。セッションが**複数の PR を持つときは `#412 #98` のように全部**並べます。`#N` を**クリックすると既定のブラウザでその PR を開きます**。

- **取得元**: usagi は GitHub に問い合わせません。**没入中にエージェントが返信に出力した PR の URL**（`https://<host>/<owner>/<repo>/pull/<N>` 形。GitHub / GitHub Enterprise）を、ターミナル出力のリンク検出と同じ走査で拾い、`<N>` を番号として取り出します。**画面にあるものを全部**（URL 単位で重複排除して）集めるので、複数リポジトリに PR を開いたセッションはそれぞれ並びます。
...
- **即時反映（再同期を待たない）**: 没入中に新しい PR URL を検知した瞬間、`pr-links/` への蓄積に加えて**その場でサイドバーの `#N` バッジを更新**します。バッジ反映のためだけに重い再同期（worktree ごとの `git status`）を待つ必要はありません。表示する集合は `pr-links/` の蓄積済み・重複排除済みの一覧（＝次回の再同期が `state.json` に畳み込むのと同じ値）なので、即時表示とその後の再同期で食い違いません。
- **クリックで開く**: 没入（Attached）中にフル幅サイドバーの**そのセッションの行（`#N` を出している行）をクリック**すると、記録した**全 PR の URL**を既定ブラウザで開きます（エージェント返信内の URL クリックと同じブラウザ起動経路を共有）。サイドバーがレールのとき・没入以外のモードでは、表示はしてもクリックでは開きません（クリックを処理するのは没入のターミナル面だけのため）。
- **集約**: セッションが複数リポジトリに跨るときは、全 worktree の PR を**順に集めて重複 URL を除いた一覧**を表示します。
- **桁揃え**: 並んだ `#N … #M` は固定幅の桁に収め、桁幅は描画ごとに表示中セッションの最大値で決めるので、PR 番号の桁数が行ごとに違っても揃って並びます。
- **出ない条件**: PR の URL をまだ一度も観測していないセッション。ルート行（ワークスペース自体）にも出ません。
- **狭いとき**: 差分バッジと並んで**右端に最優先で残します**（更新時刻・ahead/behind より先に Agent 状態が削られても PR は残る）。
- **レール（畳んだサイドバー）には出しません**（幅 5 桁に `#N` の桁がないため）。

実装の所在は [02-architecture.md](../02-architecture.md) 経由のコード（リンク検出 `presentation/tui/home/terminal/link` の `pr_links` / `parse_pr_url`、保存 `infrastructure/pr_link_store`、型 `domain/workspace_state` の `PrLink`）で、永続化先は [data/02-workspace.md](../data/02-workspace.md) の `state.json`（worktree の `pr`）です。
...
切替は起動直後の**既定モード**です。在席・没入から `Ctrl-O o`（既定。`alt` 方式は在席 `Ctrl-O`／没入 `Alt-o`）、または[コマンドパレット](#コマンドパレット統括overview)で
...
`Ctrl-N`/`Ctrl-P` でフォーカス中セッションのタブを前後へ切り替えられます（`Ctrl-O n`/`p`・`Ctrl-O →`/`←` も同じ）。`:` で[コマンドパレット](#コマンドパレット統括overview)を開けます。

#### 在席のアクション UI（Menu / Prompt）

右ペインのアクション UI のスタイルは設定 [`session_action_ui`](../05-settings.md#設定項目)（`menu` / `prompt`、既定 `menu`）で選びます。

- **Menu** — 選べるリスト（コマンド名のアルファベット順）。`↑↓` + `Enter`、または `t`（terminal）/ `a`（agent）でアクションを起動し、**新しいペインを追加**します。`agent` 行はすでに agent ペインがあるとき（agent は 1 セッション 1 つ）リストから外れます。`close` を選ぶと在席中のセッションを削除して切替へ移ります（`--force` は付けないので未コミット変更があれば削除を拒否。セッションではないルート行では `close` は出ません）。
...
予約キーの全一覧・`prefix`/`alt` 方式・タブの追加/切替/クローズは下記[没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)が正本です。シェルが終了したとき、他のペインが残れば没入の
...
[3 つのモード](#モードと状態遷移切替在席没入)ごとのキー操作です。**切替へズームアウト**（在席・没入とも[キー方式](#没入のキー操作attached--terminal--agent-実行中)に従い既定 `Ctrl-O o`。在席は `Esc` でも戻れる）と
...
| `n` / `Ctrl-E` | カーソル中の行の[メモを編集](#セッションメモの編集)（右ペイン右上のエディタ overlay。`Ctrl-E` は在席・没入と同じ。**ルート行ではワークスペースルートのメモを編集**） |
...
| `Ctrl-E` | フォーカス中の行の[メモを編集](#セッションメモの編集)（右ペインに overlay。閉じると在席に戻る。**ルート行ではワークスペースルートのメモ**）。ただし端末は `Ctrl-E` を `End` キーとして送るため、**Prompt のコマンド入力中だけは `Ctrl-E` がキャレットを行末へ移動**し、メモは開かない（Menu・ペインプレビューでは開く） |
...
| `Ctrl-O g` / `Ctrl-O s` / `Ctrl-O q`（`prefix` 方式） | `g`＝agent を起動／`s`＝[サイドバー開閉](#サイドバーの開閉ctrl-b)／`q`＝[終了確認](#終了確認モーダル)。没入の[リーダー文法](#没入のキー操作attached--terminal--agent-実行中)と同じ次キー |
| `Ctrl+C` | アプリを終了（実行中のセッションがあれば[終了確認モーダル](#終了確認モーダル)を表示） |
| `Ctrl+Q` | アプリを終了（ライブの有無にかかわらず必ず[終了確認モーダル](#終了確認モーダル)を表示） |
...
| メモ編集（`prefix`: `Ctrl-O e` ／ `alt`: `Alt-e`） | フォーカス中の行の[メモ編集](#セッションメモの編集)を開く（閉じると同じペインへ再アタッチ。**ルート行ではワークスペースルートのメモ**） |
...
- **TUI 自体はスクロールしない**: 管理画面（プロジェクト選択・ホーム・設定など）ではホイールも
  ...
  [左下のマスコット](#レイアウト)に当たれば反応を再生します（外れたクリック・ホイール・その他のレポートは
  従来どおり読み捨て）。
