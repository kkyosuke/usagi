...
- **在席（Focus）** — 1 つのセッションを選んだ状態で、そのセッションで実行できるコマンドを**右ペイン**で操作します
  ...
  `terminal` / `agent` を実行すると**没入**へ。`close` で在席中のセッションを強制削除して**統括**へ戻ります。
...
- **Menu** — 選べるリスト。`↑↓` + `Enter`、または `t`（terminal）/ `a`（agent）でアクションを起動します。リスト末尾の `close` を選ぶと在席中のセッションを強制削除して統括へ戻ります。
