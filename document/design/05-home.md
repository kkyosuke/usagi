...
| `ai` / `terminal` / `doctor` | 認識はするが本体は未実装。「coming soon」を表示 | 🚧 プレースホルダー |
...
後続コマンド（`ai` / `terminal` など）は **`Command` を実装して `register` するだけ**で
この基盤に乗ります（現状は「coming soon」プレースホルダーとして登録済み）。`session` はこの方式で
実装済みの実例で、`new` / `list` / `switch` / `remove` のサブコマンドを持ち、name 省略時は
`Effect::OpenSessionModal` を返し、event loop が名前入力モーダルを開きます。
...
- `ai` / `terminal` / `doctor` の本体実装は今後の作業で実装します（Git 操作・疑似ターミナル・
