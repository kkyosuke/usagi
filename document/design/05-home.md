...
没入 --Ctrl-N/Ctrl-P--> タブ切替（没入のままアクティブペインを前後へ） ;  --Ctrl-T/Ctrl-G--> 新しい terminal / agent タブを追加（没入のまま） ;  --Ctrl-E--> メモ編集（閉じると同じペインへ復帰） ;  --Ctrl-W--> アクティブタブを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席） ;  --Ctrl-O--> 切替（全ペインは生かしたまま） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`・`Ctrl-B`**で（`Esc` を含む他のキーは
  すべてシェルへ流れます）、`Ctrl-O` の単押しで**切替**へズームアウト、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ
  切り替え**、`Ctrl-T`/`Ctrl-G` で**没入のまま terminal / agent タブを追加**、`Ctrl-W` で**アクティブタブを閉じ**、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**します
...
没入の**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`・`Ctrl-B`**（`Esc` を含む他のキーはすべてシェルへ流れる）で、
`Ctrl-O` の**単押し**で**切替**へズームアウト、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ切り替え**、
`Ctrl-T`/`Ctrl-G` で**没入のまま terminal / agent タブを追加**、`Ctrl-W` で**アクティブタブを閉じ**、`Ctrl-B` で**[サイドバーを開閉](#サイドバーの開閉ctrl-b)**します（その分シェル / Agent 側の
...
| フッター | モード別・状態別の操作ヘルプ。統括は先頭に `[workspace]`、在席は `[session: <名前>]` を表示。切替は `↑↓ session / ←→ tab / Enter focus / t new / x close tab / c new / r rename / Esc back`、没入は `Ctrl-O: switch / Ctrl-N/P: tab / Ctrl-T: terminal / Ctrl-G: agent / Ctrl-W: close` を案内 | 淡色（dim） |
...
没入の**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-E`・`Ctrl-W`**で、`Esc` を含む他のキーはすべてシェルへ流れます
（フォーカス中の衝突を避けるため切り替え・管理専用の予約キーが必要）。`Ctrl-E` は**没入のまま[メモ編集](#セッションメモの編集)**を開き、閉じると同じペインへ復帰します。`Ctrl-O` は**単押し**で[切替](#切替switch)へズームアウトします
