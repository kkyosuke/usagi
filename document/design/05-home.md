...
Agent の状態（アイコン＋テキスト）と、その行の右端に**更新時刻**（[後述](#更新時刻)の `N分前`）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)の `↑N ↓M`）・**差分バッジ**（[後述](#差分バッジ)の `+N -M`）・**PR バッジ**（[後述](#pr-バッジ)の `#N`）です。
...
**更新時刻**（[後述](#更新時刻)）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)）・**差分バッジ**（[後述](#差分バッジ)）・**PR バッジ**（[後述](#pr-バッジ)）を右寄せのまとまりで添えます。ルート行の 2 行目は常に `workspace root`（淡色）です。アクティブを左端のガターバーで
...
- **狭いとき**: 右端のまとまりが収まらない幅では、**差分バッジと PR バッジを最優先**で右端に残します（どちらも右端の桁に揃うので、列としても読めます）。次いで Agent 状態、ahead/behind、最後に更新時刻の順で省きます。
...
2 行目の**右端（差分バッジの右隣）**に、そのセッションに紐づく **Pull Request の番号**を `#N`（青・下線、リンクであることを示す）で表示します。`#N` を**クリックすると既定のブラウザでその PR を開きます**。

- **取得元**: usagi は GitHub に問い合わせません。**没入中にエージェントが返信に出力した PR の URL**（`https://<host>/<owner>/<repo>/pull/<N>` 形。GitHub / GitHub Enterprise）を、ターミナル出力のリンク検出と同じ走査で拾い、`<N>` を番号として取り出します。画面に複数あれば**最後（最も下＝新しい行）**を採用します。
- **永続化**: 拾った PR は worktree をキーにした保存先（`pr-links/`）に記録し、次回のセッション再同期で `state.json` の worktree の `pr` に畳み込みます。エージェントが URL を出すのは一度きりでも、**`#N` は再起動後も出続けます**。
- **クリックで開く**: 没入（Attached）中にフル幅サイドバーの**そのセッションの行（`#N` を出している 2 行）をクリック**すると、記録した URL を既定ブラウザで開きます（エージェント返信内の URL クリックと同じブラウザ起動経路を共有）。サイドバーがレールのとき・没入以外のモードでは、表示はしてもクリックでは開きません（クリックを処理するのは没入のターミナル面だけのため）。
- **集約**: セッションが複数リポジトリに跨るときは、PR を持つ**最初の worktree**の 1 件を代表として 1 つ表示します。
...
実装の所在は [02-architecture.md](../02-architecture.md) 経由のコード（リンク検出 `presentation/tui/home/terminal/link` の `pr_link` / `parse_pr_url`、保存 `infrastructure/pr_link_store`、型 `domain/workspace_state` の `PrLink`）で、永続化先は [data/02-workspace.md](../data/02-workspace.md) の `state.json`（worktree の `pr`）です。
