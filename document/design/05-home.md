...
Agent の状態（アイコン＋テキスト）と、その行の右端に**更新時刻**（[後述](#更新時刻)の `N分前`）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)の `↑N ↓M`）・**リソース使用量**（[後述](#リソース使用量)の `8% 120MB`）・**差分バッジ**（[後述](#差分バッジ)の `+N -M`）です。
...
│  ▶ running 3分前 +124 -18│  hi                            │  │   2 行目：Agent 状態（左）＋ 更新時刻・↑N↓M・リソース・差分バッジ（右端）
...
**更新時刻**（[後述](#更新時刻)）・**ahead/behind マーカー**（[後述](#aheadbehind-マーカー)）・**リソース使用量**（[後述](#リソース使用量)）・**差分バッジ**（[後述](#差分バッジ)）を右寄せのまとまりで添えます。ルート行の 2 行目は常に `workspace root`（淡色）です。アクティブを左端のガターバーで
...
2 行目の**右端（差分バッジの左隣）**に、そのセッションで動いているプロセス群が使っている **CPU とメモリ**を
`8% 120MB` の形で淡色表示します。どのセッションが実際に CPU・メモリを食っているかをひと目で見分けるための目印です。
