...
統括 --`session switch`（名前なし） / Ctrl-O--> 切替（Esc で統括へ戻る） ;  --Ctrl-^--> 直前のセッション（あれば在席／ライブなら没入）
統括 --`session switch <name>`--> 在席(name)   （未知の名前 → 入力欄直下にエラー、統括のまま）
切替 --↑↓/jk--> セッション移動 ;  --←→/hl または Ctrl-N/Ctrl-P--> タブ切替（選択中セッションのアクティブタブを実切替） ;  --Enter--> ライブ? 没入 : 在席 ;  --t--> 在席（新規ペイン作成の起点） ;  --x--> 選択中セッションのアクティブタブ（ペイン）を閉じる ;  --c→インライン名入力→Enter--> 在席(新規) ;  --r→インライン表示名入力→Enter--> 表示名を変更 ;  --n または Ctrl-E→メモ編集→Ctrl-S--> メモを保存 ;  --Ctrl-^--> 直前のセッション ;  --Esc--> 開いた元のモード ;  --Ctrl-O--> 統括
在席 --terminal/agent（「+ new」タブ）--> 没入（新規ペインを追加して入る） ;  --Ctrl-N/Ctrl-P--> タブ巡回（ライブなペインのタブ＋末尾の「+ new」タブ。ペインのタブはアクティブタブを実切替しプレビューが追従） ;  --Enter（ペインのタブ）--> 没入（そのペインへ再アタッチ） ;  --Ctrl-^--> 直前のセッション ;  --Esc（「+ new」タブ・ライブなペインあり）--> アクティブペインのタブへ戻る（破棄して 在席 に留まる） ;  --Esc（それ以外）--> 統括 ;  --Ctrl-O--> 切替
没入 --Ctrl-N/Ctrl-P--> タブ切替（没入のままアクティブペインを前後へ） ;  --Ctrl-T--> 在席の「+ new」タブへズームアウト（アクションメニューで terminal / agent などを選ぶ。全ペインは生かしたまま） ;  --Ctrl-G--> 新しい agent タブを追加（没入のまま） ;  --Ctrl-^--> 直前のセッション（ライブなら再アタッチ。全ペインは生かしたまま） ;  --Ctrl-E--> メモ編集（閉じると同じペインへ復帰） ;  --Ctrl-W--> アクティブタブを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席） ;  --Ctrl-O--> 切替（全ペインは生かしたまま） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入の予約キーは `Ctrl-O`（切替へズームアウト）・`Ctrl-T`（在席へズームアウトしアクションメニューを開く）・`Ctrl-N`/`Ctrl-P`（タブを前後へ切替）・`Ctrl-G`（agent
> タブを追加）・`Ctrl-^`（[直前のセッションへ切替](#直前のセッションへ切り替えctrl-)）・`Ctrl-E`（[メモ編集](#セッションメモの編集)）・`Ctrl-W`（アクティブタブを閉じる）・`Ctrl-B`（[サイドバー開閉](#サイドバーの開閉ctrl-b)）です。タブの切替はどのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で
...
`アップデートがあるぴょん` ＋ 新バージョン（`v<X.Y.Z>`）を黄色・太字で重ねて表示します。差分が無い（最新と同じ・
...
│                                                                │
│  ▎ ⌂   root         │       (\(\  アップデートがあるぴょん     │  ← 右上に更新通知（うさぎアート＋メッセージ）
...
| `Tab` | キャレット位置の語を補完（一意なら確定、曖昧なら共通接頭辞まで補完し候補を列挙）。コマンド名のほか、コマンドの引数（サブコマンド・オプション）も補完する（例: `session ` → `create`/`list`/`switch`/`remove`、`session remove ` → `--force`、`issue ` → `list`/`graph`/`gantt`/`show`、`man ` → コマンド名） |
...
没入の**予約キーは `Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-G`・`Ctrl-E`・`Ctrl-W`**で、`Esc` を含む他のキーはすべてシェルへ流れます
（フォーカス中の衝突を避けるため切り替え・管理専用の予約キーが必要）。`Ctrl-O` は**単押し**で[切替](#切替switch)へズームアウトします
（リーダーキー方式は廃止）。`Ctrl-T` は**在席へズームアウトしてアクションメニュー**（terminal / agent などを選ぶ画面）を開きます（全ペインは生かしたまま。terminal タブの追加はここから選ぶ）。`Ctrl-N`/`Ctrl-P`（または `Ctrl-←`/`Ctrl-→`）は**没入のままアクティブタブ（ペイン）を前後へ切り替え**ます（`Ctrl` を伴わない素の矢印はシェルへ流れる）。`Ctrl-G` は
...
| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・`Ctrl-T`・`Ctrl-N`/`Ctrl-P`（`Ctrl-←`/`Ctrl-→`）・`Ctrl-G`・`Ctrl-E`・`Ctrl-W`・選択中の `Ctrl-C` 以外。`Esc` 含む） | そのままシェルへ転送（通常のターミナルとして操作） |
...
| マウス左クリック（タブストリップのチップ上） | そのタブ（ペイン）へ切り替える（`Ctrl-N`/`Ctrl-P` と同じく没入のまま）。アクティブタブをクリックしても何も起きない |
...
| `Ctrl-N` / `Ctrl-P`（または `Ctrl-→` / `Ctrl-←`） | 没入のままアクティブタブ（ペイン）を次／前へ切り替え（その分シェル / Agent 側の同キーは奪われる。`Ctrl` を伴わない素の矢印はシェルへ流れる） |
...
> タブ（ペイン）の切替は `Ctrl-N`/`Ctrl-P` でどのモードでも行えます（没入ではさらに `Ctrl-←`/`Ctrl-→`、切替では素の `←`/`→` も同じ）。没入では**タブストリップのチップを左クリック**しても切り替えられます。ペインの追加は
