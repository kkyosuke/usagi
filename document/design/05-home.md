...
- [モードとキー操作](#モードとキー操作)
- [コマンドスコープ](#コマンドスコープ)
- [右ペインのプレビュー（Markdown）](#右ペインのプレビューmarkdown)
- [TUI 内コマンド](#tui-内コマンド)
...
| **切替**（Switch） | セッションの選択・タブの切替・新規作成・タブを閉じる（セッションピッカー） | **左ペイン**（一覧そのものにフォーカス。カーソル以外の行は淡色） | **選択後に開く画面のプレビュー**（ライブでないセッションは在席の[アクション UI](#在席のアクション-uimenu--prompt)を `session_action_ui` 設定どおり（Menu／Prompt）に、live は**タブストリップ＋実際のターミナル画面**。先頭に status・Agent 状態、最下部にキー操作のヒント行） |
...
切替 --↑↓/jk--> セッション移動 ;  --←→/hl または Ctrl-N/Ctrl-P--> タブ切替（選択中セッションのアクティブタブを実切替） ;  --Enter--> ライブ? 没入 : 在席 ;  --t--> 在席（新規ペイン作成の起点） ;  --x--> 選択中セッションのアクティブタブ（ペイン）を閉じる ;  --c→インライン名入力→Enter--> 在席(新規) ;  --r→インライン表示名入力→Enter--> 表示名を変更 ;  --Esc--> 開いた元のモード ;  --Ctrl-O--> 統括
在席 --terminal/agent--> 没入（新規ペインを追加して入る） ;  --Ctrl-N/Ctrl-P--> タブ切替（フォーカス中セッションのアクティブタブを実切替） ;  --Esc--> 統括 ;  --Ctrl-O--> 切替
没入 --Ctrl-N/Ctrl-P--> タブ切替（没入のままアクティブペインを前後へ） ;  --Ctrl-T/Ctrl-G--> 新しい terminal / agent タブを追加（没入のまま） ;  --Ctrl-W--> アクティブタブを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席） ;  --Ctrl-O--> 切替（全ペインは生かしたまま） ;  --シェル終了--> ペインを閉じる（他ペインが残れば没入継続、最後の 1 枚なら在席）
```

> 没入の予約キーは `Ctrl-O`（切替へズームアウト）・`Ctrl-N`/`Ctrl-P`（タブを前後へ切替）・`Ctrl-T`/`Ctrl-G`（terminal / agent
> タブを追加）・`Ctrl-W`（アクティブタブを閉じる）です。タブの切替はどのモード（切替・在席・没入）でも `Ctrl-N`/`Ctrl-P` で
> 行えます（切替・在席では `←`/`→`・`h`/`l` も同じ）。タブの追加・閉じるは没入では `Ctrl-T`/`Ctrl-G`・`Ctrl-W`、切替では
> `t`・`x` で行えます。詳細は [没入のキー操作](#没入のキー操作attached--terminal--agent-実行中)。
...
- **切替（Switch）** — セッションを選ぶ／作る／タブを切り替えるためのピッカー。**キーボードは左ペイン**に移り（中央モーダルや
  ...
  **在席のアクション UI**（設定どおり Menu／Prompt）を開いて**新しいペインを追加**します。`x` で**選択中セッションのアクティブタブ（ペイン）を閉じ**、そのシェルを終了します（次フレームでタブを読み直し、ペインが残ればそのタブを、最後の 1 枚を閉じたら在席のアクション UI をプレビューします）。
  右ペインの**最下部には常にキー操作のヒント行**を表示し、プレビューの内容によらず操作キーが見えるようにしています（`c new` / `r rename` を含む全キーはフッターにも出ます）。`c` で**左ペイン内のインライン名入力**を開いてセッションを新規作成します。名前は
...
- **在席（Focus）** — 1 つのセッションを選んだ状態で、そのセッションで実行できるコマンドを**右ペイン**で操作します
  ...
  `terminal` / `agent` を実行すると**新しいペインを追加して没入**へ。`close` で在席中のセッションを強制削除し、次のセッションを選べるよう**切替**へ移ります。
...
- **没入（Attached）** — 埋め込みシェル / Agent が右ペインでライブ動作します。右ペインの上端には**タブストリップ**
  ...
  terminal など）を持てます。**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`**で（`Esc` を含む他のキーは
  すべてシェルへ流れます）、`Ctrl-O` の単押しで**切替**へズームアウト、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ
  切り替え**、`Ctrl-T`/`Ctrl-G` で**没入のまま terminal / agent タブを追加**、`Ctrl-W` で**アクティブタブを閉じ**ます
  （これらの予約キーはシェル / Agent 側のキー操作からは奪われる点に注意）。タブの追加・閉じるは切替（`t`・`x`）でも行えます。
...
（最上部にカーソル行の**ヘッダー＝名前＋status＋Agent 状態**を置きます。このヘッダーは**固定幅**で、名前・status・Agent をそれぞれ一定の桁に収める（名前が長ければ末尾を `…` でクリップ）ため、カーソルをセッション間で動かしても桁がぶれません。ライブのセッションでは**そのヘッダーと同じ行に、`│` の区切りを挟んでタブストリップ**（番号付きチップ）を並べ、`←→` の対象を識別子と一緒に読めるようにします。下にはライブでないセッションは在席のアクションメニュー、ライブのセッションは**実際のターミナル画面**を表示。**右ペイン最下部には操作キーのヒント行を常に表示**）。
...
│  + new session: feat/█  │ ↑↓ session ←→ tab t new x close                  │     ← 右ペイン最下部：キーのヒント行
│ ↑↓ session  ←→/^N^P tab  Enter focus  t new  x close tab  c new  r rename  Esc back  Ctrl-O home │  ← フッター（切替）
└──────────────────────────────────────────────────────────────────────────┘
```
...
- **Menu** — 選べるリスト。`↑↓` + `Enter`、または `t`（terminal）/ `a`（agent）でアクションを起動し、**新しいペインを追加**します。リスト末尾の `close` を選ぶと在席中のセッションを強制削除して切替へ移ります（セッションではないルート行では `close` は出ません）。
...
`[attached] Ctrl-O: switch / Ctrl-N/P: tab / Ctrl-T: terminal / Ctrl-G: agent / Ctrl-W: close` を表示します。
没入の**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`**（`Esc` を含む他のキーはすべてシェルへ流れる）で、
`Ctrl-O` の**単押し**で**切替**へズームアウト、`Ctrl-N`/`Ctrl-P` で**没入のままアクティブタブを前後へ切り替え**、
`Ctrl-T`/`Ctrl-G` で**没入のまま terminal / agent タブを追加**、`Ctrl-W` で**アクティブタブを閉じ**ます（その分シェル / Agent 側の
同キーは奪われます）。タブの追加・閉じるは切替（`t`・`x`）でも行えます。シェルが終了したとき、他のペインが残れば没入の
...
- 通知は右上を基準に重ねますが、その行に既に内容がある（セッションカードなど）場合や端末幅が足りない場合は、
  はみ出さないよう描画を見送ります。**右ペインにライブの埋め込み端末がある間（没入・切替のライブプレビュー）は通知ごと出しません**
  （[バックグラウンドタスク](#バックグラウンドタスクセッション作成削除)と同じ扱い）。
...
タブ切替・別セッションへの没入など）は一切止まりません**。実行中・完了直後のタスクは右上に **`tasks` と題した枠付きの
タスクパネル**として重ねて表示し、複数あれば縦に積み重ねます。

```text
┌────────────────────────────────────────────────────────────────┐
│                  usagi · 4 sessions                            │
│        Overview › Switch › Focus › Attached                    │
│  ▎ ⌂   root              │        ┌─ tasks ──────────────────┐ │  ← 右上のタスクパネル（枠付き・`tasks` 見出し）
│  ▎     workspace root    │        │ ⠹ 作成中… wip            │ │  ← 実行中：点字スピナー（シアン）
│    ●   main       pushed │        │ ✓ 削除完了 old           │ │  ← 完了：✓（緑）／失敗は ✗（赤）
│    ○   feat/login  local │        └──────────────────────────┘ │
└────────────────────────────────────────────────────────────────┘
```

- パネルは**枠（ボックス）で囲み**、上枠に `tasks` の見出しを埋め込みます。**枠の幅は固定**で、行の文言が
  変わっても（`作成中…` → `作成完了` など、スピナーの 1 コマごとを含む）パネルの大きさ・位置は変わりません。
  長いセッション名は枠内で省略（`…`）して幅を保ち、右上に固定表示してもガタつかない（レイアウトシフトしない）ようにしています。
- 各行は「マーク＋ラベル」。実行中は**点字スピナー（シアン）＋ `作成中…/削除中… <名前>`**、完了は **`✓`（緑）＋
  `作成完了/削除完了 <名前>`**、失敗は **`✗`（赤）＋ `作成失敗/削除失敗 <名前>`**。スピナーは経過時間から決まるので、
  キー入力がなくても回り続けます（タスク稼働中はイベントループが一定間隔で再描画する）。
- git 処理が 1 秒未満で終わることも多く、そのままだとスピナーがほとんど動かないまま結果へ切り替わって**止まって見える**ため、
  ...
  ひと回りして見えてから結果が出ます。
- 完了行は数秒（`DISMISS`）残ってから自動的に消えます。表示上限を超えた分は末尾に `… and N more` でまとめます。
- ワーカーは git 処理を**直列化**（共有ロック）して `state.json` の競合を防ぎます（同期処理のリネームも同じロックを取ります）。
  投入した順にパネルへ積まれ、終わったものからイベントループが回収して、**カーソルを動かさずに**結果をログへ出し一覧を更新します
  （作成しても自動でそのセッションへは入りません。`close` は対象セッションを離れて即座に**切替**へ移ります）。
...
- 重ね方は更新通知と同じで、右上の行に既に内容がある場合や端末幅が足りない場合は、はみ出さないよう描画を見送ります。
  右上の優先順位は **ローディングうさぎ（後述の同期処理）＞ タスクパネル ＞ 更新通知**。
- **右ペインにライブの埋め込み端末がある間（没入のアタッチ中・切替のライブプレビュー中）は、タスクパネルと更新通知を出しません**。
  埋め込み端末の行はクリップのみでパディングしないため、短い行（シェルのプロンプトや途中の出力）の右上に重なるとシェルの表示を覆ってしまい、
  さらに**常に再描画されるスピナーが端末の表示を上書きし続けて**しまうためです。ライブ端末を離れた瞬間にまた現れます。
  なお同期処理のローディングうさぎ（起動中の短い表示）は意図的な合図なので、ライブ端末の上でも最優先で見せます。
- 状態保持・時間由来のビューは純粋な `home/tasks.rs`（`TaskHandle`、テスト済み）、描画は `home/ui/`（`task_panel`）、
  ワーカースレッドの起動は計測対象外の `home/mod.rs` に実装。
...
タスクパネルと同じ右上を共有しますが、**この同期処理中はローディングうさぎを最優先**で見せ、終わると
タスクパネル（あれば）→更新通知（あれば）に戻ります。
...
| コマンド入力欄 | **統括では枠付きボックス**（HTML の input のように `❯ <入力>█` を罫線で囲み、上のヒント・下の結果帯と視覚的に切り分ける。画面が低すぎるときは単一行にフォールバック）。在席の Prompt では `❯ <入力>█`（プロンプト赤太字・入力シアン・カーソル位置はブロックカーソルで反転表示）。切替の左ペイン操作中は入り方のヒント、没入中は `● live terminal`（緑） | 統括：枠付きボックス／Prompt：プロンプト赤太字・入力シアン／切替：淡色／没入：緑 |
| フッター | モード別・状態別の操作ヘルプ。統括は先頭に `[workspace]`、在席は `[session: <名前>]` を表示。切替は `↑↓ move / Enter select / c new / Esc back / Ctrl-O home`、没入は `Ctrl-O: switch / Ctrl-N/P: tab / Ctrl-T: terminal / Ctrl-G: agent / Ctrl-W: close` を案内 | 淡色（dim） |
...
間ずっと生かし続けます。1 セッションは複数ペイン（agent と並行して terminal など）を持て、プールは各 worktree の
ペイン一覧とアクティブペインを管理します。別セッションへの切り替えやモード遷移でシェルは終了せず、裏で `claude` が
動き続けます。

没入の**予約キーは `Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`**で、`Esc` を含む他のキーはすべてシェルへ流れます
（フォーカス中の衝突を避けるため切り替え・管理専用の予約キーが必要）。`Ctrl-O` は**単押し**で[切替](#切替switch)へズームアウトします
（リーダーキー方式は廃止）。`Ctrl-N`/`Ctrl-P` は**没入のままアクティブタブ（ペイン）を前後へ切り替え**ます。`Ctrl-T`/`Ctrl-G` は
**没入のまま新しいターミナル / agent タブを追加**して、そのタブをアクティブにします。`Ctrl-W` は**アクティブタブを閉じ**ます
（他ペインが残れば次のタブへ、最後の 1 枚なら在席へ）。これらの予約キーの分だけシェル / Agent 側の同キーは奪われます。
...
| キー | 動作 |
|---|---|
| 任意のキー（`Ctrl-O`・`Ctrl-N`/`Ctrl-P`・`Ctrl-T`/`Ctrl-G`・`Ctrl-W`・選択中の `Ctrl-C` 以外。`Esc` 含む） | そのままシェルへ転送（通常のターミナルとして操作） |
...
| `Ctrl-T` | 没入のまま新しいターミナルタブを追加し、そのタブをアクティブにする（その分シェル / Agent 側の同キーは奪われる） |
...
> タブ（ペイン）の切替は `Ctrl-N`/`Ctrl-P` でどのモードでも行えます（切替では `←`/`→` も同じ）。ペインの追加・閉じるは
> 没入では `Ctrl-T`/`Ctrl-G`（追加）・`Ctrl-W`（閉じる）で、切替では `t`（追加の起点）・`x`（閉じる）で行えます
> （[切替](#切替switch)で `←`/`→`・`Ctrl-N`/`Ctrl-P` がタブ、`↑`/`↓` がセッション）。
> 別セッションへ切り替えても元のシェルはプールに残って動き続けるため、戻れば続きから操作できます。同じ
> セッション内でも、agent を動かしながら `Ctrl-T` で terminal を追加するなど、複数ペインをタブで行き来できます。
...
右ペインは、モードごとの中身（[統括＝空白／切替＝選択後プレビュー／在席＝アクション UI／没入＝埋め込み端末](#モードと状態遷移統括切替在席没入)）に加えて、
**Markdown ファイルのプレビュー**を第 3 の表示状態として持ちます。`preview <path|name>` コマンド（[TUI 内コマンド](#tui-内コマンド)、Workspace スコープ）で開きます。

- **開く**: 統括で `preview README` のように実行すると、ワークスペース内の Markdown ファイルを読み込んで右ペインに整形表示します。
  ファイルの解決・読み込みは infrastructure 層（`infrastructure/markdown_file.rs`）、Markdown → スタイル付き行の変換は
  純粋な `presentation/tui/markdown.rs`（テスト済み）、行の描画は `home/ui/panes.rs` が担当します。
- **レイアウト**: 最上部の 1 行はヘッダー（ファイルパス。スクロール時は `(表示開始-表示終了/総数)` も表示）。その下にレンダリング結果を
  ペイン高さに合わせて表示します。各行はペイン幅でクリップするため、**右ペインからはみ出さず、モード切替でレイアウトがずれません（CLS が起きません）**。
- **スクロール方針**: プレビューは内容が縦に長くなり得るため、TUI 全体はスクロールさせない方針を保ったまま、**プレビューペイン内のスクロールだけを許可**します
  （埋め込みターミナルがペイン内にスクロールバックを持つのと同じ扱い）。表示中はキー入力をプレビューが捕捉し、`↑↓`（`j`/`k`）で 1 行、
  `PgUp`/`PgDn` でページ単位にスクロールし、最終行がペインに収まる位置でクランプします。`Esc` / `Enter` / `q` で閉じて元の右ペインに戻ります。
  フッターには `[preview] ↑↓ scroll / PgUp/PgDn page / Esc / q: close` を表示します。
- **整形の範囲**: 見出し（`#`〜`######`、レベルで色分け）・箇条書き（`•`）・番号付きリスト・引用（`│`）・フェンス済みコードブロック、
  インラインの強調 / 斜体 / コード / リンク文字。装飾は `console` の色・装飾で表現し、純粋なレンダリング関数に切り出してテストします。
- **diff プレビュー**: セッションと main の差分表示（`preview diff`）は別 issue（フルスクリーン Diff ビューア）に委ね、現状は案内のみを出します。

## TUI 内コマンド

統括の下部コマンドライン（Workspace スコープ）／在席の右ペイン（Session スコープ）で実行できる
TUI 内コマンド（`session` / `issue` / `preview` / `terminal` / `agent` / `config` ＋ 共通の `man` / `history` / `clear` / `quit`）の
...
- `Enter` で選択したセッションを**一括削除**。各セッションは[バックグラウンドタスク](#バックグラウンドタスクセッション作成削除)として
  ワーカースレッドへ投入され（モーダルは即座に閉じ、画面は止まらない）、右上のタスクパネルに積まれて順に処理される。
