...
| `usagi config` | Config 画面を対話的に表示する（scope を Tab で切替、テーマを `←→` で編集し、`S` で保存。`Esc` で Welcome へ、`Ctrl-C` で終了） |
...
workspace scope を切り替え、`←→` で Theme を編集する。scope ごとに独立した draft と dirty state を持ち、
`S` で明示保存する。保存失敗時は draft を保ち error を表示するため、同じ `S` で安全に再試行できる。
Esc で Welcome へ戻る（`usagi config` から直接開いた場合も Welcome が home）。合成ルートは対話ループの
