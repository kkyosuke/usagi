...
| コマンド | 動作 |
|---|---|
| `usagi` / `usagi hop` | Welcome 画面を対話的に表示する（`↑↓` で選択移動、Open で workspace 一覧へ、New で新規作成フォームへ、`q` / Ctrl-C で終了） |
...
対話ループを持たない起動画面（Workspace / Config / Doctor）は、選択された画面名と Workspace の
