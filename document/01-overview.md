...
| `usagi clean` | 登録 workspace、workspace ごとの daemon lifecycle data、Git worktree と `usagi/*` branch、**消えた build から取り残された daemon / bootstrap broker process**、**消滅した generation が握ったままの capacity claim** を照合し、紐付いていない resource を dry-run 表示する。process の所属は PID と process-start identity の組で照合し、retired generation の stale PID は現行 process の所属根拠にしない。`--apply` で安全な候補を削除し、dirty worktree・未マージ branch・session を 1 つも記録していない lifecycle document が根拠の候補・取り残された daemon process は `--force` も指定した場合だけ削除する。capacity claim は「どの owner shard も説明せず、かつ generation registry が owner を載せていない」ものだけを解放する（[説明されない claim の回収](05-daemon.md#説明されない-claim-の回収)）。workspace fence を取得できない使用中 workspace は変更しない |
...
Switch の `p`、Closeup の `Ctrl-O v`（または `Ctrl-O Ctrl-V`）、または右端の PR 表示のクリックは、同じ snapshot projection から
対象 session に dismissed でない PR がある場合だけ Pull Request モーダルを開く。新しい PR URL の検知時は、別の modal や Director drawer が前面になければ
