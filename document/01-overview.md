...
| コマンド | 動作 |
|---|---|
| `usagi` / `usagi hop` | Welcome TUI を起動画面に選ぶ |
...
TUI の concrete runner である `BannerScreenRunner` は、選択された起動画面名と
Workspace の path をバナーとして表示する。初期画面の選択は TUI の application 層が行う。
