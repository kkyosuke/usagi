...
```jsonc
{
  ...
  "agent_cli": "claude",          // 起動する AI エージェント CLI（claude | codex | gemini）
...
- `session remove` でセッションを破棄すると、その worktree の phase ファイルも会話履歴（Claude の transcript / Codex の rollout）と
