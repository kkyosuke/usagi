...
```jsonc
{
  ...
  "session_labels": [             // 切替で手で付けるステータスラベルのマスタ（省略で既定 5 件、[] で休止）
...
[プロジェクト選択画面（Open）](../design/02-open.md#統合uniteモードで開く)で**最後にまとめて開いたワークスペースの集合**を保持し、次回 Open 画面がその顔ぶれをあらかじめチェック状態にするために使います。`resume-focus/` がワークスペース別なのに対し、これはマシン全体で 1 つの事実なので、ハッシュ名ではなくデータディレクトリ直下の 1 ファイルに置きます。
