...
CLI / TUI / MCP のどの経路で発生したエラーでも横断的に書き出します。記録対象は次の 4 系統です。

- **`main` まで伝播したエラー**: CLI 各コマンドが返す `Err`。
- **TUI 内の長時間処理の失敗**: `main` に到達せず画面表示だけで消えてしまう失敗も書き出します。
  対象はセッションの作成・削除、エージェント / ターミナルの起動（PTY spawn を含む）で、起動が
  そもそも live なペインに至らなかった場合も記録します。これにより「セッションが失敗した」事象を
  画面を閉じた後からでも追跡できます。
