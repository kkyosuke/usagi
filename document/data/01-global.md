...
- 既に登録の無い名前は Open 画面のプリチェック時に無視されるため、古いエントリが残っていてもエラーになりません。一時的な状態のため `version` は持たず、読み取り失敗・破損時は空集合（何もプリチェックしない）に縮退します。`infrastructure/unite_store.rs` が save/load を担います。
