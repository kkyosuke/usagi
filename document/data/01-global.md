...
ここへ置き、ホーム画面がそのセッションのエージェントペインを次にフレッシュ起動するとき（設定
[`autostart_queued_prompts`](../05-settings.md#設定項目) が有効なら TUI 稼働中の自動 spawn 時）に取り出して、
...
- `{ "worktree": "<path>", "prompt": "<text>", "retry": null | { "attempts": N, "next_retry_unix_secs": N, "last_error": "...", "dead": bool } }`。
  永続的な状態ではないため `version` は持ちません。`retry` は自動起動の失敗時だけ入り、指数 backoff と最大 5 回の
  試行上限を保持します。`dead = true` の prompt は自動起動の対象外ですが、ファイルは残るため silent drop しません。
...
- 現行形式は `<hash>/meta.json` と `<hash>/<連番>.json` です。`meta.json` は
  `{ "worktree": "<path>", "next_id": N, "count": N, "bytes": N }`、item は `{ "prompt": "<text>" }` です。
  旧形式の `{ "worktree": "<path>", "prompts": ["<text>", …] }` は最初の append/drain 時に同じ store lock の下で
  現行形式へ移行します。永続的な状態ではないため `version` は持ちません。
- worktree ごとの live queue 上限は **64 件 / 512 KiB** です。1 回の delivery batch は **4 件 / 256 KiB** までで、
  残りは同じ順序でキューに残ります。追記は順序を保持し、取り出しは読み取りと同時に削除する**ワンショット**です。
