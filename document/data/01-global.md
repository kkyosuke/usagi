...
├── agent-live-prompts/ # session_send がキューした、起動中 Agent pane へ送るプロンプト（worktree 別）
...
MCP の [`session_send`](../03-commands/03-mcp.md#session_send-の挙動) がキューしたプロンプトを worktree
ごとに保持する**一時キャッシュ**です。`usagi mcp` プロセスは動作中 TUI の PTY を直接操作できないため、プロンプトを
ここへ追記し、ホーム画面の監視スレッドが対象セッションの live agent ペインへ貼り付けて Enter します
（[../03-commands/03-mcp.md#session_send-の挙動](../03-commands/03-mcp.md#session_send-の挙動) が正本）。
