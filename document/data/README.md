...
| 2 | workspace 毎（リポジトリ単位） | [02-workspace.md](02-workspace.md) | `<repo>/.usagi/` の `state.json` / `settings.json` / `history.jsonl` | そのリポジトリの worktree / ブランチの状態、プロジェクト固有の設定上書き、コマンド実行履歴 |
...
**タスク issue とエージェントのメモリは git で共有**するため、保存フォーマットをそれぞれ
[03-issues.md](03-issues.md) / [04-memory.md](04-memory.md) に分けています。役割が重ならないよう、
保存場所もファイルも分離しています。
...
| infrastructure | `infrastructure/history_store.rs` | リポジトリ `<repo>/.usagi/history.jsonl` の load/append（`HistoryStore`） |
...
| infrastructure | `infrastructure/agent_live_prompt_store.rs` | グローバル `~/.usagi/agent-live-prompts/` の session_send キュー（append/take_all） |
